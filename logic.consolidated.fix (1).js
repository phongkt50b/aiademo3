...
function performCalculations(state) {
    ...
    const mdp3FeeStartMarker = '';
    const allPersonsForFees = [state.mainPerson, ...state.supplementaryPersons].filter(p=>p);
    ...
    window.personFees = {};
    const keys = Object.keys(fees.byPerson);
    keys.forEach(pid => {
...
function renderUI() {
    ...
    // Update age for MDP3 'other' person if present
    (function(){ 
      const other = document.getElementById('person-container-mdp3-other'); 
      if (other) { 
        const dob = other.querySelector('.dob-input'); 
        const ageSpan = other.querySelector('.age-span'); 
        if (dob && ageSpan) { 
          const { age } = calculateAge(dob.value); 
          ageSpan.textContent = age || 0; 
        } 
      } 
    })();
// Không reset 0 khi invalid; vẫn hiển thị phí, validation chỉ gắn lỗi input
...
function renderSupplementaryProductsForPerson(customer, mainProductKey, mainPremium, container) {
    ...
    // Auto default for SCL: if empty, set 'nang_cao'
    if (sclSection) {
      const prog = sclSection.querySelector('.health-scl-program');
      if (prog && !prog.value) prog.value = 'nang_cao';
      // If TTA & main person: auto-check SCL and show options
...
function runAllValidations(state) {
    ...
    try {
        if (window.MDP3 && MDP3.isEnabled && MDP3.isEnabled() && MDP3.getSelectedId && MDP3.getSelectedId()==='other') {
          const other = document.getElementById('person-container-mdp3-other');
          if (other) {
            const dob = other.querySelector('.dob-input');
            if (!validateDobField(dob)) isValid = false;
          }
        }
      } catch(e){}
      return isValid;
...
function updateTargetAge() {
    ...
    if (mainProduct === 'TRON_TAM_AN') { targetAgeInput.value = mainPersonInfo.age + 10 - 1;
...
function generateSummaryTable() {
    ...
    let grand = { per:0, eq:0, base:0, diff:0 };const pushRow = (acc, personName, prodName, stbhDisplay, years, baseAnnual, isRider)=>{
    ...
    for (const p of persons){
      ...
      grand.per += acc.per; grand.eq += acc.eq; grand.base += acc.base; grand.diff += acc.diff;}    html += `<tr class="bg-gray-100 font-bold">
        ...
        + (!isAnnual ? `<td class="p-2 border text-right">${grand.diff!==0?`<span class="text-red-600 font-bold">${fmt(grand.diff)}</span>`:'0'}</td>` : '') 
        + `</tr>`;
    ...
    for (const r of rowsData){ sumMain += r.baseAnnualMain; sumExtra += r.extra; sumTotalQN += r.totalQN; for (let i=0;i<persons.length;i++){ sumSuppCols[i] += r.perPersonSuppYearAF[i]; }if (!isAnnual){ sumTotalPeriod += r.totalPeriod; sumDiff += r.diff; }
