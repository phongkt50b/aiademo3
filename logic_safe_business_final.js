    ...
    function collectSupplementProducts(personInfo, container) {
      ...
      if (sclSec && sclSec.querySelector('.health-scl-checkbox')?.checked) {
        ...
        if (baseAnnual > 0) {
          ...
          const scopeVal = sclSec.querySelector('.health-scl-scope')?.value || '';
const scopeLabel = ({main_vn:'Trong nước', main_intl:'Nước ngoài', international:'Nước ngoài', domestic:'Trong nước'}[scopeVal]) || '';
const hasOP = !!sclSec.querySelector('.health-scl-outpatient')?.checked;
const hasDental = !!sclSec.querySelector('.health-scl-dental')?.checked;
const optBits = [];
if (scopeLabel) optBits.push(scopeLabel);
if (hasOP) optBits.push('Ngoại trú');
if (hasDental) optBits.push('Nha khoa');
const cfgText = optBits.length ? ` (${optBits.join(', ')})` : '';
const productName = `Sức khoẻ Bùng Gia Lực${programLabel ? ' - ' + programLabel : ''}${cfgText}`;
    ...
    container.innerHTML = summaryHtml + tableHtml + `<div class="mt-4 text-center"><button id=\"export-html-btn\" class=\"bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 mr-2\">Xuất HTML</button><button id=\"export-pdf-btn\" class=\"bg-stone-600 text-white px-4 py-2 rounded hover:bg-stone-700\">Xuất PDF</button></div>`;
try {
  if (window.__fixPart1Table) { window.__fixPart1Table(container, periods, factor); }
} catch(err) { console.error('fixPart1Table error', err); }

// build export document from current modal content
function __buildExportDoc(){
  const host = document.getElementById('summary-content-container');
  const clone = host.cloneNode(true);
  clone.querySelectorAll('#export-html-btn,#export-pdf-btn').forEach(el=>el.remove());
  const css = `body{font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;margin:24px;color:#111}table{border-collapse:collapse;width:100%;font-size:14px}th,td{border:1px solid #ddd;padding:8px;text-align:right}th{text-align:left;background:#f3f4f6}td:first-child,th:first-child{text-align:left}.text-danger{color:#d00;font-weight:700}`;
  ...
  const ymd = `${y}-${m}-${d}`;
  const html = `<!doctype html><html lang=\"vi\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>Bảng tóm tắt quyền lợi & phí - ${ymd}</title><style>${css}${printCss}</style></head><body>${clone.innerHTML}</body></html>`;
  return {html, ymd};
}
const __htmlBtn = document.getElementById('export-html-btn');
if (__htmlBtn && !__htmlBtn.__bind) { __htmlBtn.__bind=true; __htmlBtn.addEventListener('click',()=>{ const {html, ymd} = __buildExportDoc(); const blob=new Blob([html],{type:'text/html;charset=utf-8'}); const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download=`bang-tom-tat_${ymd}.html`; document.body.appendChild(a); a.click(); setTimeout(()=>{URL.revokeObjectURL(url); a.remove();},0); }); }
const __pdfBtn = document.getElementById('export-pdf-btn');
if (__pdfBtn && !__pdfBtn.__bind) { __pdfBtn.__bind=true; __pdfBtn.addEventListener('click',()=>{ const {html} = __buildExportDoc(); const iframe=document.createElement('iframe'); iframe.style.position='fixed'; iframe.style.right=0; iframe.style.bottom=0; iframe.style.width=0; iframe.style.height=0; iframe.style.border=0; document.body.appendChild(iframe); iframe.onload=()=>{ setTimeout(()=>{ try{ iframe.contentWindow.focus(); iframe.contentWindow.print(); } finally { setTimeout(()=>iframe.remove(), 1000); } }, 300); }; iframe.srcdoc = html; }); }
...
(function(){
  ...
  window.generateSummaryTable = customGenerateSummary;
  // Force using the comprehensive implementation defined earlier
  if (window.__generateSummaryTableOld) {
    window.generateSummaryTable = window.__generateSummaryTableOld;
  }
