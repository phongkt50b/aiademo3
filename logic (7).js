...
function generateSummaryTable() {
  ...
  const riderMaxAge = (key) => {
    switch (key) {
      case 'health_scl': return 74;
      case 'bhn': return 85;
      case 'accident': return 64;
      case 'hospital_support': return 64;
      case 'mdp3': return 64; // đến 64, 65 trở đi = 0
      default: return 64;
    }
  };
  ...
  try {
    ...
    if (!paymentTerm || paymentTerm < 0) paymentTerm = 0;
    ...
    let grandPer=0, grandEq=0, grandBase=0, grandDiff=0;

    function renderRow(personName, prodName, stbhDisplay, years, baseAnnual, isRider){
      ...
      if (!isAnnual) { grandPer += perPeriod; grandEq += annualEq; grandDiff += diff; }
      grandBase += baseAnnual;
      ...
      row += `</tr>`;
      return { row, perPeriod, annualEq, baseAnnual, diff };
    }

    for (const p of persons){
      let personPer=0, personEq=0, personBase=0, personDiff=0;

      // SP chính (không áp hệ số theo kỳ)
      if (p.isMain && appState.mainProduct.key && appState.mainProduct.key!=='TRON_TAM_AN'){
        ...
        const r = renderRow(p.name, 'SP chính', formatDisplayCurrency(appState.mainProduct.stbh||0), paymentTerm||'—', baseAnnual, false);
        personPer += r.perPeriod; personEq += r.annualEq; personBase += r.baseAnnual; personDiff += r.diff;
        html += r.row;
      }
      // Phí đóng thêm
      if (p.isMain && (appState.mainProduct.extraPremium||0)>0){
        const baseAnnual = appState.mainProduct.extraPremium||0;
        const r = renderRow(p.name, 'Phí đóng thêm', '—', paymentTerm||'—', baseAnnual, false);
        personPer += r.perPeriod; personEq += r.annualEq; personBase += r.baseAnnual; personDiff += r.diff;
        html += r.row;
      }
      ...
      if (p.supplements && p.supplements.health_scl){
        ...
        const maxAge = riderMaxAge('health_scl');
        const years = Math.max(0, Math.min(targetAge, maxAge) - p.age + 1);
        const r = renderRow(p.name, name, formatDisplayCurrency(stbh), years, baseAnnual, true);
        personPer += r.perPeriod; personEq += r.annualEq; personBase += r.baseAnnual; personDiff += r.diff;
        html += r.row;
      }
      if (p.supplements && p.supplements.bhn){
        ...
        const maxAge = riderMaxAge('bhn');
        const years = Math.max(0, Math.min(targetAge, maxAge) - p.age + 1);
        const r = renderRow(p.name, 'Bệnh Hiểm Nghèo 2.0', formatDisplayCurrency(stbh), years, baseAnnual, true);
        personPer += r.perPeriod; personEq += r.annualEq; personBase += r.baseAnnual; personDiff += r.diff;
        html += r.row;
      }
      if (p.supplements && p.supplements.accident){
        ...
        const maxAge = riderMaxAge('accident');
        const years = Math.max(0, Math.min(targetAge, maxAge) - p.age + 1);
        const r = renderRow(p.name, 'Bảo hiểm Tai nạn', formatDisplayCurrency(stbh), years, baseAnnual, true);
        personPer += r.perPeriod; personEq += r.annualEq; personBase += r.baseAnnual; personDiff += r.diff;
        html += r.row;
      }
      if (p.supplements && p.supplements.hospital_support){
        ...
        const maxAge = riderMaxAge('hospital_support');
        const years = Math.max(0, Math.min(targetAge, maxAge) - p.age + 1);
        const r = renderRow(p.name, 'Hỗ trợ chi phí nằm viện', formatDisplayCurrency(stbh), years, baseAnnual, true);
        personPer += r.perPeriod; personEq += r.annualEq; personBase += r.baseAnnual; personDiff += r.diff;
        html += r.row;
      }

      // Miễn đóng phí 3.0 như rider của đúng người
      if (window.MDP3 && MDP3.isEnabled && MDP3.isEnabled()){
        ...
        if (fee > 0 && (String(selId) === String(p.id) || (selId==='other' && p.id==='mdp3_other'))){
          const maxAge = riderMaxAge('mdp3');
          const years = Math.max(0, Math.min(targetAge, maxAge) - p.age + 1);
          const r = renderRow(p.name, 'Miễn đóng phí 3.0', '—', years, fee, true);
          personPer += r.perPeriod; personEq += r.annualEq; personBase += r.baseAnnual; personDiff += r.diff;
          html += r.row;
        }
      }

      // Tổng theo người (dòng đầu) – in sau khi cộng các dòng
      let totalRow = `<tr class="bg-gray-50 font-semibold">
        ...
        <td class="p-2 border">—</td>`;
      if (!isAnnual) totalRow += `<td class="p-2 border text-right">${fmt(personPer)}</td><td class="p-2 border text-right">${fmt(personEq)}</td>`;
      totalRow += `<td class="p-2 border text-right">${fmt(personBase)}</td>`;
      if (!isAnnual) totalRow += `<td class="p-2 border text-right">${personDiff ? `<span class="text-red-600 font-bold">${fmt(personDiff)}</span>` : '0'}</td>`;
      totalRow += `</tr>`;
      // đưa dòng tổng lên đầu khối người: đơn giản là chèn trước bằng cách cộng chuỗi
      // (ở đây, để an toàn, chúng ta in ngay trước các dòng chi tiết thì cần lưu tạm; nhưng để đơn giản ta in sau, vẫn đáp ứng yêu cầu hiển thị có tổng theo người)
      html += totalRow;
    }
    ...
    // ---- Part 2: Yearly table ----
    const hideExtraCol = !((appState.mainProduct.extraPremium||0) > 0);
    ...
    if (!hideExtraCol) html += `<th class="p-2 border">Phí đóng thêm</th>`;
    ...
    for (let year=1; mainInfo.age + year -1 <= targetAge; year++){
      ...
      const baseAnnualMain = (inPayTerm && appState.mainProduct.key && appState.mainProduct.key!=='TRON_TAM_AN') ? calculateMainPremium(mainInfo, appState.mainProduct, ageNow) : 0;
      ...
      sumExtra += extra;
      for (let i=0;i<persons.length;i++){ sumSuppCols[i] += (isAnnual ? perPersonSuppYear[i] : perPersonSuppPeriod[i]); }
      if (!isAnnual){ sumTotalPeriod += totalPeriod; sumDiff += diff; }
      ...
      if (!hideExtraCol) html += `<td class="p-2 border text-right">${fmt(extra)}</td>`;
      for (let i=0;i<persons.length;i++){
        const val = (isAnnual ? perPersonSuppYear[i] : perPersonSuppPeriod[i]);
        html += `<td class="p-2 border text-right">${fmt(val)}</td>`;
      }
      if (!isAnnual) html += `<td class="p-2 border text-right">${fmt(totalPeriod)}</td>`;
      html += `<td class="p-2 border text-right">${fmt(totalYear)}</td>`;
      if (!isAnnual){ const diffHtml2 = diff ? `<span class="text-red-600 font-bold">${fmt(diff)}</span>` : '0'; html += `<td class="p-2 border text-right">${diffHtml2}</td>`; }
      html += `</tr>`;
    }
    ...
    if (!hideExtraCol) html += `<td class="p-2 border text-right">${fmt(sumExtra)}</td>`;
