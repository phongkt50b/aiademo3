  ...
  try {
    ...
    for (let year=1; mainInfo.age + year -1 <= targetAge; year++){
      ...
      html += `<tr>
        ...
        ${ showExtraCol ? `<td class="p-2 border text-right">${fmt(extra)}</td>` : `` }`;
    ...
    html += `<tr class="font-semibold bg-gray-50">
      ...
      ${ showExtraCol ? `<td class="p-2 border text-right">${fmt(sumExtra)}</td>` : `` }`;
...
// --- UI polish for summary pane font sizes & diff color already handled ---
document.addEventListener('DOMContentLoaded', ()=>{
  const pane = document.getElementById('results-container');
  if (pane){
    pane.querySelectorAll('.text-xs').forEach(el=>el.classList.remove('text-xs'));
  }
});
