...
function performCalculations(state) {
    ...
    const allPersonsForFees = [state.mainPerson, ...state.supplementaryPersons].filter(Boolean);
    ...
    // Recompute window.personFees AFTER assigning MDP3 to byPerson
    window.personFees = {};
    Object.keys(fees.byPerson).forEach(pid => {
      const pMain = (pid === state.mainPerson.id) ? (fees.baseMain + fees.extra) : 0;
      const supp = fees.byPerson[pid]?.supp || 0;
      window.personFees[pid] = { main: pMain, mainBase: (pid === state.mainPerson.id ? fees.baseMain : 0), supp, total: pMain + supp };
    });
...
// Luôn hiển thị phí; validate chỉ để gắn lỗi input (không reset 0)
...
function renderSupplementaryProductsForPerson(customer, mainProductKey, mainPremium, container) {
    ...
    // Auto SCL cho TTA và mặc định 'nang_cao'
    if (sclSection) {
      const prog = sclSection.querySelector('.health-scl-program');
      if (prog && !prog.value) prog.value = 'nang_cao';
      if (isTTA && customer.isMain) {
        const cb = sclSection.querySelector('.health_scl-checkbox') || sclSection.querySelector('.health-scl-checkbox');
        if (cb && !cb.checked) {
          cb.checked = true;
    ...
    // Auto defaults for TTA main: check and default program
    if (isTTA && customer.isMain && sclSection) {
        const checkbox = sclSection.querySelector('.health_scl-checkbox') || sclSection.querySelector('.health-scl-checkbox');
    ...
    if (sclSection) {
        const programSel = sclSection.querySelector('.health-scl-program');
        if (programSel && !programSel.value) programSel.value = 'nang_cao';
    }
}
    ...
    return CONFIG.supplementaryProducts.map(prod => {
        ...
        if (prod.id === 'health_scl') {
            ...
            optionsHtml = `<div>
                ...
                <input type="text" class="form-input ${prod.id}-stbh ${prod.id==='hospital_support'?'hospital-support-stbh':''}" placeholder="${
                    prod.id==='bhn' ? '≥ 200.000.000, ≤ 5.000.000.000' :
                    prod.id==='accident' ? '≥ 10.000.000, ≤ 8.000.000.000' :
                    prod.id==='hospital_support' ? 'Bội số 100.000 đ/ngày' : 'Nhập STBH'
                }">
               </div>
               <p class="hospital-support-validation text-sm text-gray-500 mt-1">${
                 prod.id==='hospital_support' ? 'Lưu ý: Bội số 100.000 đ/ngày; giới hạn theo tuổi & tổng hỗ trợ.' : ''
               }</p>`;
        }
        return `
...
const diffStr = (!isAnnual ? (grand.diff!==0 ? `<span class="text-red-600 font-bold">${fmt(grand.diff)}</span>` : '0') : '');
html += `<tr class="bg-gray-100 font-bold">
        <td class="p-2 border">Tổng cộng</td>
        <td class="p-2 border">—</td>
        <td class="p-2 border">—</td>
        <td class="p-2 border">—</td>` 
        + (!isAnnual ? `<td class="p-2 border text-right">${fmt(grand.per)}</td><td class="p-2 border text-right">${fmt(grand.eq)}</td>` : '') 
        + `<td class="p-2 border text-right">${fmt(grand.base)}</td>` 
        + (!isAnnual ? `<td class="p-2 border text-right">${diffStr}</td>` : '') 
        + `</tr>`;
    ...
    for (let year=1; mainInfo.age + year -1 <= targetAge; year++){
      ...
      for (let pi=0; pi<persons.length; pi++){
        ...
        perPersonSuppYearAF.push(Math.round(sYear*suppFactor/1000)*1000);
      }

      const totalSuppPeriod = perPersonSuppPeriod.reduce((a,b)=>a+b,0);
      const totalSuppYear = perPersonSuppYear.reduce((a,b)=>a+b,0);
      const totalSuppYearAF = perPersonSuppYearAF.reduce((a,b)=>a+b,0);
      const totalQN = baseAnnualMain + extra + totalSuppYearAF; // Quy năm (main+extra no factor, riders AF)
      const totalYear = baseAnnualMain + extra + totalSuppYear;  // Theo năm
      const diff = totalQN - totalYear;

      rowsData.push({ year, ageNow, baseAnnualMain, extra, perPersonSuppYearAF, perPersonSuppYear, totalQN, totalYear, diff });
    }
    ...
    html += `<th class="p-2 border">Nếu đóng theo năm</th>`;
    html += `<th class="p-2 border">Chênh lệch</th>`;
    html += `</tr></thead><tbody>`;

    let sumMain=0,sumExtra=0,sumSuppCols=new Array(persons.length).fill(0);
    let sumTotalQN=0,sumTotalYear=0,sumDiff=0;

    for (const r of rowsData){
      sumMain += r.baseAnnualMain;
      sumExtra += r.extra;
      for (let i=0;i<persons.length;i++){ sumSuppCols[i] += r.perPersonSuppYearAF[i]; }
      sumTotalYear += r.totalYear;
      sumTotalQN += r.totalQN;
      sumDiff += r.diff;
      const totalQN = r.totalQN;
      ...
      for (let i=0;i<persons.length;i++){
        const val = r.perPersonSuppYearAF[i];
    ...
    html += `<td class="p-2 border text-right"><b>${fmt(sumTotalQN)}</b></td>`;
    html += `<td class="p-2 border text-right">${fmt(sumTotalYear)}</td>`;
    html += `<td class="p-2 border text-right"><b>${fmt(sumTotalQN - sumTotalYear)}</b></td>`;
    html += `</tr></tbody></table>`;
...
/* ====== PATCH: Đồng bộ danh sách "Xem từng người" chỉ gồm Riders + MĐP3 ====== */
(function(){
  ...
  function renderSuppList(){
    ...
    if (!box) return;
    const persons = [];
    const mainCont = $('main-person-container');
    const mainInfo = collectPersonData ? collectPersonData(mainCont, true) : null;
    ...
    const byPerson = (window.appState && window.appState.fees && window.appState.fees.byPerson) || {};
    const rows = persons.map(p=>{
      const details = (byPerson[p.id] && byPerson[p.id].suppDetails) || {};
      let sum = 0;
      ...
      return `<div class="flex justify-between"><span>${sanitizeHtml(p.name||'Người')}</span><span>${fmt(sum)}</span></div>`;
    });
  ...
  function ensureSuppListReRender(){
    try {
      ...
      $('mdp3-section')?.addEventListener('change', (e)=>{
        if (e.target && (e.target.name==='mdp3-target' || e.target.matches('select') || e.target.id==='mdp3-enable')) {
          setTimeout(renderSuppList, 0);
        }
      });
  ...
  document.addEventListener('DOMContentLoaded', ensureSuppListReRender, {once:true});
})();
...
(function(){
  function generateSummaryTableV2(){
    ...
    if (productKey === 'TRON_TAM_AN') paymentTerm = 10;
    if (productKey === 'AN_BINH_UU_VIET') {
      paymentTerm = parseInt(document.getElementById('abuv-term')?.value || '0',10) || 0;
    }
    const minByKey = (k)=>{
    ...
    const minTargetAge = mainInfo.age + paymentTerm;
    if (!targetAge || targetAge < minTargetAge) {
      container.innerHTML = `<div class="text-red-600">Tuổi mục tiêu phải ≥ ${minTargetAge}.</div>`;
    ...
    // MDP3 as person (other)
    const mdpEnabled = window.MDP3 && MDP3.isEnabled && MDP3.isEnabled();
    const mdpTargetId = mdpEnabled ? (MDP3.getSelectedId && MDP3.getSelectedId()) : null;
    const mdpFeeYear = mdpEnabled ? (MDP3.getPremium && MDP3.getPremium()) : 0;
    if (mdpEnabled && mdpTargetId === 'other') {
      // make synthetic person
      const form = document.getElementById('person-container-mdp3-other');
      let ageOther = mainInfo.age;
      if (form) {
        const info = getCustomerInfo(form, false);
        ageOther = info.age || mainInfo.age;
      }
      persons.push({ id:'mdp3_other', isMain:false, name:'Người khác (MĐP3)', gender:'Nam', age:ageOther, supplements:{} });
    }
    ...
    for (const p of persons){
      ...
      if (p.isMain && appState.mainProduct.key){
        const baseAnnual = calculateMainPremium(p, appState.mainProduct);
        const stbh = fmt(appState.mainProduct.stbh|| (appState.mainProduct.key==='TRON_TAM_AN'?100000000:0));
        rows.push( pushRow(acc, p.name, 'Sản phẩm chính', stbh, paymentTerm||'—', baseAnnual, false) );
      }
      // Extra
      if (p.isMain && (appState.mainProduct.extraPremium||0)>0){
        rows.push( pushRow(acc, p.name, 'Phí đóng thêm', '—', paymentTerm||'—', appState.mainProduct.extraPremium||0, false) );
      }
      // Riders
      if (p.supplements && p.supplements.health_scl){
        const scl = p.supplements.health_scl;
        ...
        const prodName = `Sức khoẻ Bùng Gia Lực – ${programName} (${scopeStr})`;
        const baseAnnual = calculateHealthSclPremium(p, appState.fees.baseMain, 0);
        const stbh = getHealthSclStbhByProgram(scl.program);
        const years = Math.max(0, Math.min(targetAge, riderMaxAge('health_scl')) - p.age + 1);
        rows.push( pushRow(acc, p.name, prodName, fmt(stbh), years, baseAnnual, true) );
      }
      if (p.supplements && p.supplements.bhn){
        const stbh = num(p.supplements.bhn.stbh);
        const baseAnnual = calculateBhnPremium(p, appState.fees.baseMain, 0);
      ...
      if (mdpEnabled && mdpFeeYear>0){
        if (mdpTargetId === p.id || (mdpTargetId==='other' && p.id==='mdp3_other')){
          const years = Math.max(0, Math.min(targetAge, riderMaxAge('mdp3')) - p.age + 1);
    ...
    for (let year=1; mainInfo.age + year -1 <= targetAge; year++){
      const ageNow = mainInfo.age + year -1;
      const inPayTerm = (year <= paymentTerm);
      const baseAnnualMain = inPayTerm ? (appState.fees?.baseMain||0) : 0;  // fixed through term
      const extra = inPayTerm ? (appState.mainProduct.extraPremium||0) : 0;
      ...
      for (const p of persons){
        ...
        if (p.supplements && p.supplements.health_scl){
          const base = calculateHealthSclPremium(p, baseAnnualMain, 0, y);
          sYear += base;
          sPeriod += isAnnual ? 0 : Math.round(base*riderFactor/periods/1000)*1000;
        }
        if (p.supplements && p.supplements.bhn){
          const base = calculateBhnPremium(p, baseAnnualMain, 0, y);
          sYear += base;
        ...
        if (mdpEnabled && mdpTargetId && (mdpTargetId===p.id || (mdpTargetId==='other' && p.id==='mdp3_other'))){
          const fee = (y <= 64) ? mdpFeeYear : 0;
          sYear += fee;
          sPeriod += isAnnual ? 0 : Math.round(fee*riderFactor/periods/1000)*1000;
        }
        perPersonSuppPeriod.push(isAnnual?0:sPeriod);
        perPersonSuppYear.push(sYear);
        perPersonSuppYearAF.push(Math.round(sYear*(periods===2?1.02:(periods===4?1.04:1))/1000)*1000);
      }

      const totalSuppPeriod = perPersonSuppPeriod.reduce((a,b)=>a+b,0);
      const totalSuppYear = perPersonSuppYear.reduce((a,b)=>a+b,0);
      const totalQN = baseAnnualMain + extra + perPersonSuppYearAF.reduce((a,b)=>a+b,0);
      const totalSuppPeriod);
      const totalYear = baseAnnualMain + extra + totalSuppYear;
      const diff = totalQN - totalYear;

      rowsData.push({year, ageNow, baseAnnualMain, extra, perPersonSuppYear, perPersonSuppYearAF, totalQN, totalYear, diff});
    }
    ...
    for (const r of rowsData){ sumMain += r.baseAnnualMain; sumExtra += r.extra; for (let i=0;i<persons.length;i++){ sumSuppCols[i] += r.perPersonSuppYearAF[i]; } sumTotalQN += r.totalQN; sumTotalYear += r.totalYear; sumDiff += r.diff; }
    ...
    if (!extraAllZero) html += `<td class="p-2 border text-right">${fmt(sumExtra)}</td>`;
    for (let i=0;i<persons.length;i++){ html += `<td class="p-2 border text-right">${fmt(sumSuppCols[i])}</td>`; }
    html += `<td class="p-2 border text-right"><b>${fmt(sumTotalQN)}</b></td>`; html += `<td class="p-2 border text-right">${fmt(sumTotalYear)}</td>`; html += `<td class=\"p-2 border text-right\"><b>${fmt(sumTotalQN - sumTotalYear)}</b></td>`;
    html += `</tr></tbody></table>`;
  ...
  document.addEventListener('DOMContentLoaded', ()=>{
    ...
    if (btn && !btn.dataset._v2bound){
      btn.addEventListener('click', (e)=>{
        ...
        try{ generateSummaryTableV2(); }catch(err){
          const c = document.getElementById('summary-content-container');
          if (c) c.innerHTML = `<div class="text-red-600">${sanitizeHtml(err && err.message ? err.message : String(err))}</div>`;
          document.getElementById('summary-modal')?.classList.remove('hidden');
        }
...
function initMdp3OtherPerson() {
  const c = document.getElementById('person-container-mdp3-other');
  if (!c || c._mdp3Init) return;
  c._mdp3Init = true;
  const dob = c.querySelector('.dob-input');
  const ageSpan = c.querySelector('.age-span');
  if (dob) {
    initDateFormatter(dob);
    dob.addEventListener('input', () => {
      const { age } = calculateAge(dob.value);
      if (ageSpan) ageSpan.textContent = age || 0;
    });
    dob.addEventListener('blur', () => {
      Validators.validateDobField ? Validators.validateDobField(dob) : validateDobField(dob);
      const { age } = calculateAge(dob.value);
      if (ageSpan) ageSpan.textContent = age || 0;
      runWorkflow();
    });
...
document.addEventListener('DOMContentLoaded', initMdp3OtherPerson);
