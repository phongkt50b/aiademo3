...
function performCalculations(state) {
    ...
    // assign mdp3 to correct person in byPerson map (or 'other' as separate synthetic id)
...
function generateSummaryTable() {
  ...
  try {
    ...
    for (const p of persons){
      ...
      // Header row per person (nhãn tổng)
      ...
      // SP chính + Phí đóng thêm (không nhân hệ số)
      ...
      if (p.supplements && p.supplements.health_scl){
        ...
        const name = `Sức khoẻ Bùng Gia Lực – ${programName} (${scope==='main_global'?'Nước ngoài':'Trong nước'}${outpatient?', Ngoại trú':''}${dental?', Nha khoa':''})`;
    ...
    for (let year=1; mainInfo.age + year -1 <= targetAge; year++){
      ...
      for (let pi=0; pi<persons.length; pi++){
        ...
        if (p.supplements && p.supplements.health_scl){
          ...
          sYear += base;
          sPeriod += Math.round(base * suppFactor / periods /1000)*1000;
        }
        if (p.supplements && p.supplements.bhn){
        ...
        if (mdpEnabled && mdpTargetId === p.id){
          ...
          sYear += fee;
          sPeriod += periods===1 ? fee : Math.round(fee*suppFactor/periods/1000)*1000;
        }
      ...
      const totalPeriod = periods===1 ? 0 : (Math.round(baseAnnualMain/periods/1000)*1000 + Math.round(extra/periods/1000)*1000 + totalSuppPeriod);
...
function renderSuppList(){
  ...
  // Build persons array from DOM (main + supplements)
  const persons = [];
  ...
  // Helper: annual supplementary for a person from fees.byPerson.suppDetails
  const getSuppAnnual = (pid) => {
    try {
      const d = (window.appState && appState.fees && appState.fees.byPerson && appState.fees.byPerson[pid] && appState.fees.byPerson[pid].suppDetails) || {};
      const sum = Object.values(d).reduce((a,b)=> a + (typeof b === 'number' ? b : (b && typeof b.annual === 'number' ? b.annual : 0)), 0);
      if (sum > 0) return sum;
    } catch {}
    // fallback
    const pf = (window.personFees && window.personFees[pid]) || {};
    for (const k of ['suppOnly','supp','supplementary','supp_total','suppTotal']) {
      if (typeof pf[k] === 'number') return pf[k];
    }
    return 0;
  };
  ...
  let rows = persons.map(p=>{
    const supp = getSuppAnnual(p.id) + ((mdpOn && mdpId === p.id) ? mdpFee : 0);
    return `<div class="flex justify-between"><span>${sanitizeHtml(p.name||'Người')}</span><span>${formatDisplayCurrency(supp)}</span></div>`;
  });

  if (mdpOn && mdpId === 'other' && mdpFee > 0) {
