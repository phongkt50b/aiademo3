...
function renderUI() {
    ...
    // Enforce Trọn Tâm An: hide supplementary insured section & clear list
    try {
      const keyEl = document.getElementById('main-product');
      const mainKey = keyEl ? keyEl.value : (appState?.mainProduct?.key || '');
...
function updateSummaryUI(fees){ const _f = fees || {};
  const safe = (n)=> Math.round((Number(n)||0)/1000)*1000;
  const fmt  = (n)=> formatCurrency(safe(n));
  try {
    // baseline by year
    const totalYear = (_f.baseMain||0) + (_f.extra||0) + (_f.totalSupp||0);
    ...
    if (elMain) elMain.textContent = formatCurrency(_f.baseMain||0);
    const elExtra = document.getElementById('extra-premium-result');
    if (elExtra) elExtra.textContent = formatCurrency(_f.extra||0);

    const suppBox = document.getElementById('supplementary-premiums-results');
    if (suppBox) {
      suppBox.innerHTML = (_f.totalSupp||0) ? `<div class="flex justify-between items-center py-2 border-b">
        <span class="text-gray-700">Phí sản phẩm bổ sung (năm)</span>
        <span class="font-bold text-gray-900">${formatCurrency(_f.totalSupp||0)}</span>
      </div>` : '';
    }

    // frequency breakdown (read only)
    ...
    const perMain  = periods===1 ? 0 : safe((_f.baseMain||0) / periods);
    const perExtra = periods===1 ? 0 : safe((_f.extra||0) / periods);
    const perSupp  = periods===1 ? 0 : safe(((_f.totalSupp||0) * factor) / periods);
    ...
    const pairs = [
      ['freq-main', perMain],
      ['freq-extra', perExtra],
      ['freq-supp-total', perSupp],
      ['freq-total-period', totalPeriod],
      ['freq-total-year', totalYear],
      ['freq-diff', diff],
    ];
    pairs.forEach(([k, val])=>{ const el=id(k); if (el) el.textContent = fmt(val); });

    const compat = document.getElementById('total-premium-result');
    if (compat) compat.textContent = formatCurrency(totalYear);
  } catch (e) { /* no-op */ }
}
...
function validateMainPersonInputs(person) {
    ...
    // Validate payment term: applies to products with manual term
    try {
