...
function initSummaryModal() {
  ...
  // [removed duplicate view-summary-btn binding]
...
document.addEventListener('DOMContentLoaded', ()=>{
  const btn = // [removed duplicate view-summary-btn binding]
...
// === Unified once-only binder (stable) ===
(function(){
  const btn = document.getElementById('view-summary-btn');
  if (btn && !btn.dataset._bound_once){
    const handler = (e) => {
      e.preventDefault();
      try {
        const fn = window.generateSummaryTable 
               || window.generateSummaryTableV2 
               || window.generateSummaryTableV3;
        if (typeof fn === 'function') fn();
        else throw new Error('generateSummaryTable is not available.');
      } catch (err) {
        (window.showError || console.error)(err.message || String(err));
      }
    };
    btn.addEventListener('click', handler, {passive:false});
    btn.dataset._bound_once = '1';
  }
  // keep supp list updated live
  (function bindSuppListLive(){
    const tgt = document.getElementById('supplementary-insured-container');
    if (tgt && !tgt._observerAttached) {
      const mo = new MutationObserver(()=>{
        const list = document.getElementById('supp-insured-summaries');
        if (list && !list.classList.contains('hidden') && typeof window.renderSuppList==='function') {
          window.renderSuppList();
        }
      });
      mo.observe(tgt, {childList:true, subtree:true});
      tgt._observerAttached = true;
    ...
    document.getElementById('toggle-supp-list-btn')?.addEventListener('click', ()=>{
      ...
      if (!list) return;
      list.classList.toggle('hidden');
      if (!list.classList.contains('hidden') && typeof window.renderSuppList==='function') window.renderSuppList();
    });
    document.getElementById('add-supp-insured-btn')?.addEventListener('click', ()=> setTimeout(()=>{
      if (typeof window.renderSuppList==='function') window.renderSuppList();
    },0));
    document.getElementById('mdp3-section')?.addEventListener('change', ()=>{
      if (typeof window.renderSuppList==='function') window.renderSuppList();
    });
  })();
})();
