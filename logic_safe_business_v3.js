...
/** Business Enhancer v3 (safe, JS-only)
 * Part 1 (modal): columns & formulas
 * - Baseline annual column labeled **Phí đóng theo năm** (normalize/merge aliases: 'Phí đóng (quy năm)', 'Phí đóng (năm)')
 * - Annualized-by-frequency column labeled **Phí đóng quy năm** (no parentheses) = ×2 (nửa năm) / ×4 (quý) / ×1 (năm)
 * - Difference **Chênh lệch** = (theo năm) - (quy năm), bold+red when freq != năm and value != 0
 * - Avoid duplicates, compute missing values, add/refresh 'Tổng cộng' row
 */
(function BusinessEnhancerSafeV3(){
  ...
  const NAMES = {
    ...
    baseline: ['phí đóng theo năm','phí đóng (quy năm)','phí đóng (năm)'],
    annualized: ['phí đóng quy năm'],
    diff: ['chênh lệch']
  };
  ...
  function getFreq(){
    ...
    if (r) {
      const v = (r.value || r.getAttribute('value') || r.textContent || '').toLowerCase();
  ...
  function headerIndexMap(table){
    ...
    cells.forEach((c,i)=>{ map[norm(c.textContent)] = i; });
    return map;
  }
  ...
  function ensureHeader(table, label, insertAfterIdx){
    const thead = table.tHead || table.querySelector('thead');
    ...
    let insertPos = (typeof insertAfterIdx === 'number' && insertAfterIdx >= 0 && insertAfterIdx < row.cells.length)
      ? insertAfterIdx + 1
      : row.cells.length;
  ...
  function deleteColumn(table, idx){
    const thead = table.tHead || table.querySelector('thead');
    if (thead && thead.rows[0] && idx >= 0 && idx < thead.rows[0].cells.length) {
      for (const row of thead.rows) row.deleteCell(idx);
    }
    for (const tb of table.tBodies) {
      for (const row of tb.rows) {
        if (idx >= 0 && idx < row.cells.length) row.deleteCell(idx);
      }
    }
  }
  ...
  function enhancePart1Tables(root){
    const freq = getFreq();
    ...
    $$$('table', root).forEach(table=>{
      ...
      // Merge duplicates: if both 'Phí đóng (quy năm)' and 'Phí đóng theo năm' exist, keep 'theo năm'
      const headers = Array.from(thead0.rows[0].cells).map(c=>norm(c.textContent));
      const idxParenAnnual = headers.indexOf('phí đóng (quy năm)');
      const idxTheoNam = headers.indexOf('phí đóng theo năm');
      ...
      // Ensure baseline exists, and normalize label
      let afterIdx = Math.max(idxHalf, idxQuarter);
      if (idxBaseline < 0) {
        idxBaseline = ensureHeader(table, 'Phí đóng theo năm', afterIdx);
      } else {
        renameHeader(table, idxBaseline, 'Phí đóng theo năm');
      }

      // Ensure annualized & diff columns
      ...
      Array.from(tbody.rows).forEach(r=>{
        ...
        // If baseline is empty/0, backfill from per-period columns
        if (!vBaseline) {
          if (vHalf != null && vHalf > 0) vBaseline = vHalf * 2;
          else if (vQuarter != null && vQuarter > 0) vBaseline = vQuarter * 4;
        }

        // Annualized-by-frequency value
        let vEq = vBaseline;
        if (freq === 'half') {
          vEq = (vHalf != null && vHalf > 0) ? vHalf * 2 : vBaseline;
        } else if (freq === 'quarter') {
          vEq = (vQuarter != null && vQuarter > 0) ? vQuarter * 4 : vBaseline;
        } // 'year' -> vEq = vBaseline
      ...
      const hasTotal = lastRow && /tổng/i.test((lastRow.cells[0]?.textContent || ''));
      const row = hasTotal ? lastRow : document.createElement('tr');
      if (!hasTotal) {
  ...
  function run(){ enhancePart1Tables(document); }

  // Idempotent binding
  function once(el, ev, fn){
    if (!el) return;
    const key = '__enhV3_' + ev;
    if (el[key]) return;
    el[key] = true;
    el.addEventListener(ev, ()=>setTimeout(fn,0));
  }
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', ()=>{ bind(); run(); });
  } else {
    bind(); run();
  }
