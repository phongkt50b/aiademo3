...
/** Business Enhancer v2 (safe)
 * Part 1 (modal): enforce columns && formulas
 * - Baseline annual: header normalized to **Phí đóng theo năm** (treat 'Phí đóng (quy năm)'/'Phí đóng (năm)' as same)
 * - Annualized-by-frequency: **Phí đóng quy năm**
 * - Difference: **Chênh lệch** = (theo năm) - (quy năm)
 * - Avoid duplicates, compute missing values, add 'Tổng cộng', highlight diff when freq != năm
 */
(function BusinessEnhancerSafeV2(){
  ...
  function headerIndexMap(table){
    ...
    Array.from(thead.rows[0].cells).forEach((c,i)=>{
      map[norm(c.textContent)] = i;
    });
    return map;
  }
  ...
  function ensureHeader(table, label, insertAfterIdx=null){
    ...
    if (insertAfterIdx == null || insertAfterIdx >= row.cells.length - 1) {
      row.appendChild(th);
      insertAfterIdx = row.cells.length - 1;
    } else {
      row.insertBefore(th, row.cells[insertAfterIdx + 1]);
      insertAfterIdx = insertAfterIdx + 1;
    }
    ...
    if (tbody) {
      Array.from(tbody.rows).forEach(r=>{
        ...
        if (insertAfterIdx >= r.cells.length) r.appendChild(td);
        else r.insertBefore(td, r.cells[insertAfterIdx]);
      });
    }
  ...
  function renameHeader(table, idx, newText){
    const thead = table.tHead || table.querySelector('thead');
    if (!thead || !thead.rows[0] || !thead.rows[0].cells[idx]) return;
  ...
  function deleteColumn(table, idx){
    ...
    if (thead) {
      for (const row of thead.rows) {
        if (idx < row.cells.length) row.deleteCell(idx);
      }
    }
  ...
  function enhancePart1Tables(root){
    ...
    $$$('table').forEach(table=>{
      ...
      // If both "(quy năm)" && "theo năm" exist, drop "(quy năm)" && keep "theo năm"
      ...
      // Ensure baseline exists && is labeled "Phí đóng theo năm"
      ...
      // Ensure annualized column exists after baseline
      ...
      Array.from(tbody.rows).forEach(r=>{
        ...
        if (!vBaseline):
          ...
          elif (vQuarter != null && vQuarter > 0): vBaseline = vQuarter * 4

        let vEq = 0
        if (freq=='year'): vEq = vBaseline
        elif (freq=='half'): vEq = (vHalf!=null && vHalf>0) && (vHalf*2) || vBaseline
        elif (freq=='quarter'): vEq = (vQuarter!=null && vQuarter>0) && (vQuarter*4) || vBaseline
