...
function attachGlobalListeners() {
  ...
  document.body.addEventListener('input', (e) => {
    // For general text inputs (excluding DOB, occupation, name), format numbers and recalc
    if (e.target.matches('input[type="text"]') && !e.target.classList.contains('dob-input') &&
      ...
      // Numeric inputs trigger recalculation directly
      calculateAll();
    } else if (e.target.classList.contains('dob-input')) {
      // Ensure age is recalculated when date of birth changes
      // We do not format date fields here; initDateFormatter handles formatting
      calculateAll();
    }
...
function initSummaryModal() {
  ...
  // Bind the click handler to call the current generateSummaryTable on window when invoked.
  ...
  document.getElementById('main-product').addEventListener('change', () => {
    ...
    if (document.getElementById('summary-modal').classList.contains('hidden')) {
      ...
      // Invoke the latest generateSummaryTable implementation from window
      if (typeof window.generateSummaryTable === 'function') {
        window.generateSummaryTable();
      }
    }
...
(() => {
  ...
  (function() {
    ...
    // Assign our implementation to both the window property and the module-scoped variable.
    // This ensures event listeners referencing the generateSummaryTable variable execute
    // the latest version of the function defined here.
    window.generateSummaryTable = customGenerateSummary;
    try {
      // Assign the module-scoped variable if it exists; in ES modules this may
      // throw if generateSummaryTable is undefined, so wrap in try/catch.
      generateSummaryTable = customGenerateSummary;
    } catch (e) {
      // In some scopes generateSummaryTable may not exist as a variable. Ignore errors.
    }
    // Store a reference for any later overrides. Some subsequent patches
    // check window.__generateSummaryTableOld to decide whether to delegate
    // to a previous implementation. Without setting this, the later override
    // would run its own code instead of deferring to this comprehensive version.
...
(function(){
  // This IIFE originally contained a second implementation of generateSummaryTable
  // that largely duplicates the comprehensive summary generator defined earlier.
  // To avoid duplicate logic and potential conflicts, we disable this override
  // by immediately returning here. The existing summary generator will handle
