...
function renderSuppList(){
  ...
  const mainInfo = (typeof collectPersonData==='function' ? collectPersonData(mainCont, true) : null);
  if (mainInfo) persons.push(mainInfo);
  $qsa('#supplementary-insured-container .person-container').forEach(c => {
    const p = (typeof collectPersonData==='function' ? collectPersonData(c,false) : null);
    if (p) persons.push(p);
  });
  ...
  const rows = persons.map(p => {
    ...
    return `<div class="flex justify-between"><span>${sanitize(p.name||'Người')}</span><span>${fmt(sum)}</span></div>`;
  });
