...
function validateMainPersonInputs(person) {
    ...
    // Validate payment term for applicable products
    try {
      const key = productInfo.key;
      if (['KHOE_BINH_AN', 'VUNG_TUONG_LAI', 'PUL_TRON_DOI', 'PUL_15_NAM', 'PUL_5_NAM'].includes(key)) {
        const el = document.getElementById('payment-term');
        if (el) {
          const v = parseInt(el.value, 10);
          const bounds = getPaymentTermBounds(customer.age);
          let min = 4;
          if (key === 'PUL_5_NAM') min = 5;
          if (key === 'PUL_15_NAM') min = 15;
          if (!v || v < min || v > bounds.max) {
            setFieldError(el, `Nhập từ ${min} đến ${bounds.max} năm`);
            ok = false;
          } else {
            clearFieldError(el);
          }
...
function generateSummaryTable() {
  ...
  try {
    ...
    if (!targetAge || targetAge <= mainInfo.age) throw new Error('Vui lòng nhập độ tuổi mục tiêu hợp lệ.');

    const mainProductKey = document.getElementById('main-product')?.value || appState.mainProduct.key || '';
    let paymentTerm = appState.mainProduct.paymentTerm || 999;
    if (mainProductKey === 'TRON_TAM_AN') paymentTerm = 10;
    if (mainProductKey === 'AN_BINH_UU_VIET') paymentTerm = parseInt(document.getElementById('abuv-term')?.value || '10', 10);

    // Collect persons: main + others
    const others = Array.from(document.querySelectorAll('#supplementary-insured-container .person-container')).map(c => getCustomerInfo(c,false));
    const persons = [mainInfo, ...others];

    // Helpers to check selections in a person container
    const hasChecked = (container, sel) => {
      const sec = container?.querySelector(sel);
      if (!sec) return false;
    ...
    function addRow(personName, prodName, stbhDisplay, years, perPeriod, annualEq, baseAnnual){
      let diff = annualEq - baseAnnual;
      if (periods===1){ perPeriod=0; annualEq=0; diff=0; }
      if (periods!==1) { totalPer += perPeriod; totalAnnEq += annualEq; totalDiff += diff; }
      totalBase += baseAnnual;
      html += `<tr>
      ...
      if (periods!==1){ const diffHtml = diff ? `<span class="text-red-600 font-bold">${fmt(diff)}</span>` : '0'; html += `<td class="p-2 border text-right">${diffHtml}</td>`; }
      html += `</tr>`;
    }

    for (const p of persons){
      const cont = p.isMain ? document.getElementById('main-person-container') : p.container;
      // group header row (Tổng) – we'll just print an empty row for layout consistency
      html += `<tr><td class="p-2 border font-semibold">${sanitizeHtml(p.name|| (p.isMain?'NĐBH Chính':'NĐBH Bổ sung'))}</td><td class="p-2 border font-semibold">Tổng</td><td class="p-2 border">—</td><td class="p-2 border">—</td>`;
      ...
      if (p.isMain && appState.mainProduct.key && appState.mainProduct.key!=='TRON_TAM_AN'){
        ...
        addRow(p.name,'SP chính', formatDisplayCurrency(appState.mainProduct.stbh||0), appState.mainProduct.paymentTerm||'—', perPeriod, annualEq, baseAnnual);
      }
      if (p.isMain && (appState.mainProduct.extraPremium||0)>0){
        const baseAnnual = appState.mainProduct.extraPremium||0;
        const perPeriod = periods===1 ? 0 : Math.round(baseAnnual/periods/1000)*1000;
        const annualEq  = periods===1 ? 0 : perPeriod*periods;
        addRow(p.name,'Phí đóng thêm','—', appState.mainProduct.paymentTerm||'—', perPeriod, annualEq, baseAnnual);
      }

      // Riders
      const sclSec = cont?.querySelector('.health-scl-section');
      if (sclSec && sclSec.querySelector('.health-scl-checkbox')?.checked){
        const program = sclSec.querySelector('.health-scl-program')?.value || '';
        const scope = sclSec.querySelector('.health-scl-scope')?.value || 'domestic';
        const outpatient = !!sclSec.querySelector('.health-scl-outpatient')?.checked;
        const dental = !!sclSec.querySelector('.health-scl-dental')?.checked;
        const name = `Sức khoẻ Bùng Gia Lực – ${({co_ban:'Cơ bản',nang_cao:'Nâng cao',toan_dien:'Toàn diện',hoan_hao:'Hoàn hảo'}[program]||'')}
          (${scope==='abroad'?'Nước ngoài':'Trong nước'}${outpatient?', Ngoại trú':''}${dental?', Nha khoa':''})`.replace(/\\s+/g,' ').trim();
        const stbh = (CONFIG?.supplementaryProducts?.[0]?.stbhByProgram?.[program]) || 0;
        const baseAnnual = calculateHealthSclPremium(p, appState.fees.baseMain, cont);
        const perPeriod = periods===1 ? 0 : Math.round(baseAnnual*suppFactor/periods/1000)*1000;
        const annualEq  = periods===1 ? 0 : perPeriod*periods;
        addRow(p.name, name, formatDisplayCurrency(stbh), appState.mainProduct.paymentTerm||'—', perPeriod, annualEq, baseAnnual);
      }
      const bhnSec = cont?.querySelector('.bhn-section');
      if (bhnSec && bhnSec.querySelector('.bhn-checkbox')?.checked){
        const stbh = num(bhnSec.querySelector('.bhn-stbh')?.value);
        const baseAnnual = calculateBhnPremium(p, appState.fees.baseMain, cont);
        const perPeriod = periods===1 ? 0 : Math.round(baseAnnual*suppFactor/periods/1000)*1000;
        const annualEq  = periods===1 ? 0 : perPeriod*periods;
        addRow(p.name,'Bệnh Hiểm Nghèo 2.0', formatDisplayCurrency(stbh), appState.mainProduct.paymentTerm||'—', perPeriod, annualEq, baseAnnual);
      }
      const accSec = cont?.querySelector('.accident-section');
      if (accSec && accSec.querySelector('.accident-checkbox')?.checked){
        const stbh = num(accSec.querySelector('.accident-stbh')?.value);
        const baseAnnual = calculateAccidentPremium(p, appState.fees.baseMain, cont);
        const perPeriod = periods===1 ? 0 : Math.round(baseAnnual*suppFactor/periods/1000)*1000;
        const annualEq  = periods===1 ? 0 : perPeriod*periods;
        addRow(p.name,'Bảo hiểm Tai nạn', formatDisplayCurrency(stbh), appState.mainProduct.paymentTerm||'—', perPeriod, annualEq, baseAnnual);
      }
      const hsSec = cont?.querySelector('.hospital-support-section');
      if (hsSec && hsSec.querySelector('.hospital-support-checkbox')?.checked){
        const stbh = num(hsSec.querySelector('.hospital-support-stbh')?.value);
        const baseAnnual = calculateHospitalSupportPremium(p, appState.fees.baseMain, cont);
        const perPeriod = periods===1 ? 0 : Math.round(baseAnnual*suppFactor/periods/1000)*1000;
        const annualEq  = periods===1 ? 0 : perPeriod*periods;
        addRow(p.name,'Hỗ trợ chi phí nằm viện', formatDisplayCurrency(stbh), appState.mainProduct.paymentTerm||'—', perPeriod, annualEq, baseAnnual);
      }
    }
    ...
    if (periods!==1){ const diffHtml = totalDiff ? `<span class="text-red-600 font-bold">${fmt(totalDiff)}</span>` : '0'; html += `<td class="p-2 border text-right">${diffHtml}</td>`; }
    html += `</tr></tbody></table>`;
    ...
    for (let year=1; mainInfo.age + year -1 <= targetAge; year++){
      ...
      const baseAnnualMain = inPayTerm && appState.mainProduct.key!=='TRON_TAM_AN' ? calculateMainPremium(mainInfo, appState.mainProduct, ageNow) : 0;
      ...
      for (let pi=0; pi<persons.length; pi++){
        ...
        const secS = cont.querySelector('.health-scl-section');
        if (secS && secS.querySelector('.health-scl-checkbox')?.checked){
          const base = calculateHealthSclPremium(p, baseAnnualMain, cont, y);
          sYear += base;
          sPeriod += Math.round(base * suppFactor /1000)*1000;
        }
        const secB = cont.querySelector('.bhn-section');
        if (secB && secB.querySelector('.bhn-checkbox')?.checked){
          const base = calculateBhnPremium(p, baseAnnualMain, cont, y);
          sYear += base;
          sPeriod += Math.round(base * suppFactor /1000)*1000;
        }
        const secA = cont.querySelector('.accident-section');
        if (secA && secA.querySelector('.accident-checkbox')?.checked){
          const base = calculateAccidentPremium(p, baseAnnualMain, cont, y);
          sYear += base;
          sPeriod += Math.round(base * suppFactor /1000)*1000;
        }
        const secH = cont.querySelector('.hospital-support-section');
        if (secH && secH.querySelector('.hospital-support-checkbox')?.checked){
          const base = calculateHospitalSupportPremium(p, baseAnnualMain, cont, y);
          sYear += base;
          sPeriod += Math.round(base * suppFactor /1000)*1000;
        }
      ...
      const diff = periods===1 ? 0 : ( (totalPeriod*periods) - totalYear );

      sumMain += baseAnnualMain;
      sumExtra += extra;
      for (let i=0;i<persons.length;i++){ sumSuppCols[i] += (periods===1 ? perPersonSuppYear[i] : perPersonSuppPeriod[i]); }
      ...
      if (periods!==1){ const diffHtml2 = diff ? `<span class="text-red-600 font-bold">${fmt(diff)}</span>` : '0'; html += `<td class="p-2 border text-right">${diffHtml2}</td>`; }
      html += `</tr>`;
    }
