...
const scopeLabel = {domestic:'Trong nước', international:'Nước ngoài'}[scopeVal] || '';
const outpatient = sclSec.querySelector('.health-scl-outpatient')?.checked || false;
const dental = sclSec.querySelector('.health-scl-dental')?.checked || false;
const optLabels = [];
if (scopeLabel) optLabels.push(scopeLabel);
if (outpatient) optLabels.push('Ngoại trú');
if (dental) optLabels.push('Nha khoa');
const extraCfg = optLabels.length ? ` (${optLabels.join(', ')})` : '';
const productName = `Sức khoẻ Bùng Gia Lực${programLabel ? ' - ' + programLabel : ''}${extraCfg}`;
    ...
    summaryHtml += `<th class="p-2 border">Phí đóng (quy năm)</th><th class="p-2 border">Phí đóng theo năm</th>`;
    if (showDiffColumn) summaryHtml += `<th class="p-2 border">Chênh lệch</th>`;
    summaryHtml += `</tr></thead><tbody>`;

    // Dòng tổng cho từng người
    let sumPerPeriodAll=0,sumAnnualEqAll=0,sumBaseAll=0,sumDiffAll=0;
    personsList.forEach(person => {
      ...
      person.products.forEach(pr => {
        ...
        summaryHtml += `<td class=\"p-2 border text-right\">\${formatCurrency(pr.perPeriod)}</td>`; sumPerPeriodAll += pr.perPeriod;
        ...
        summaryHtml += `<td class=\"p-2 border text-right\">\${formatCurrency(pr.baseAnnual)}</td>`; sumBaseAll += pr.baseAnnual;
        if (showDiffColumn) {
          summaryHtml += `<td class=\"p-2 border text-right\">\${pr.diff ? formatCurrency(pr.diff) : ''}</td>`; if (pr.diff) sumDiffAll += pr.diff;
        }
        summaryHtml += `</tr>`;
      });
    });
    summaryHtml += `</tbody><tfoot><tr><td class="p-2 border font-semibold">Tổng cộng</td><td class="p-2 border text-right">—</td><td class="p-2 border text-center">—</td><td class="p-2 border text-right font-semibold">${formatCurrency(sumPerPeriodAll)}</td><td class="p-2 border text-right font-semibold">${formatCurrency(sumAnnualEqAll)}</td><td class="p-2 border text-right font-semibold">${formatCurrency(sumBaseAll)}</td>${showDiffColumn ? `<td class=\"p-2 border text-right font-semibold\">${formatCurrency(sumDiffAll)}</td>` : ''}</tr></tfoot></table></div>`;
    ...
    // Tổng cộng Part 2 accumulators
    let sumMainBase=0, sumExtra=0, sumMainSupp=0, sumSuppCols=[]; let sumTotalAnnualEq=0, sumBaselineTotal=0, sumTotalDiff=0;
// Hạn mức chung Hỗ trợ viện phí (theo phí sản phẩm chính)
    ...
    for (let i = 0; (mainPersonInfo.age + i) <= targetAge; i++) {
      ...
      if (periods === 1) {
        ...
        diffMainSupp = baseMainSuppAnnual - annualEqMainSupp;
      }
      ...
      const baselineTotal = annualEqMainBase + annualEqMainExtra + baseMainSuppAnnual + suppAnnuals.reduce((a,b)=>a+b,0);
      ...
      tableHtml += `<td class="p-2 border text-right">${annualEqMainBase ? formatCurrency(annualEqMainBase) : ""}</td>`; sumMainBase += annualEqMainBase;
      // Cột phí đóng thêm cho NĐBH chính (ẩn nếu không cần hiển thị)
      if (hasExtraMain) {
        tableHtml += `<td class="p-2 border text-right">${annualEqMainExtra ? formatCurrency(annualEqMainExtra) : ""}</td>`; sumExtra += annualEqMainExtra;
      }
      // Cột phí bổ sung cho NĐBH chính (ẩn nếu không cần hiển thị)
      if (hasSuppMain) {
        tableHtml += `<td class="p-2 border text-right">${annualEqMainSupp ? formatCurrency(annualEqMainSupp) : ""}</td>`; sumMainSupp += annualEqMainSupp;
      }
      // Cột phí cho từng NĐBH bổ sung (bao gồm MDP3 nếu có)
      annualEqSupps.forEach((val,idx) => {
        tableHtml += `<td class=\"p-2 border text-right\">${val ? formatCurrency(val) : ""}</td>`; sumSuppCols[idx] += (val||0);
      });
    ...
    tableHtml += `</tbody><tfoot><tr><td class="p-2 border font-semibold">Tổng cộng</td><td class="p-2 border"></td><td class="p-2 border text-right font-semibold">${formatCurrency(sumMainBase)}</td>` + (hasExtraMain ? `<td class=\"p-2 border text-right font-semibold\">${formatCurrency(sumExtra)}</td>` : '') + (hasSuppMain ? `<td class=\"p-2 border text-right font-semibold\">${formatCurrency(sumMainSupp)}</td>` : '') + tablePersons.map((_,idx)=>`<td class=\"p-2 border text-right font-semibold\">${formatCurrency(sumSuppCols[idx]||0)}</td>`).join('') + ((periods!==1)? `<td class=\"p-2 border text-right font-semibold\">${formatCurrency(sumTotalAnnualEq)}</td><td class=\"p-2 border text-right font-semibold\">${formatCurrency(sumBaselineTotal)}</td><td class=\"p-2 border text-right font-semibold\">${formatCurrency(sumTotalDiff)}</td>` : `<td class=\"p-2 border text-right font-semibold\">${formatCurrency(sumBaselineTotal)}</td>`) + `</tr></tfoot></table></div>`;
  ...
  (function() {
    function customGenerateSummary() {
      ...
      try {
        ...
        if (container) container.innerHTML = summaryHtml + tableHtml + `<div class="mt-4 text-center"><button id="export-html-btn" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 mr-2">Xuất HTML</button><button id="export-pdf-btn" class="bg-stone-600 text-white px-4 py-2 rounded hover:bg-stone-700">Xuất PDF</button></div>`;
