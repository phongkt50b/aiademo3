...
(function(){
  ...
  function customGenerateSummary() {
    ...
    try {
      ...
      // Build Part 2 - Bảng phí với cột tách riêng cho mỗi loại phí và MDP3
      html += `<div class="mb-4"><div class="text-lg font-semibold mb-2">Phần 2 · Bảng phí (${freq === 'year' ? 'năm' : (freq === 'half' ? 'nửa năm' : 'theo quý')})</div>`;
      // Chuẩn bị dữ liệu cho từng năm và xác định cột cần hiển thị
      const rowsData = [];
      let showExtra = false;
      let showSuppMain = false;
      const showSuppPerson = new Array(suppPersons.length).fill(false);
      let showMdp3Other = false;
      // Xử lý Miễn Đóng Phí 3.0
      let mdp3SelId = null;
      let mdp3Annual = 0;
      let mdp3OtherName = '';
      let mdp3Years = 0;
      try {
        if (window.MDP3) {
          mdp3SelId = window.MDP3.getSelectedId && window.MDP3.getSelectedId();
          mdp3Annual = Number(window.MDP3.getPremium ? (window.MDP3.getPremium() || 0) : 0);
        }
      } catch (e) {}
      if (mdp3Annual > 0) {
        // Determine age start and name for MDP3
        if (mdp3SelId === 'other') {
          const otherForm = document.getElementById('person-container-mdp3-other');
          const otherInfo = getCustomerInfo(otherForm, false);
          mdp3OtherName = otherInfo.name?.trim() || 'Người khác';
          const startAge = otherInfo.age || 0;
          mdp3Years = Math.max(0, Math.min(targetAge, 65) - startAge + 1);
        } else if (mdp3SelId === 'main-person-container') {
          mdp3Years = Math.max(0, Math.min(targetAge, 65) - mainInfo.age + 1);
        } else {
          const idx = suppPersons.findIndex(p => p.container?.id === mdp3SelId);
          const startAge = (idx >= 0 ? suppPersons[idx].age : mainInfo.age);
          mdp3Years = Math.max(0, Math.min(targetAge, 65) - startAge + 1);
        }
      }
      // Duyệt từng năm để tính phí
      for (let i = 0; (mainInfo.age + i) <= targetAge; i++) {
        const ageThisYear = mainInfo.age + i;
        const contractYear = i + 1;
        const mainAnnual = (contractYear <= paymentTerm) ? calculateMainPremium(mainInfo) : 0;
        ...
        // Phí bổ sung của từng người
        const suppVals = [];
        suppPersons.forEach((pInfo, idx) => {
          const pAge = pInfo.age + i;
          const pCont = pInfo.container.querySelector('.supplementary-products-container');
          let sVal = 0;
          if (pCont) {
            sVal += calculateHealthSclPremium({ ...pInfo, age: pAge }, pCont, pAge);
            sVal += calculateBhnPremium({ ...pInfo, age: pAge }, pCont, pAge);
            sVal += calculateAccidentPremium({ ...pInfo, age: pAge }, pCont, pAge);
            sVal += calculateHospitalSupportPremium({ ...pInfo, age: pAge }, calculateMainPremium(mainInfo), pCont, 0, pAge);
          }
          suppVals.push(sVal);
        });
        // Tính phí MDP3 cho năm này nếu có
        let mdp3OtherAnnual = 0;
        if (mdp3Annual > 0 && contractYear <= mdp3Years) {
          if (mdp3SelId === 'main-person-container') {
            suppMainAnnual += mdp3Annual;
          } else if (mdp3SelId === 'other') {
            mdp3OtherAnnual = mdp3Annual;
          } else {
            const idx2 = suppPersons.findIndex(p => p.container?.id === mdp3SelId);
            if (idx2 >= 0) {
              suppVals[idx2] += mdp3Annual;
            }
          }
        }
        // Bỏ qua năm không có bất kỳ phí nào
        const totalAnnualThisRow = mainAnnual + extraAnnual + suppMainAnnual + suppVals.reduce((a,b)=>a+b,0) + mdp3OtherAnnual;
        if (totalAnnualThisRow === 0) continue;
        // Đánh dấu cột cần hiển thị
        if (extraAnnual > 0) showExtra = true;
        if (suppMainAnnual > 0) showSuppMain = true;
        suppVals.forEach((v, idx3) => { if (v > 0) showSuppPerson[idx3] = true; });
        if (mdp3OtherAnnual > 0) showMdp3Other = true;
        rowsData.push({
          year: contractYear,
          age: ageThisYear,
          main: mainAnnual,
          extra: extraAnnual,
          suppMain: suppMainAnnual,
          suppVals: suppVals,
          mdp3Other: mdp3OtherAnnual
        });
      }
      // Khởi tạo bảng
      html += `<table class="w-full text-left border-collapse"><thead class="bg-gray-100"><tr>`;
      ...
      // Cột phí chính (sản phẩm chính)
      html += `<th class="p-2 border">Phí sản phẩm chính</th>`;
      // Cột phí đóng thêm (ẩn nếu không cần)
      if (showExtra) {
        html += `<th class="p-2 border">Phí đóng thêm</th>`;
      }
      // Cột phí bổ sung của NĐBH chính (ẩn nếu không cần)
      if (showSuppMain) {
        html += `<th class="p-2 border">Phí sản phẩm bổ sung<br>(${sanitizeHtml(mainInfo.name)})</th>`;
      }
      // Cột của từng NĐBH bổ sung
      suppPersons.forEach((p, idx4) => {
        if (showSuppPerson[idx4]) {
          html += `<th class="p-2 border">Phí sản phẩm bổ sung<br>(${sanitizeHtml(p.name)})</th>`;
        }
      });
      // Cột dành cho MDP3 nếu là người khác
      if (showMdp3Other) {
        html += `<th class="p-2 border">Phí miễn đóng phí<br>(${sanitizeHtml(mdp3OtherName)})</th>`;
      }
      // Tổng cộng và chênh lệch
      ...
      html += `</tr></thead><tbody>`;
      // Tính hệ số kỳ đóng phí
      const periods = freq === 'half' ? 2 : (freq === 'quarter' ? 4 : 1);
      const factor = freq === 'half' ? 1.02 : (freq === 'quarter' ? 1.04 : 1.0);
      // Duyệt từng dòng dữ liệu và xuất ra bảng
      rowsData.forEach(r => {
        // Tính theo kỳ đóng phí
        let perMain = r.main;
        let perExtra = r.extra;
        let perSuppMain = r.suppMain;
        const perSuppEach = [];
        let perMdp3Other = r.mdp3Other;
        if (periods > 1) {
          perMain = r.main > 0 ? Math.floor((r.main / periods) / 1000) * 1000 : 0;
          perExtra = r.extra > 0 ? Math.floor((r.extra / periods) / 1000) * 1000 : 0;
          perSuppMain = r.suppMain > 0 ? Math.round((r.suppMain / 1000 * factor / periods)) * 1000 : 0;
          perMdp3Other = r.mdp3Other > 0 ? Math.round((r.mdp3Other / 1000 * factor / periods)) * 1000 : 0;
          r.suppVals.forEach(val => {
            const v = val > 0 ? Math.round((val / 1000 * factor / periods)) * 1000 : 0;
            perSuppEach.push(v);
          });
        } else {
          // year: giữ nguyên
          perSuppEach.push(...r.suppVals);
        }
        // Tính tổng cộng và chênh lệch
        let rowSum = perMain + (showExtra ? perExtra : 0) + (showSuppMain ? perSuppMain : 0);
        // Add each supplementary person
        perSuppEach.forEach((val, idx5) => {
          if (showSuppPerson[idx5]) {
            rowSum += val;
          }
        });
        if (showMdp3Other) {
        ...
        // Compute difference
        const annualTotal = r.main + (showExtra ? r.extra : 0) + (showSuppMain ? r.suppMain : 0) + r.suppVals.reduce((a,b,idx6)=> {
          if (showSuppPerson[idx6]) return a + b;
          return a;
        }, 0) + (showMdp3Other ? r.mdp3Other : 0);
        const toYearFromPer = rowSum * periods;
        const diff = toYearFromPer - annualTotal;
        // Xuất dòng
        html += `<tr>`;
        html += `<td class="p-2 border text-center">${r.year}</td>`;
        html += `<td class="p-2 border text-center">${r.age}</td>`;
        html += `<td class="p-2 border text-right">${formatCurrency(perMain)}</td>`;
        if (showExtra) {
          html += `<td class="p-2 border text-right">${formatCurrency(perExtra)}</td>`;
        }
        if (showSuppMain) {
          html += `<td class="p-2 border text-right">${formatCurrency(perSuppMain)}</td>`;
        }
        // Supplementary persons
        perSuppEach.forEach((val, idx7) => {
          if (showSuppPerson[idx7]) {
            html += `<td class="p-2 border text-right">${formatCurrency(val)}</td>`;
          }
        });
        // MDP3 other column
        if (showMdp3Other) {
          html += `<td class="p-2 border text-right">${formatCurrency(perMdp3Other)}</td>`;
        }
        // Tổng cộng
        html += `<td class="p-2 border text-right">${formatCurrency(rowSum)}</td>`;
        if (freq !== 'year') {
