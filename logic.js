...
// Ngày tham chiếu tính tuổi - sử dụng ngày hiện tại thay vì cố định 09/08/2025
// Việc sử dụng new Date() sẽ tự động lấy ngày hiện tại theo múi giờ của trình duyệt.
...
(function(){
  ...
  function customGenerateSummary() {
    ...
    try {
      ...
      const extraAnnual = Number(getExtraPremiumValue ? (getExtraPremiumValue() || 0) : 0);
      let hasExtraColumn = extraAnnual > 0;
      let hasSuppMainColumn = false;
      const mainSuppCont = document.querySelector('#main-supp-container .supplementary-products-container');
      if (mainSuppCont) {
        if (
          (mainSuppCont.querySelector('.health-scl-checkbox')?.checked) ||
          (mainSuppCont.querySelector('.bhn-checkbox')?.checked) ||
          (mainSuppCont.querySelector('.accident-checkbox')?.checked) ||
          (mainSuppCont.querySelector('.hospital-support-checkbox')?.checked)
        ) {
          hasSuppMainColumn = true;
        }
      }
      // Determine MDP3 assignment
      let mdp3SelectedId = null;
      let mdp3Premium = 0;
      try {
        if (window.MDP3) {
      ...
      // If MDP3 is assigned to main person, ensure supplementary column shown
      if (mdp3Premium > 0 && mdp3SelectedId === 'main-person-container') {
        hasSuppMainColumn = true;
      }
      // Determine if MDP3 is assigned to "other" person
      let hasMdp3OtherColumn = false;
      let mdp3OtherName = '';
      if (mdp3Premium > 0 && mdp3SelectedId === 'other') {
        hasMdp3OtherColumn = true;
        const otherForm = document.getElementById('person-container-mdp3-other');
        const nameInput = otherForm?.querySelector('.name-input');
      ...
      if (hasMdp3OtherColumn) {
        html += `<th class="p-2 border">Phí miễn đóng phí 3.0<br>(${sanitizeHtml(mdp3OtherName)})</th>`;
      }
      ...
      for (let i = 0; (mainInfo.age + i) <= targetAge; i++) {
        ...
        const mainSuppContainer = document.querySelector('#main-supp-container .supplementary-products-container');
        if (mainSuppContainer) {
          suppMain += calculateHealthSclPremium({ ...mainInfo, age: ageThisYear }, mainSuppContainer, ageThisYear);
          suppMain += calculateBhnPremium({ ...mainInfo, age: ageThisYear }, mainSuppContainer, ageThisYear);
          suppMain += calculateAccidentPremium({ ...mainInfo, age: ageThisYear }, mainSuppContainer, ageThisYear);
          suppMain += calculateHospitalSupportPremium({ ...mainInfo, age: ageThisYear }, calculateMainPremium(mainInfo), mainSuppContainer, 0, ageThisYear);
        }
        ...
        suppPersons.forEach(pInfo => {
          ...
          let sum = 0;
          if (cont) {
            sum += calculateHealthSclPremium({ ...pInfo, age: pAge }, cont, pAge);
            sum += calculateBhnPremium({ ...pInfo, age: pAge }, cont, pAge);
            sum += calculateAccidentPremium({ ...pInfo, age: pAge }, cont, pAge);
            sum += calculateHospitalSupportPremium({ ...pInfo, age: pAge }, calculateMainPremium(mainInfo), cont, 0, pAge);
          }
          suppEachArr.push(sum);
        });
        // MDP3 fee per assignment
        let mdp3OtherAnnual = 0;
        // apply mdp3Premium to appropriate person for this year
        if (mdp3Premium > 0) {
          if (mdp3SelectedId === 'main-person-container') {
            suppMain += mdp3Premium;
          } else if (mdp3SelectedId === 'other') {
            mdp3OtherAnnual = mdp3Premium;
          } else {
            const index = suppPersons.findIndex(p => p.container?.id === mdp3SelectedId);
            if (index >= 0) {
              suppEachArr[index] += mdp3Premium;
            }
          }
        }
        // Calculate total annual actual premium for this row
        const annualActual = mainThisYear + extraThisYear + suppMain + suppEachArr.reduce((a,b) => a + b, 0) + mdp3OtherAnnual;
        if (annualActual === 0) continue;
        // Compute per-period amounts
        const perMain = (periods === 1) ? mainThisYear : Math.floor(mainThisYear / periods / 1000) * 1000;
        const perExtra = (periods === 1) ? extraThisYear : Math.floor(extraThisYear / periods / 1000) * 1000;
        const perSuppMain = (periods === 1) ? suppMain : Math.round((suppMain / 1000 * factor / periods)) * 1000;
        const perSuppEach = suppEachArr.map(val => {
          return (periods === 1) ? val : Math.round((val / 1000 * factor / periods)) * 1000;
        });
        const perSuppOther = hasMdp3OtherColumn ? ((periods === 1) ? mdp3OtherAnnual : Math.round((mdp3OtherAnnual / 1000 * factor / periods)) * 1000) : 0;
        // Sum per-period total for this row
        let perTotal = perMain;
        if (hasExtraColumn) perTotal += perExtra;
        if (hasSuppMainColumn) perTotal += perSuppMain;
        perSuppEach.forEach(val => { perTotal += val; });
        if (hasMdp3OtherColumn) perTotal += perSuppOther;
        const annualFromPeriod = perTotal * periods;
        const diff = annualFromPeriod - annualActual;
        // Build row
        html += `<tr><td class="p-2 border text-center">${contractYear}</td>`;
        html += `<td class="p-2 border text-center">${ageThisYear}</td>`;
        // Main product premium
        html += `<td class="p-2 border text-right">${perMain ? formatCurrency(perMain) : ''}</td>`;
        // Extra premium column (optional)
        if (hasExtraColumn) {
          html += `<td class="p-2 border text-right">${perExtra ? formatCurrency(perExtra) : ''}</td>`;
        }
        // Supplementary main column (optional)
        if (hasSuppMainColumn) {
          html += `<td class="p-2 border text-right">${perSuppMain ? formatCurrency(perSuppMain) : ''}</td>`;
        }
        // Supplementary each person
        perSuppEach.forEach(val => {
          html += `<td class="p-2 border text-right">${val ? formatCurrency(val) : ''}</td>`;
        });
        // MDP3-other column (optional)
        if (hasMdp3OtherColumn) {
          html += `<td class="p-2 border text-right">${perSuppOther ? formatCurrency(perSuppOther) : ''}</td>`;
        }
        // Total per period
        html += `<td class="p-2 border text-right font-semibold">${formatCurrency(perTotal)}</td>`;
        // Difference column
        if (freq !== 'year') {
          html += `<td class="p-2 border text-right">${diff ? formatCurrency(diff) : ''}</td>`;
        }
        html += `</tr>`;
      }
      html += `</tbody></table></div>`;
