...
function collectPersonData(container, isMain = false) {
  try {
    const p = collectPersonData(container, !!isMain) || {};
    const info = {
      age: Number.isFinite(p.age) ? p.age : 0,
      daysFromBirth: Number.isFinite(p.daysFromBirth) ? p.daysFromBirth : 0,
      gender: p.gender || 'Nam',
      riskGroup: Number.isFinite(p.riskGroup) ? p.riskGroup : 0,
      container: p.container || container || null,
      name: p.name || (isMain ? 'NĐBH Chính' : 'NĐBH Bổ sung')
    };
    if (isMain) {
      const mp = document.getElementById('main-product');
      info.mainProduct = mp ? mp.value : '';
    }
    return info;
  } catch (e) {
    console.error('[refactored] getCustomerInfo error:', e);
    return {
      age: 0, daysFromBirth: 0, gender: 'Nam', riskGroup: 0,
      container: container || null, name: isMain ? 'NĐBH Chính' : 'NĐBH Bổ sung',
      mainProduct: isMain ? (document.getElementById('main-product')?.value || '') : undefined
    };
  }
}
