...
function updateSummaryUI(fees) {
  ...
  // Frequency breakdown (only show when not yearly)
  ...
  const factor  = periods===2 ? 1.02 : (periods===4 ? 1.04 : 1); // rider only
  ...
  try {
    ...
    for (const p of persons){
      ...
      if (sclSec && sclSec.querySelector('.health-scl-checkbox')?.checked){
        ...
        const name = `Sức khoẻ Bùng Gia Lực – ${program} (${scope==='abroad'?'Nước ngoài':'Trong nước'}${outpatient?', Ngoại trú':''}${dental?', Nha khoa':''})`;
