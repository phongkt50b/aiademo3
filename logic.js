...
function renderSuppListSimple() {
  ...
  if (!wrap) return;
  // if user has expanded the list, keep it locked to avoid overwriting on unrelated UI changes
  ...
  // NĐBH bổ sung
  // Include MDP3 into main if assigned
  try{
    if(window.MDP3){
      const selId = (window.MDP3.getSelectedId && window.MDP3.getSelectedId()) || (document.getElementById('mdp3-person-select')?.value||null);
      const fee = Number(window.MDP3.getPremium ? (window.MDP3.getPremium()||0) : 0);
      if(selId==='main-person-container' && fee>0){
        // update last appended mainRow amount
        const valEl = mainRow.querySelector('span.font-semibold');
        if(valEl){ valEl.textContent = formatCurrency((toInt(valEl.textContent)||0) + fee); }
      }
    }
  }catch(e){}
  ...
  Array.from(document.querySelectorAll('#supplementary-insured-container .person-container')).forEach((cont, idx)=>{
    ...
    row.innerHTML = `<span>${sanitizeHtml(name)}</span><span class="font-semibold">${formatCurrency(convertByFreq(fee))}</span>`;
  ...
  try {
    if (window.MDP3) {
      ...
      if (selId === 'other' && fee > 0) {
        const row = document.createElement('div');
        row.className = 'flex justify-between items-center py-1 text-sm';
        row.innerHTML = `<span>Miễn đóng phí 3.0 (Người khác)</span><span class="font-semibold">${formatCurrency(convertByFreq(fee))}</span>`;
        wrap.appendChild(row);
      }
    }
  } catch {}
...
function generateSummaryTable() {
  function convertByFreq(annual){
    const sel = document.getElementById('payment-frequency');
    const v = Number(annual||0);
    if(!sel || sel.value==='year') return v;
    if(sel.value==='half') return Math.round(v/2000)*1000; // round to 1,000
    if(sel.value==='quarter') return Math.round(v/4000)*1000;
    return v;
  }
  const modal = document.getElementById('summary-modal');
  ...
  try {
      ...
      <div class="text-lg font-semibold mb-2">Phần 1 · Tóm tắt sản phẩm</div>
        ...
        <thead class="bg-gray-100">
          <tr>
            ...
            <th class="p-2 border">Phí đóng (theo kỳ đã chọn)</th>
          </tr>
        </thead>
        <tbody>
          ${(() => {
            const rows = [];
            const sel = document.getElementById('payment-frequency');
            const periods = !sel||sel.value==='year' ? 1 : (sel.value==='half'?2:4);

            function cv(annual){ return periods===1 ? annual : Math.round(annual/periods/1000)*1000; }

            // Nhóm: NĐBH chính (dòng tổng + các dòng sản phẩm)
            const mainSuppNow = (window.personFees?.['main-person-container']?.supp||0) + (function(){
              try{
                if(window.MDP3){
                  const sid = window.MDP3.getSelectedId ? window.MDP3.getSelectedId() : (document.getElementById('mdp3-person-select')?.value||null);
                  const fee = Number(window.MDP3.getPremium ? (window.MDP3.getPremium()||0) : 0);
                  return sid==='main-person-container' ? fee : 0;
                }
              }catch(e){}
              return 0;
            })();
            const mainTotalNow = initialBaseMainPremium + extraPremium + mainSuppNow;
            rows.push(`<tr class="bg-gray-50">
              <td class="p-2 border font-medium" colspan="3">${sanitizeHtml(mainPersonInfo.name || 'NĐBH chính')}</td>
              <td class="p-2 border text-right font-semibold">${formatCurrency(cv(mainTotalNow))}</td>
            </tr>`);
            ...
            rows.push(`<tr>
              ...
              <td class="p-2 border text-center">${mainProduct==='AN_BINH_UU_VIET' ? (document.getElementById('abuv-term')?.value||'—') : (mainPersonInfo.mainProduct && mainPersonInfo.mainProduct.startsWith('PUL') ? (document.getElementById('payment-term')?.value || '—') : (mainPersonInfo.mainProduct==='TRON_TAM_AN' ? 10 : '—'))}</td>
              <td class="p-2 border text-right">${formatCurrency(cv(initialBaseMainPremium))}</td>
            </tr>`);

            // Phí đóng thêm
            if(Number(extraPremium)>0){
              const ptermEl = document.getElementById('payment-term');
              const pterm = ptermEl ? (ptermEl.value||'—') : '—';
              rows.push(`<tr>
                <td class="p-2 border">Phí đóng thêm</td>
                <td class="p-2 border text-right">—</td>
                <td class="p-2 border text-center">${pterm}</td>
                <td class="p-2 border text-right">${formatCurrency(cv(extraPremium))}</td>
              </tr>`);
            }

            // SP bổ sung thuộc NĐBH chính — đọc từ DOM personFees + kỳ
            const mainPf = (window.personFees?.['main-person-container']) || {supp:0};
            if((mainPf.supp||0) > 0 || (function(){ try{ return (window.MDP3 && window.MDP3.getSelectedId && window.MDP3.getSelectedId()==='main-person-container' && (window.MDP3.getPremium && window.MDP3.getPremium()>0)); }catch(e){}; return false; })()){
              rows.push(`<tr><td class="p-2 border italic text-gray-600" colspan="4">Sản phẩm bổ sung của ${sanitizeHtml(mainPersonInfo.name||'NĐBH chính')}</td></tr>`);
            }

            // MDP3 (nếu gán cho NĐBH chính)
            try{
              if(window.MDP3){
                const sid = window.MDP3.getSelectedId ? window.MDP3.getSelectedId() : (document.getElementById('mdp3-person-select')?.value||null);
                const fee = Number(window.MDP3.getPremium ? (window.MDP3.getPremium()||0) : 0);
                if(sid==='main-person-container' && fee>0){
                  // Tính STBH cho MDP3
                  let stbhBase = 0;
                  ...
                  rows.push(`<tr>
                    ...
                    <td class="p-2 border text-center">${document.getElementById('payment-term')?.value || '—'}</td>
                    <td class="p-2 border text-right">${formatCurrency(cv(fee))}</td>
                  </tr>`);
                }
              }
            }catch(e){}

            // Nhóm NĐBH bổ sung: mỗi người 1 nhóm (dòng tổng + các dòng SP)
            const suppNodes = Array.from(document.querySelectorAll('#supplementary-insured-container .person-container'));
            suppNodes.forEach((cont, idx) => {
              const name = cont.querySelector('.name-input')?.value?.trim() || `NĐBH bổ sung ${idx+1}`;
              // tổng SP bổ sung của người này (bao gồm MDP3 nếu gán)
              let totalSupp = (window.personFees?.[cont.id]?.supp||0);
              try{
                if(window.MDP3){
                  const sid = window.MDP3.getSelectedId ? window.MDP3.getSelectedId() : (document.getElementById('mdp3-person-select')?.value||null);
                  const fee = Number(window.MDP3.getPremium ? (window.MDP3.getPremium()||0) : 0);
                  if(sid===cont.id) totalSupp += fee;
                }
              }catch(e){}

              rows.push(`<tr class="bg-gray-50">
                <td class="p-2 border font-medium" colspan="3">${sanitizeHtml(name)}</td>
                <td class="p-2 border text-right font-semibold">${formatCurrency(cv(totalSupp))}</td>
              </tr>`);

              // MDP3 (nếu gán cho người này)
              try{
                if(window.MDP3){
                  const sid = window.MDP3.getSelectedId ? window.MDP3.getSelectedId() : (document.getElementById('mdp3-person-select')?.value||null);
                  const fee = Number(window.MDP3.getPremium ? (window.MDP3.getPremium()||0) : 0);
                  if(sid===cont.id && fee>0){
                    let stbhBase = 0;
                    for (let pid in window.personFees) {
                      stbhBase += (window.personFees[pid].mainBase || 0) + (window.personFees[pid].supp || 0);
                    }
                    // trừ phần bổ sung của chính người này theo công thức MDP3 đang dùng
                    stbhBase -= (window.personFees?.[cont.id]?.supp||0);
                    rows.push(`<tr>
                      <td class="p-2 border">Miễn đóng phí 3.0</td>
                      <td class="p-2 border text-right">${formatCurrency(stbhBase)}</td>
                      <td class="p-2 border text-center">${document.getElementById('payment-term')?.value || '—'}</td>
            ...
            // MDP3 gán cho "Người khác" → nhóm riêng
            try{
              if(window.MDP3){
                const sid = window.MDP3.getSelectedId ? window.MDP3.getSelectedId() : (document.getElementById('mdp3-person-select')?.value||null);
                const fee = Number(window.MDP3.getPremium ? (window.MDP3.getPremium()||0) : 0);
                if(sid==='other' && fee>0){
                  // STBH tổng (không trừ ai)
                  let stbhBase = 0;
                  for (let pid in window.personFees) {
                    stbhBase += (window.personFees[pid].mainBase || 0) + (window.personFees[pid].supp || 0);
                  }
                  rows.push(`<tr class="bg-gray-50">
                    <td class="p-2 border font-medium" colspan="3">Người khác</td>
                    <td class="p-2 border text-right font-semibold">${formatCurrency(cv(fee))}</td>
                  </tr>`);
                  rows.push(`<tr>
                    <td class="p-2 border">Miễn đóng phí 3.0</td>
                    <td class="p-2 border text-right">${formatCurrency(stbhBase)}</td>
    ...
    if (periods !== 1) { tableHtml += `<th class="p-2 border">Chênh lệch so với năm</th>`; }
    ...
    for (let i = 0; (mainPersonInfo.age + i) <= targetAge; i++) {
      ...
      if (periods !== 1) { tableHtml += `<td class="p-2 border text-right">${formatCurrency(diff)}</td>`; }
      tableHtml += `</tr>`;
    }
...
function buildSupplementSummaryRows(personInfo, container, targetAge) {
  ...
  if (sclSec && sclSec.querySelector('.health-scl-checkbox')?.checked) {
    ...
    rows.push(`<tr>
      ...
      <td class="p-2 border text-right">${formatCurrency(convertByFreq(fee))}</td>
    </tr>`);
  }
...
function updateMainProductFeeDisplay(basePremium, extraPremium) {
  ...
  if (extraPremium > 0) {
    el.innerHTML = `Phí SP chính: ${formatCurrency(basePremium)} | Phí đóng thêm: ${formatCurrency(convertByFreq(extraPremium))} | Tổng: ${formatCurrency(basePremium + extraPremium)}`;
  } else {
    ...
    try{
      ...
      if(suppPersons.length>0){
        ...
        suppPersons.forEach((p, idx)=>{
          ...
          html += `<div class="flex justify-between items-center py-1"><span class="text-sm">${sanitizeHtml(name)}</span><span class="font-semibold">${formatCurrency(convertByFreq(fee))}</span></div>`;
        });
        html += `</div>`;
      }
      ...
      // frequency breakdown handled elsewhere; skipping here;
      ...
      if(freqInfo.periods === 1){
        ...
        freqHtml = `<div class="mb-2"> ${freq==='half'?'Nửa năm':'Quý'}</div>`;
      ...
      if(freqContainer){
        ...
        freqContainer.classList.toggle("hidden", freqInfo.periods === 1);
      }
...
(() => {
  ...
  function renderSection6V2(){
    try{
      ...
      if(list){
        if (list.dataset.locked === '1') { /* keep user's view stable */ } else {
        list.innerHTML = '';
  ...
  document.body.addEventListener('input', (e) => { if(!(e && (e.target && (e.target.id==='payment-frequency' || e.target.closest && e.target.closest('#payment-frequency'))))) { try { renderSection6V2(); } catch(e){} } });
  document.body.addEventListener('change', (e) => { if(!(e && (e.target && (e.target.id==='payment-frequency' || e.target.closest && e.target.closest('#payment-frequency'))))) { try { renderSection6V2(); } catch(e){} } });
})();
...
(function() {
  ...
  function updatePeriodBreakdown() {
    ...
    const {main, extra, suppAll, total} = computeYearTotals();

    // Theo kỳ
    const periods = sel.value === "half" ? 2 : (sel.value === "quarter" ? 4 : 1);
    const mainPeriod = periods===1 ? main : round1000(main/periods);
    const extraPeriod = periods===1 ? extra : round1000(extra/periods);
    let suppPeriod;
    if (periods===2) {
    ...
    const totalPeriod = Math.round(mainPeriod + extraPeriod + suppPeriod);
    const toYear = periods * totalPeriod;
    const diff = toYear - total;

    setText("#freq-main", mainPeriod);
    setText("#freq-extra", extraPeriod);
  ...
  function setupSuppAccordion() {
    ...
    btn.addEventListener("click", () => {
      ...
      // lock content when visible so other re-renders (e.g., changing frequency) won't overwrite it
      list.dataset.locked = list.classList.contains("hidden") ? "0" : "1";
