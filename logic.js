...
(function(){
  ...
  function customGenerateSummary() {
    ...
    try {
      ...
      let paymentTerm = 0;
      if (mainInfo.mainProduct === 'TRON_TAM_AN') {
        paymentTerm = 10;
      ...
      // Validate and compute target age
      const targetAgeInput = document.getElementById('target-age-input');
      const targetAge = parseInt(targetAgeInput?.value || '0', 10);
      ...
      // Determine frequency and per period factors
      const freqSel = document.getElementById('payment-frequency');
      const freq = freqSel ? freqSel.value : 'year';
      const periods = freq === 'half' ? 2 : (freq === 'quarter' ? 4 : 1);
      const factor = freq === 'half' ? 1.02 : (freq === 'quarter' ? 1.04 : 1.0);
      // Helpers to compute per period values
      const perMainExtra = (annual) => {
        if (periods === 1) return annual;
        return Math.floor(annual / periods / 1000) * 1000;
      };
      const perSupp = (annual) => {
        if (periods === 1) return annual;
        return Math.round((annual / 1000 * factor / periods)) * 1000;
      };
      const annualised = (per) => per * periods;
      const diffVal = (annualisedVal, annual) => annualisedVal - annual;
      // Compute years of payment for supplementary products
      const maxCoverage = {
        health_scl: 74,
        bhn: 85,
        accident: 64,
      ...
      function computeYears(type, info) {
        let maxAge = maxCoverage[type] || 0;
        if (!maxAge) return 0;
        if (info.isMain) {
          const stop = Math.min(targetAge, maxAge);
          return Math.max(0, stop - info.age + 1);
        }
        const stop = Math.min(targetAge - mainInfo.age + info.age, maxAge);
        return Math.max(0, stop - info.age + 1);
      }
      // Helper to get annual premiums per product for a person at their current age
      function getAnnualPremiums(info, container) {
        const premiums = { health_scl: 0, bhn: 0, accident: 0, hospital_support: 0 };
        if (!container) return premiums;
        premiums.health_scl = calculateHealthSclPremium(info, container, info.age) || 0;
        premiums.bhn = calculateBhnPremium(info, container, info.age) || 0;
        premiums.accident = calculateAccidentPremium(info, container, info.age) || 0;
        premiums.hospital_support = calculateHospitalSupportPremium(info, calculateMainPremium(mainInfo), container, 0, info.age) || 0;
        return premiums;
      }
      // Helper to get STBH for supplementary products
      function getStbh(info, container) {
        const stbh = { health_scl: 0, bhn: 0, accident: 0, hospital_support: 0 };
        if (!container) return stbh;
        // Health SCL STBH from program value
        const sclSec = container.querySelector('.health-scl-section');
        if (sclSec) {
          const program = sclSec.querySelector('.health-scl-program')?.value || '';
          stbh.health_scl = getHealthSclStbhByProgram(program);
        }
        const bhnSec = container.querySelector('.bhn-section');
        if (bhnSec) {
          stbh.bhn = parseFormattedNumber(bhnSec.querySelector('.bhn-stbh')?.value || '0');
        }
        const accSec = container.querySelector('.accident-section');
        if (accSec) {
          stbh.accident = parseFormattedNumber(accSec.querySelector('.accident-stbh')?.value || '0');
        }
        const hsSec = container.querySelector('.hospital-support-section');
        if (hsSec) {
          stbh.hospital_support = parseFormattedNumber(hsSec.querySelector('.hospital-support-stbh')?.value || '0');
        }
        return stbh;
      }
      // Retrieve supplementary person infos
      const suppPersons = [];
      document.querySelectorAll('#supplementary-insured-container .person-container').forEach(pCont => {
        ...
        pInfo.isMain = false;
        pInfo.container = pCont;
        suppPersons.push(pInfo);
      });
      // Determine MDP3 selection and premium & STBH
      let mdp3Sel = null;
      let mdp3Annual = 0;
      let mdp3Stbh = 0;
      let mdp3PersonInfo = null;
      if (window.MDP3 && window.MDP3.getPremium && Number(window.MDP3.getPremium())) {
        try {
          mdp3Sel = window.MDP3.getSelectedId && window.MDP3.getSelectedId();
          mdp3Annual = Number(window.MDP3.getPremium ? (window.MDP3.getPremium() || 0) : 0);
        } catch (e) {}
        if (mdp3Sel) {
          let stbhBase = 0;
          if (window.personFees) {
            ...
            if (mdp3Sel !== 'other' && window.personFees[mdp3Sel]) {
              stbhBase -= (window.personFees[mdp3Sel].supp || 0);
            }
          }
          mdp3Stbh = stbhBase;
          if (mdp3Sel === 'main-person-container') {
            mdp3PersonInfo = { ...mainInfo, isMain: true };
          } else if (mdp3Sel === 'other') {
            const otherCont = document.getElementById('person-container-mdp3-other');
            if (otherCont) {
              const info = getCustomerInfo(otherCont, false);
              info.isMain = false;
              info.container = otherCont;
              mdp3PersonInfo = info;
            }
          } else {
            const idx = suppPersons.findIndex(p => p.container.id === mdp3Sel);
            if (idx >= 0) {
              mdp3PersonInfo = suppPersons[idx];
            }
          }
        }
      }
      // Build persons array with rows
      const persons = [];
      // Function to add person data
      function buildPerson(info, isMainPerson) {
        const personData = { name: info.name || (isMainPerson ? 'NĐBH chính' : 'NĐBH bổ sung'), rows: [], totalAnnual: 0, totalPeriod: 0, totalAnnualised: 0, totalDiff: 0 };
        const containerEl = isMainPerson ? document.querySelector('#main-supp-container .supplementary-products-container') : info.container.querySelector('.supplementary-products-container');
        // Main base and extra only for main person
        if (isMainPerson) {
          const stbhMain = parseFormattedNumber(document.getElementById('main-stbh')?.value || '0');
          const annualMain = calculateMainPremium(info);
          if (annualMain > 0) {
            const years = paymentTerm;
            const per = perMainExtra(annualMain);
            const ann2 = annualised(per);
            const diff = freq === 'year' ? 0 : diffVal(ann2, annualMain);
            personData.rows.push({
              name: 'Sản phẩm chính',
              stbh: stbhMain,
              years,
              annual: annualMain,
            ...
            personData.totalAnnual += annualMain;
            personData.totalPeriod += per;
            personData.totalAnnualised += ann2;
            personData.totalDiff += diff;
          }
          const extraVal = Number(getExtraPremiumValue ? (getExtraPremiumValue() || 0) : 0);
          if (extraVal > 0) {
            const years = paymentTerm;
            const per = perMainExtra(extraVal);
            const ann2 = annualised(per);
            const diff = freq === 'year' ? 0 : diffVal(ann2, extraVal);
            personData.rows.push({
              name: 'Phí đóng thêm',
              stbh: 0,
              years,
            ...
            personData.totalAnnual += extraVal;
            personData.totalPeriod += per;
            personData.totalAnnualised += ann2;
            personData.totalDiff += diff;
          }
        }
        // Supplementary products for this person
        const premObj = getAnnualPremiums(info, containerEl);
        const stbhObj = getStbh(info, containerEl);
        // Health SCL
        if (premObj.health_scl > 0) {
          const years = computeYears('health_scl', info);
          if (years > 0) {
            const annual = premObj.health_scl;
            const per = perSupp(annual);
            const ann2 = annualised(per);
            const diff = freq === 'year' ? 0 : diffVal(ann2, annual);
            personData.rows.push({
              name: 'Sức khỏe Bùng Gia Lực',
              stbh: stbhObj.health_scl,
              years,
              annual,
              per,
              annualised: ann2,
        ...
        // BHN
        if (premObj.bhn > 0) {
          const years = computeYears('bhn', info);
          if (years > 0) {
            const annual = premObj.bhn;
            const per = perSupp(annual);
            const ann2 = annualised(per);
            const diff = freq === 'year' ? 0 : diffVal(ann2, annual);
            personData.rows.push({
        ...
        // Accident
        if (premObj.accident > 0) {
          const years = computeYears('accident', info);
          if (years > 0) {
            const annual = premObj.accident;
            const per = perSupp(annual);
            const ann2 = annualised(per);
            const diff = freq === 'year' ? 0 : diffVal(ann2, annual);
            personData.rows.push({
              name: 'Bảo hiểm Tai nạn',
              stbh: stbhObj.accident,
              years,
              annual,
              per,
              annualised: ann2,
        ...
        // Hospital support
        if (premObj.hospital_support > 0) {
          const years = computeYears('hospital_support', info);
          if (years > 0) {
            const annual = premObj.hospital_support;
            const per = perSupp(annual);
            const ann2 = annualised(per);
            const diff = freq === 'year' ? 0 : diffVal(ann2, annual);
            personData.rows.push({
              name: 'Hỗ trợ Chi phí nằm viện',
              stbh: stbhObj.hospital_support,
              years,
              annual,
              per,
              annualised: ann2,
        ...
        // MDP3 for this person
        if (mdp3Annual > 0 && mdp3PersonInfo && mdp3PersonInfo.container && mdp3PersonInfo.container.id === (info.container?.id || (isMainPerson ? 'main-person-container' : info.container.id))) {
          const years = computeYears('mdp3', info);
          if (years > 0) {
            const annual = mdp3Annual;
            const per = perSupp(annual);
            const ann2 = annualised(per);
            const diff = freq === 'year' ? 0 : diffVal(ann2, annual);
            personData.rows.push({
      ...
      mainInfo.container = mainContainer;
      const mainPersonData = buildPerson(mainInfo, true);
      persons.push(mainPersonData);
      // Build supplementary persons
      suppPersons.forEach(info => {
        const pd = buildPerson(info, false);
        if (pd.rows.length > 0) persons.push(pd);
      });
      // Build mdp3-other person if assigned and not included above
      if (mdp3Annual > 0 && mdp3Sel === 'other' && mdp3PersonInfo) {
        const otherPd = { name: mdp3PersonInfo.name || 'Người khác', rows: [], totalAnnual: 0, totalPeriod: 0, totalAnnualised: 0, totalDiff: 0 };
        const years = computeYears('mdp3', mdp3PersonInfo);
        if (years > 0) {
          const per = perSupp(mdp3Annual);
          const ann2 = annualised(per);
          const diff = freq === 'year' ? 0 : diffVal(ann2, mdp3Annual);
          otherPd.rows.push({
            name: 'Miễn đóng phí 3.0',
            stbh: mdp3Stbh,
            years,
          ...
          otherPd.totalAnnual = mdp3Annual;
          otherPd.totalPeriod = per;
          otherPd.totalAnnualised = ann2;
          otherPd.totalDiff = diff;
          persons.push(otherPd);
        }
      }
      // Construct HTML for Part 1
      let html = '';
      ...
      // Append person rows
      persons.forEach(pd => {
        html += `<tr class="bg-gray-50"><td class="p-2 border font-semibold">${sanitizeHtml(pd.name)}</td><td class="p-2 border"></td><td class="p-2 border text-center"></td><td class="p-2 border text-right font-semibold text-aia-red">${formatCurrency(pd.totalPeriod)}</td><td class="p-2 border text-right">${formatCurrency(pd.totalAnnualised)}</td>`;
        if (freq !== 'year') {
          html += `<td class="p-2 border text-right">${formatCurrency(pd.totalDiff)}</td>`;
        }
        html += `</tr>`;
        pd.rows.forEach(r => {
          if (!r.annual || r.annual <= 0) return;
          html += `<tr><td class="p-2 border">${sanitizeHtml(r.name)}</td><td class="p-2 border text-right">${r.stbh ? formatCurrency(r.stbh) : ''}</td><td class="p-2 border text-center">${r.years}</td><td class="p-2 border text-right">${formatCurrency(r.per)}</td><td class="p-2 border text-right">${formatCurrency(r.annualised)}</td>`;
          if (freq !== 'year') {
            html += `<td class="p-2 border text-right">${formatCurrency(r.diff)}</td>`;
          }
          html += `</tr>`;
        });
      });
      ...
      html += `</tr></thead><tbody>`;
      // Recompute per-year table using existing logic
      for (let i = 0; (mainInfo.age + i) <= targetAge; i++) {
        const ageThisYear = mainInfo.age + i;
        const contractYear = i + 1;
        ...
        let mdp3Fee = 0;
        let mdp3Selected = null;
        try {
          if (window.MDP3) {
            mdp3Selected = window.MDP3.getSelectedId && window.MDP3.getSelectedId();
            mdp3Fee = Number(window.MDP3.getPremium ? (window.MDP3.getPremium() || 0) : 0);
          }
        } catch (e) {}
        let mdp3OtherAnnual = 0;
        if (mdp3Fee > 0) {
          if (mdp3Selected === 'main-person-container') {
            suppMain += mdp3Fee;
          } else if (mdp3Selected === 'other') {
            mdp3OtherAnnual = mdp3Fee;
          } else {
            const idx = suppPersons.findIndex(p => p.container?.id === mdp3Selected);
            if (idx >= 0) suppEachArr[idx] += mdp3Fee;
          }
        }
        const annualSupp = suppMain + suppEachArr.reduce((a,b) => a+b,0) + mdp3OtherAnnual;
        if ((mainThisYear + extraThisYear + annualSupp) === 0) continue;
        const perME = (periods === 1) ? (mainThisYear + extraThisYear) : Math.floor((mainThisYear + extraThisYear) / periods / 1000) * 1000;
        const perSuppTot = (periods === 1) ? annualSupp : Math.round((annualSupp / 1000 * factor / periods)) * 1000;
        const perTot = perME + perSuppTot;
        const toYear = perTot * periods;
        const diff2 = toYear - (mainThisYear + extraThisYear + annualSupp);
        html += `<tr><td class="p-2 border text-center">${contractYear}</td><td class="p-2 border text-center">${ageThisYear}</td><td class="p-2 border text-right">${formatCurrency(mainThisYear)}</td><td class="p-2 border text-right">${formatCurrency(extraThisYear)}</td><td class="p-2 border text-right">${formatCurrency(suppMain)}</td>`;
        suppEachArr.forEach(val => {
          html += `<td class="p-2 border text-right">${formatCurrency(val)}</td>`;
        });
        html += `<td class="p-2 border text-right">${formatCurrency(perTot)}</td>`;
        if (freq !== 'year') {
          html += `<td class="p-2 border text-right">${formatCurrency(diff2)}</td>`;
        }
        html += `</tr>`;
      }
      html += `</tbody></table></div>`;
