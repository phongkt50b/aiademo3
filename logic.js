...
(() => {
    ...
    try{
      ...
      if(fb){
        if(freq==='year'){
          ...
          const elMainExtra = document.getElementById('freq-main-plus-extra');
          ...
          if(elMainExtra) elMainExtra.textContent = fmt(info.perMain + info.perExtra);
  ...
  // Disable legacy override of generateSummaryTable.  Preserve the legacy implementation
  // under a different name so we can reference it if needed, but do not
  // reassign window.generateSummaryTable here.  The final summary override is
  // defined later in the file.
  window.__legacyGenerateSummary = function(){
    const container = document.getElementById('summary-content-container');
    const modal = document.getElementById('summary-modal');
...
(function(){
  ...
  // This implementation builds a detailed illustration table that follows the
  // specifications from the user.  It shows per-person totals along with rows
  // for each product, includes the number of years premiums are payable, the
  // per‑period premium, the annualised premium and the difference between the
  // annualised premium and the standard annual premium.  It also hides zero
  // rows and omits the difference column when paying annually.
  function customGenerateSummary() {
    const container = document.getElementById('summary-content-container');
    const modal = document.getElementById('summary-modal');
    if (container) container.innerHTML = '';
    try {
      // Retrieve main person info and check validity
      const mainContainer = document.getElementById('main-person-container');
      const mainInfo = getCustomerInfo(mainContainer, true);
      // Determine payment term for main product
      let paymentTerm = 0;
      if (mainInfo.mainProduct === 'TRON_TAM_AN') paymentTerm = 10;
      else if (mainInfo.mainProduct === 'AN_BINH_UU_VIET') {
        paymentTerm = parseInt(document.getElementById('abuv-term')?.value || '15', 10);
      } else {
        paymentTerm = parseInt(document.getElementById('payment-term')?.value || '0', 10) || 0;
      }
      // Retrieve target age and validate
      const targetAgeInput = document.getElementById('target-age-input');
      const targetAge = parseInt(targetAgeInput?.value || '0', 10);
      ...
      // Determine payment frequency: year, half, quarter
      const freqSel = document.getElementById('payment-frequency');
      const freq = freqSel ? freqSel.value : 'year';
      const periods = freq === 'half' ? 2 : (freq === 'quarter' ? 4 : 1);
      const factor = freq === 'half' ? 1.02 : (freq === 'quarter' ? 1.04 : 1.0);
      // Helper to compute per period premium for main and extra
      const perMainExtra = (val) => {
        val = Number(val) || 0;
        if (periods === 1) return roundDownTo1000(val);
        return Math.floor(val / periods / 1000) * 1000;
      };
      // Helper to compute per period premium for supplementary (including MDP3)
      const perSupplementary = (val) => {
        val = Number(val) || 0;
        if (periods === 1) return roundDownTo1000(val);
        return Math.round((val / 1000 * factor / periods)) * 1000;
      };
      // Helper to compute number of payment years for supplementary products
      function computeYears(currAge, maxAge) {
        return Math.max(0, Math.min(targetAge, maxAge) - currAge + 1);
      }
      // Collect supplementary persons info
      const suppPersons = [];
      document.querySelectorAll('#supplementary-insured-container .person-container').forEach(pCont => {
        const info = getCustomerInfo(pCont, false);
        suppPersons.push(info);
      });
      // Helper to compute supplementary product rows for a given person
      function computeSupplementRows(personInfo, annualMainPremium) {
        const rows = [];
        const container = personInfo.container ? personInfo.container.querySelector('.supplementary-products-container') : document.querySelector('#main-supp-container .supplementary-products-container');
        if (!container) return rows;
        // Sức khỏe Bùng Gia Lực
        const scl = container.querySelector('.health-scl-section');
        if (scl && scl.querySelector('.health-scl-checkbox')?.checked) {
          const program = scl.querySelector('.health-scl-program')?.value || '';
          const programLabel = {co_ban:'Cơ bản', nang_cao:'Nâng cao', toan_dien:'Toàn diện', hoan_hao:'Hoàn hảo'}[program] || '';
          const stbh = getHealthSclStbhByProgram(program);
          const annualFee = calculateHealthSclPremium(personInfo, container, personInfo.age);
          const years = computeYears(personInfo.age, 74);
          const perFee = perSupplementary(annualFee);
          const annualised = perFee * periods;
          const diff = annualised - annualFee;
          if (perFee > 0) {
            rows.push({
        ...
        if (bhnSec && bhnSec.querySelector('.bhn-checkbox')?.checked) {
          const stbh = parseFormattedNumber(bhnSec.querySelector('.bhn-stbh')?.value || '0');
          const annualFee = calculateBhnPremium(personInfo, container, personInfo.age);
          const years = computeYears(personInfo.age, 85);
          const perFee = perSupplementary(annualFee);
          const annualised = perFee * periods;
          const diff = annualised - annualFee;
          if (perFee > 0) {
        ...
        if (accSec && accSec.querySelector('.accident-checkbox')?.checked) {
          const stbh = parseFormattedNumber(accSec.querySelector('.accident-stbh')?.value || '0');
          const annualFee = calculateAccidentPremium(personInfo, container, personInfo.age);
          const years = computeYears(personInfo.age, 64);
          const perFee = perSupplementary(annualFee);
          const annualised = perFee * periods;
          const diff = annualised - annualFee;
          if (perFee > 0) {
        ...
        if (hsSec && hsSec.querySelector('.hospital-support-checkbox')?.checked) {
          const stbh = parseFormattedNumber(hsSec.querySelector('.hospital-support-stbh')?.value || '0');
          const annualFee = calculateHospitalSupportPremium(personInfo, annualMainPremium || 0, container, 0, personInfo.age);
          const years = computeYears(personInfo.age, 64);
          const perFee = perSupplementary(annualFee);
          const annualised = perFee * periods;
          const diff = annualised - annualFee;
          if (perFee > 0) {
            rows.push({
              name: 'Hỗ trợ chi phí nằm viện (đ/ngày)',
              stbh,
              years,
              perFee,
              annualised,
      ...
      // Helper to compute MDP3 row for a person
      function computeMdp3Row(personInfo) {
        let result = null;
        try {
          if (window.MDP3) {
            const selId = window.MDP3.getSelectedId && window.MDP3.getSelectedId();
            const premium = Number(window.MDP3.getPremium ? (window.MDP3.getPremium() || 0) : 0);
            if (premium > 0 && selId === (personInfo.container ? personInfo.container.id : 'main-person-container')) {
              // Compute STBH: sum of all mainBase + supplementary minus this person's supplementary if not 'other'
              let stbhBase = 0;
              ...
              // Determine person's age for mdp3: use their current age
              const years = computeYears(personInfo.age, 64);
              const perFee = perSupplementary(premium);
              const annualised = perFee * periods;
              const diff = annualised - premium;
              result = {
      ...
      // Compute person objects with rows and totals
      const persons = [];
      // Helper to build product rows for each person and compute total
      function buildPerson(personInfo, isMain) {
        const person = {
          name: personInfo.name || (isMain ? 'NĐBH chính' : 'NĐBH bổ sung'),
          rows: [],
          perTotal: 0,
          annualTotal: 0,
          diffTotal: 0,
          containerId: personInfo.container ? personInfo.container.id : 'main-person-container'
        };
        // Main product row for main person
        if (isMain) {
          const baseAnnual = calculateMainPremium(personInfo);
          const perFee = perMainExtra(baseAnnual);
          const annualised = perFee * periods;
          const diff = annualised - baseAnnual;
          if (perFee > 0) {
            // Determine STBH for main product
            let stbhMain;
            if (personInfo.mainProduct === 'TRON_TAM_AN') {
              stbhMain = 100_000_000;
            } else {
            ...
            person.rows.push({
              name: getProductLabel(personInfo.mainProduct || ''),
              stbh: stbhMain,
              years: paymentTerm || 0,
              perFee,
              annualised,
            ...
            person.perTotal += perFee;
            person.annualTotal += baseAnnual;
            person.diffTotal += diff;
          }
          // Extra premium row
          const extraAnnual = Number(getExtraPremiumValue ? (getExtraPremiumValue() || 0) : 0);
          if (extraAnnual > 0) {
            const perFeeExtra = perMainExtra(extraAnnual);
            const annualisedExtra = perFeeExtra * periods;
            const diffExtra = annualisedExtra - extraAnnual;
            person.rows.push({
              name: 'Phí đóng thêm',
              stbh: '',
              years: paymentTerm || 0,
              perFee: perFeeExtra,
              annualised: annualisedExtra,
              diff: diffExtra
            });
            person.perTotal += perFeeExtra;
            person.annualTotal += extraAnnual;
            person.diffTotal += diffExtra;
          }
        }
        // Supplementary products rows
        const suppRows = computeSupplementRows(personInfo, calculateMainPremium(mainInfo));
        suppRows.forEach(r => {
          person.rows.push(r);
          person.perTotal += r.perFee;
          person.annualTotal += (r.perFee * periods - r.diff); // original annual is annualised - diff
          person.diffTotal += r.diff;
        });
        // MDP3 row if selected for this person
        const mdp3Row = computeMdp3Row(personInfo);
        if (mdp3Row) {
          person.rows.push(mdp3Row);
          person.perTotal += mdp3Row.perFee;
          person.annualTotal += (mdp3Row.perFee * periods - mdp3Row.diff);
          person.diffTotal += mdp3Row.diff;
        }
        // Only include person if any per period premium exists
        if (person.perTotal > 0 || person.annualTotal > 0) {
          persons.push(person);
        }
      }
      // Build main person
      buildPerson(mainInfo, true);
      // Build each supplementary person
      suppPersons.forEach(sp => buildPerson(sp, false));
      // Build MDP3-other person if selected and not assigned above
      try {
        if (window.MDP3) {
          const selId = window.MDP3.getSelectedId && window.MDP3.getSelectedId();
          const premium = Number(window.MDP3.getPremium ? (window.MDP3.getPremium() || 0) : 0);
          if (premium > 0 && selId === 'other') {
            // Get info of other person from mdp3-other container
            const otherCont = document.getElementById('person-container-mdp3-other');
            const otherInfo = getCustomerInfo(otherCont, false) || {};
            // Compute STBH: sum of all mainBase + supplementary of all persons
            let stbhBase = 0;
            for (const pid in window.personFees) {
              stbhBase += (window.personFees[pid].mainBase || 0) + (window.personFees[pid].supp || 0);
            }
            // Age and years for other
            const years = computeYears(otherInfo.age || 0, 64);
            const perFee = perSupplementary(premium);
            const annualised = perFee * periods;
            const diff = annualised - premium;
            const person = {
              name: otherInfo.name || 'Người khác',
              rows: [{
                name: 'Miễn đóng phí 3.0',
                stbh: stbhBase,
                years,
              ...
              annualTotal: premium,
              diffTotal: diff,
              containerId: 'mdp3-other'
            };
            persons.push(person);
          }
        }
      } catch (e) {}
      // Start building HTML for Part 1
      let html = '';
      const freqLabel = (freq === 'year') ? 'năm' : (freq === 'half' ? 'nửa năm' : 'theo quý');
      html += `<div class="mb-4"><div class="text-lg font-semibold mb-2">Phần 1 · Tóm tắt sản phẩm (${freqLabel})</div>`;
      html += `<table class="w-full text-left border-collapse"><thead class="bg-gray-100"><tr>`;
      html += `<th class="p-2 border">Sản phẩm / Người</th>`;
      html += `<th class="p-2 border">STBH</th>`;
      html += `<th class="p-2 border">Số năm đóng phí</th>`;
      html += `<th class="p-2 border">Phí đóng (${freqLabel})</th>`;
      html += `<th class="p-2 border">Phí đóng (quy năm)</th>`;
      if (periods !== 1) {
        html += `<th class="p-2 border">Chênh lệch</th>`;
      }
      html += `</tr></thead><tbody>`;
      persons.forEach(person => {
        // Header row for person
        html += `<tr>
          <td class="p-2 border font-semibold">${sanitizeHtml(person.name)}</td>
          <td class="p-2 border text-right">—</td>
          <td class="p-2 border text-center">—</td>
          <td class="p-2 border text-right font-semibold">${formatCurrency(person.perTotal)}</td>
          <td class="p-2 border text-right font-semibold">${formatCurrency(person.perTotal * periods)}</td>`;
        if (periods !== 1) {
          html += `<td class="p-2 border text-right font-semibold">${formatCurrency(person.diffTotal)}</td>`;
        }
        html += `</tr>`;
        // Product rows
        person.rows.forEach(row => {
          html += `<tr>
            <td class="p-2 border">${sanitizeHtml(row.name)}</td>
            <td class="p-2 border text-right">${row.stbh ? formatCurrency(row.stbh) : ''}</td>
            <td class="p-2 border text-center">${row.years || ''}</td>
            <td class="p-2 border text-right">${formatCurrency(row.perFee)}</td>
            <td class="p-2 border text-right">${formatCurrency(row.annualised)}</td>`;
          if (periods !== 1) {
            html += `<td class="p-2 border text-right">${formatCurrency(row.diff)}</td>`;
          }
          html += `</tr>`;
        });
      });
      html += `</tbody></table></div>`;
      // Build Part 2 - per-year fee table
      html += `<div class="mb-4"><div class="text-lg font-semibold mb-2">Phần 2 · Bảng phí (${freqLabel})</div>`;
      html += `<table class="w-full text-left border-collapse"><thead class="bg-gray-100"><tr>`;
      html += `<th class="p-2 border">Năm HĐ</th>`;
      html += `<th class="p-2 border">Tuổi NĐBH chính<br>(${sanitizeHtml(mainInfo.name)})</th>`;
      html += `<th class="p-2 border">Phí chính</th>`;
      html += `<th class="p-2 border">Phí đóng thêm</th>`;
      // Columns for supplementary premiums: main person and each supplementary person + MDP3 other if exists
      html += `<th class="p-2 border">Phí bổ sung<br>(${sanitizeHtml(mainInfo.name)})</th>`;
      suppPersons.forEach(p => {
        html += `<th class="p-2 border">Phí bổ sung<br>(${sanitizeHtml(p.name)})</th>`;
      });
      // Column for MDP3-other if present
      let mdp3OtherIncluded = false;
      try {
        if (window.MDP3) {
          const selId = window.MDP3.getSelectedId && window.MDP3.getSelectedId();
          const premium = Number(window.MDP3.getPremium ? (window.MDP3.getPremium() || 0) : 0);
          if (premium > 0 && selId === 'other') {
            mdp3OtherIncluded = true;
            const otherInfo = getCustomerInfo(document.getElementById('person-container-mdp3-other'), false) || {};
            html += `<th class="p-2 border">Phí bổ sung<br>(${sanitizeHtml(otherInfo.name || 'Người khác')})</th>`;
          }
        }
      } catch (e) {}
      // Total and difference columns
      html += `<th class="p-2 border">Tổng (${freqLabel})</th>`;
      html += `<th class="p-2 border">Tổng (quy năm)</th>`;
      if (periods !== 1) {
        html += `<th class="p-2 border">Chênh lệch</th>`;
      }
      html += `</tr></thead><tbody>`;
      // Loop over each contract year
      for (let i = 0; (mainInfo.age + i) <= targetAge; i++) {
        const ageThisYear = mainInfo.age + i;
        const contractYear = i + 1;
        // Base and extra premium for this contract year
        const baseAnnual = (contractYear <= paymentTerm) ? calculateMainPremium(mainInfo) : 0;
        const extraAnnual = (contractYear <= paymentTerm) ? Number(getExtraPremiumValue ? (getExtraPremiumValue() || 0) : 0) : 0;
        // Supplementary premiums for main person at this age
        ...
        if (mainSuppCont) {
          ...
          suppMainAnnual += calculateHospitalSupportPremium({ ...mainInfo, age: ageThisYear }, calculateMainPremium(mainInfo), mainSuppCont, 0, ageThisYear);
        }
        // Supplementary premiums for each supplementary person
        ...
        suppPersons.forEach(pInfo => {
          ...
          if (cont) {
            ...
            s += calculateHospitalSupportPremium({ ...pInfo, age: pAge }, calculateMainPremium(mainInfo), cont, 0, pAge);
          }
          suppEachAnnual.push(s);
        });
        // MDP3 distribution per person/year
        let mdp3Fee = 0;
        let mdp3Sel = null;
        try {
          if (window.MDP3) {
        ...
        let mdp3OtherAnnual = 0;
        if (mdp3Fee > 0) {
          if (mdp3Sel === 'main-person-container') {
            suppMainAnnual += mdp3Fee;
          } else if (mdp3Sel === 'other') {
            mdp3OtherAnnual = mdp3Fee;
          } else {
            const idx = suppPersons.findIndex(p => p.container?.id === mdp3Sel);
            if (idx >= 0) suppEachAnnual[idx] += mdp3Fee;
          }
        }
        // Compute totals and skip rows where everything is zero
        const annualSupp = suppMainAnnual + suppEachAnnual.reduce((a,b) => a+b,0) + mdp3OtherAnnual;
        const annualMainExtra = baseAnnual + extraAnnual;
        if (annualMainExtra + annualSupp === 0) continue;
        // Compute per period totals for this year
        const perMainExtra = (periods === 1) ? annualMainExtra : Math.floor(annualMainExtra / periods / 1000) * 1000;
        const perSupp = (periods === 1) ? annualSupp : Math.round((annualSupp / 1000 * factor / periods)) * 1000;
        const perTotal = perMainExtra + perSupp;
        const annualisedTotal = perTotal * periods;
        const diffTotal = annualisedTotal - (annualMainExtra + annualSupp);
        // Build row
        html += `<tr>
          <td class="p-2 border text-center">${contractYear}</td>
          <td class="p-2 border text-center">${ageThisYear}</td>
          <td class="p-2 border text-right">${formatCurrency(baseAnnual)}</td>
          <td class="p-2 border text-right">${formatCurrency(extraAnnual)}</td>
          <td class="p-2 border text-right">${formatCurrency(suppMainAnnual)}</td>`;
        suppEachAnnual.forEach(sVal => {
          html += `<td class="p-2 border text-right">${formatCurrency(sVal)}</td>`;
        });
        if (mdp3OtherIncluded) {
          html += `<td class="p-2 border text-right">${formatCurrency(mdp3OtherAnnual)}</td>`;
        }
        html += `<td class="p-2 border text-right">${formatCurrency(perTotal)}</td>`;
        html += `<td class="p-2 border text-right">${formatCurrency(annualisedTotal)}</td>`;
        if (periods !== 1) {
          html += `<td class="p-2 border text-right">${formatCurrency(diffTotal)}</td>`;
        }
        html += `</tr>`;
      }
      html += `</tbody></table></div>`;
