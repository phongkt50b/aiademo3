...
function generateSummaryTable() {
  ...
  try {
    ...
    // Chỉ hiển thị cột Phí đóng (quy năm) khi không đóng theo năm
    if (periods !== 1) {
      summaryHtml += `<th class="p-2 border">Phí đóng (quy năm)</th>`;
    }
    // Hiển thị cột Chênh lệch khi đóng theo nửa năm hoặc quý
    if (showDiffColumn) summaryHtml += `<th class="p-2 border">Chênh lệch</th>`;
    summaryHtml += `</tr></thead><tbody>`;

    // Dòng tổng cho từng người
    personsList.forEach(person => {
      // Tính số cột dữ liệu cho dòng tổng: nếu đóng theo năm thì chỉ có STBH, Số năm và Phí đóng
      const colspan = (periods === 1) ? 3 : (showDiffColumn ? 5 : 4);
    ...
    try{
      ...
      if(freqInfo.periods === 1){
        ...
        freqHtml += `<div class="grid grid-cols-2 gap-2">
          ...
          <!-- Always style the chênh lệch row in red and bold for non-yearly frequencies -->
          <div class="font-semibold text-red-600">Chênh lệch:</div><div class="text-right font-semibold text-red-600">${formatCurrency(freqInfo.diff)}</div>
        </div>`;
      }
...
(function(){
  ...
  function buildMdp3OtherSummary(targetAge, freq) {
    try {
      ...
      if (fee > 0 && selId === 'other') {
        ...
        // For "Người khác" treat as a supplementary insured relative to main person
        // Use computeSupplementYears to calculate years like other supplementary persons
        const mainInfoForMdp = getCustomerInfo(document.getElementById('main-person-container'), true);
        const mdpYears = computeSupplementYears((otherInfo.age || 0), mainInfoForMdp.age, targetAge, 65);
  ...
  function customGenerateSummary() {
    ...
    try {
      ...
      html += `</tr></thead><tbody>`;
      // Determine periods and factor for payment frequency
      const periods = freq === 'half' ? 2 : (freq === 'quarter' ? 4 : 1);
      const factor = freq === 'half' ? 1.02 : (freq === 'quarter' ? 1.04 : 1.0);
      // Determine if MDP3 is applied to 'other' and get the name
      let hasMdpOther = false;
      let mdpOtherName = '';
      let mdpFee = 0;
      let mdpSelectedId = null;
      try {
        if (window.MDP3) {
          mdpSelectedId = window.MDP3.getSelectedId && window.MDP3.getSelectedId();
          mdpFee = Number(window.MDP3.getPremium ? (window.MDP3.getPremium() || 0) : 0);
          if (mdpFee > 0 && mdpSelectedId === 'other') {
            const otherForm = document.getElementById('person-container-mdp3-other');
            const otherNameInput = otherForm?.querySelector('.name-input');
            mdpOtherName = (otherNameInput?.value?.trim() || 'Người khác');
            hasMdpOther = true;
          }
        }
      } catch (e) {}
      // If MDP3-other exists, add an extra header column for that person
      if (hasMdpOther) {
        html = html.replace(/<th class=\"p-2 border\">Tổng cộng<\\\/th>/, `<th class=\"p-2 border\">Phí bổ sung<br>(${sanitizeHtml(mdpOtherName)})</th><th class=\"p-2 border\">Tổng cộng</th>`);
      }
      // Prepare totals accumulator
      let sumMain = 0;
      let sumExtra = 0;
      let sumSuppMain = 0;
      const sumSuppEach = Array(suppPersons.length).fill(0);
      let sumMdp = 0;
      let sumPerTotal = 0;
      let sumDiff = 0;
      // Iterate through each contract year
      for (let i = 0; (mainInfo.age + i) <= targetAge; i++) {
        const ageThisYear = mainInfo.age + i;
        const contractYear = i + 1;
        // Premiums for base and extra
        ...
        // Supplementary base for main person
        let suppMainBase = 0;
        const mainSuppCont = document.querySelector('#main-supp-container .supplementary-products-container');
        if (mainSuppCont) {
          suppMainBase += calculateHealthSclPremium({ ...mainInfo, age: ageThisYear }, mainSuppCont, ageThisYear);
          suppMainBase += calculateBhnPremium({ ...mainInfo, age: ageThisYear }, mainSuppCont, ageThisYear);
          suppMainBase += calculateAccidentPremium({ ...mainInfo, age: ageThisYear }, mainSuppCont, ageThisYear);
          suppMainBase += calculateHospitalSupportPremium({ ...mainInfo, age: ageThisYear }, calculateMainPremium(mainInfo), mainSuppCont, 0, ageThisYear);
        }
        // Supplementary base for each supplementary person
        const suppEachBase = [];
        suppPersons.forEach(pInfo => {
          ...
          let sBase = 0;
          if (pCont) {
            sBase += calculateHealthSclPremium({ ...pInfo, age: pAge }, pCont, pAge);
            sBase += calculateBhnPremium({ ...pInfo, age: pAge }, pCont, pAge);
            sBase += calculateAccidentPremium({ ...pInfo, age: pAge }, pCont, pAge);
            sBase += calculateHospitalSupportPremium({ ...pInfo, age: pAge }, calculateMainPremium(mainInfo), pCont, 0, pAge);
          }
          suppEachBase.push(sBase);
        });
        // Determine mdp base for "other" if applicable
        let mdpBase = 0;
        if (mdpFee > 0 && mdpSelectedId === 'other') {
          mdpBase = mdpFee;
        } else if (mdpFee > 0 && mdpSelectedId === 'main-person-container') {
          // If MDP3 is applied to main person, add to main supplementary base
          suppMainBase += mdpFee;
        } else if (mdpFee > 0) {
          // Check if MDP3 applies to one of the supplementary persons
          const idx = suppPersons.findIndex(p => p.container?.id === mdpSelectedId);
          if (idx >= 0) {
            suppEachBase[idx] += mdpFee;
          }
        }
        // Compute adjusted supplementary premiums (quy năm) for each person
        const suppMainAdj = (periods === 1) ? suppMainBase : Math.round((suppMainBase / 1000 * factor)) * 1000;
        const suppEachAdj = suppEachBase.map(val => (periods === 1) ? val : Math.round((val / 1000 * factor)) * 1000);
        const mdpAdj = (mdpBase > 0) ? ((periods === 1) ? mdpBase : Math.round((mdpBase / 1000 * factor)) * 1000) : 0;
        // Compute per-period premium for main and extra
        let perMainExtra;
        let diffMainExtra = 0;
        const basePlusExtra = mainThisYear + extraThisYear;
        if (periods === 1) {
          // Round down to nearest 1.000đ for yearly payments
          perMainExtra = roundDownTo1000(basePlusExtra);
          // No difference shown for yearly rounding
        } else {
          perMainExtra = Math.floor(basePlusExtra / periods / 1000) * 1000;
          diffMainExtra = perMainExtra * periods - basePlusExtra;
        }
        // Compute total adjusted supplementary base and per-period supplementary premium
        const baseSuppTotal = suppMainBase + suppEachBase.reduce((a,b) => a + b, 0) + mdpBase;
        const suppAdjTotal = suppMainAdj + suppEachAdj.reduce((a,b) => a + b, 0) + mdpAdj;
        // per-period supplementary premium based on original base (not adjusted) and factor
        let perSuppTotal;
        if (periods === 1) {
          perSuppTotal = baseSuppTotal;
        } else {
          perSuppTotal = Math.round((baseSuppTotal / 1000 * factor / periods)) * 1000;
        }
        // Total per period and annual equivalent
        const perTotal = perMainExtra + perSuppTotal;
        const toYearTotal = (periods === 1) ? perTotal : perTotal * periods;
        // Difference due to rounding and supplementary adjustments
        const diffSupp = suppAdjTotal - baseSuppTotal;
        const totalDiff = diffMainExtra + diffSupp;
        // Skip row if no premiums at all
        if ((mainThisYear + extraThisYear + baseSuppTotal) === 0) continue;
        // Build row
        html += `<tr>`;
        html += `<td class=\"p-2 border text-center\">${contractYear}</td>`;
        html += `<td class=\"p-2 border text-center\">${ageThisYear}</td>`;
        html += `<td class=\"p-2 border text-right\">${mainThisYear ? formatCurrency(mainThisYear) : ''}</td>`;
        html += `<td class=\"p-2 border text-right\">${extraThisYear ? formatCurrency(extraThisYear) : ''}</td>`;
        html += `<td class=\"p-2 border text-right\">${suppMainAdj ? formatCurrency(suppMainAdj) : ''}</td>`;
        // Each supplementary person
        suppEachAdj.forEach(val => {
          html += `<td class=\"p-2 border text-right\">${val ? formatCurrency(val) : ''}</td>`;
        });
        // MDP3-other column if exists
        if (hasMdpOther) {
          html += `<td class=\"p-2 border text-right\">${mdpAdj ? formatCurrency(mdpAdj) : ''}</td>`;
        }
        // Total per period
        html += `<td class=\"p-2 border text-right\">${formatCurrency(perTotal)}</td>`;
        // Difference column
        if (freq !== 'year') {
          html += `<td class=\"p-2 border text-right\">${totalDiff ? formatCurrency(totalDiff) : ''}</td>`;
        }
        html += `</tr>`;
        // Accumulate totals
        sumMain += mainThisYear;
        sumExtra += extraThisYear;
        sumSuppMain += suppMainAdj;
        suppEachAdj.forEach((val, idx) => {
          sumSuppEach[idx] = (sumSuppEach[idx] || 0) + val;
        });
        if (hasMdpOther) sumMdp += mdpAdj;
        sumPerTotal += perTotal;
        sumDiff += totalDiff;
      }
      // Add totals row if there was at least one row
      if (sumMain + sumExtra + sumSuppMain + sumSuppEach.reduce((a,b) => a+b, 0) + sumMdp > 0) {
        html += `<tr>`;
        html += `<td class=\"p-2 border font-semibold\">Tổng</td>`;
        html += `<td class=\"p-2 border\"></td>`;
        html += `<td class=\"p-2 border text-right font-semibold\">${sumMain ? formatCurrency(sumMain) : ''}</td>`;
        html += `<td class=\"p-2 border text-right font-semibold\">${sumExtra ? formatCurrency(sumExtra) : ''}</td>`;
        html += `<td class=\"p-2 border text-right font-semibold\">${sumSuppMain ? formatCurrency(sumSuppMain) : ''}</td>`;
        sumSuppEach.forEach(val => {
          html += `<td class=\"p-2 border text-right font-semibold\">${val ? formatCurrency(val) : ''}</td>`;
        });
        if (hasMdpOther) {
          html += `<td class=\"p-2 border text-right font-semibold\">${sumMdp ? formatCurrency(sumMdp) : ''}</td>`;
        }
        html += `<td class=\"p-2 border text-right font-semibold\">${sumPerTotal ? formatCurrency(sumPerTotal) : ''}</td>`;
        if (freq !== 'year') {
          html += `<td class=\"p-2 border text-right font-semibold\">${sumDiff ? formatCurrency(sumDiff) : ''}</td>`;
        }
        html += `</tr>`;
      }
      html += `</tbody></table></div>`;
