...
function renderUI() {
    ...
    // --- Enforce Trọn Tâm An: no supplementary insured persons ---
    try {
      const mainKey = document.getElementById('main-product')?.value || (appState?.mainProduct?.key||'');
      const isTTA = (mainKey === 'TRON_TAM_AN');
      ...
      if (isTTA) {
        ...
        if (appState && Array.isArray(appState.supplementaryPersons)) appState.supplementaryPersons = [];
...
function updateSummaryUI(fees) {
  // fees: { baseMain, extra, totalSupp, byPerson }
  const fmt = (n)=> formatCurrency(Math.round((Number(n)||0)/1000)*1000);
  try {
    // Tổng năm (baseline)
    const totalYear = (fees.baseMain||0) + (fees.extra||0) + (fees.totalSupp||0);
    const elTotal = document.getElementById('summary-total');
    if (elTotal) elTotal.textContent = formatCurrency(totalYear);

    // Nhóm chính
    const elMain = document.getElementById('main-premium-result');
    if (elMain) elMain.textContent = formatCurrency(fees.baseMain||0);

    const elExtra = document.getElementById('extra-premium-result') || document.getElementById('freq-extra');
    if (elExtra) elExtra.textContent = formatCurrency(fees.extra||0);

    const suppBox = document.getElementById('supplementary-premiums-results');
    if (suppBox) {
      if ((fees.totalSupp||0) > 0) {
        suppBox.innerHTML = `<div class="flex justify-between items-center py-2 border-b">
          <span class="text-gray-700">Phí sản phẩm bổ sung (năm)</span>
          <span class="font-bold text-gray-900">${formatCurrency(fees.totalSupp||0)}</span>
        </div>`;
      } else {
        suppBox.innerHTML = '';
      }
    ...
    // Tính theo kỳ (nếu không phải năm)
    ...
    const perMain  = periods===1 ? 0 : Math.round((fees.baseMain||0)/periods/1000)*1000;
    const perExtra = periods===1 ? 0 : Math.round((fees.extra||0)/periods/1000)*1000;
    const perSupp  = periods===1 ? 0 : Math.round(((fees.totalSupp||0)*factor)/periods/1000)*1000;

    const totalPeriod = (periods===1) ? 0 : (perMain + perExtra + perSupp);
    const diff = totalYear - totalPeriod;

    const elFm = document.getElementById('freq-main');
    if (elFm) elFm.textContent = fmt(perMain);
    const elFx = document.getElementById('freq-extra');
    if (elFx) elFx.textContent = fmt(perExtra);
    const elFs = document.getElementById('freq-supp-total');
    if (elFs) elFs.textContent = fmt(perSupp);
    const elTp = document.getElementById('freq-total-period');
    if (elTp) elTp.textContent = fmt(totalPeriod);
    const elTy = document.getElementById('freq-total-year');
    if (elTy) elTy.textContent = fmt(totalYear);
    const elDf = document.getElementById('freq-diff');
    if (elDf) elDf.textContent = fmt(diff);

    // Giữ tương thích cũ
    const compatTotal = document.getElementById('total-premium-result');
    if (compatTotal) compatTotal.textContent = formatCurrency(totalYear);
  } catch (e) {
    // no-op
  }
}
...
function validateMainPersonInputs(person) {
    ...
    // Validate payment term (manual-entry products)
    try {
      const mainKey = productInfo.key;
      if (['KHOE_BINH_AN','VUNG_TUONG_LAI','PUL_TRON_DOI','PUL_15_NAM','PUL_5_NAM'].includes(mainKey)) {
        const ptEl = document.getElementById('payment-term');
        if (ptEl) {
          const v = parseInt(ptEl.value, 10);
          const bounds = getPaymentTermBounds(customer.age);
          let min = 4;
          if (mainKey === 'PUL_5_NAM') min = 5;
          if (mainKey === 'PUL_15_NAM') min = 15;
          if (!v || v < min || v > bounds.max) {
            setFieldError(ptEl, `Nhập từ ${min} đến ${bounds.max} năm`);
            ok = false;
          } else {
            clearFieldError(ptEl);
          }
        }
      }
    } catch(_) {}
...
function generateSummaryTable() {
  ...
  const getPeriods = () => {
    ...
    const v = sel ? sel.value : 'year';
    if (v === 'half') return 2;
    if (v === 'quarter') return 4;
    return 1;
  };
  const periods = getPeriods();
  const suppFactor = periods === 2 ? 1.02 : (periods === 4 ? 1.04 : 1);
  const fmt = (n)=> (Math.round((Number(n)||0)/1000)*1000).toLocaleString('vi-VN');
  ...
  try {
    ...
    const mainPerson = state?.mainPerson;
    if (!mainPerson) throw new Error('Thiếu thông tin NĐBH chính.');

    const targetAgeInput = document.getElementById('target-age-input');
    const targetAge = parseInt(targetAgeInput?.value, 10);
    if (!targetAge || targetAge <= mainPerson.age) throw new Error('Vui lòng nhập độ tuổi mục tiêu hợp lệ.');

    // Xác định payment term
    let paymentTerm = 999;
    const paymentTermInput = document.getElementById('payment-term');
    if (paymentTermInput) paymentTerm = parseInt(paymentTermInput.value, 10) || 999;
    const mainProductKey = document.getElementById('main-product')?.value || state?.mainProduct?.key || '';
    if (mainProductKey === 'TRON_TAM_AN') paymentTerm = 10;
    if (mainProductKey === 'AN_BINH_UU_VIET') {
      const abuvTerm = document.getElementById('abuv-term')?.value;
      if (abuvTerm) paymentTerm = parseInt(abuvTerm,10);
    }

    // Build list persons
    const suppContainers = Array.from(document.querySelectorAll('#supplementary-insured-container .person-container'));
    const others = suppContainers.map(c => getCustomerInfo(c,false));
    const persons = [mainPerson, ...others];

    function getSuppSelections(containerEl){
      const cont = containerEl || document;
      const res = {};
      const map = {
        'health_scl': '.health-scl-section',
        'bhn': '.bhn-section',
        'accident': '.accident-section',
        'hospital_support': '.hospital-support-section'
      };
      Object.entries(map).forEach(([id, sel])=>{
        const sec = cont.querySelector(sel);
        if (!sec) return;
    ...
    if (periods!==1) html += `<th class="p-2 border">Phí đóng (${periods===2?'nửa năm':'theo quý'})</th><th class="p-2 border">Phí đóng (quy năm)</th>`;
    html += `<th class="p-2 border">Phí đóng theo năm</th>`;
    if (periods!==1) html += `<th class="p-2 border">Chênh lệch</th>`;
    html += `</tr></thead><tbody>`;

    // Tổng cộng
    let totalPer=0,totalAnnEq=0,totalBase=0,totalDiff=0;
    function addRow(personName, prodName, stbhDisplay, years, perPeriod, annualEq, baseAnnual){
      let diff = baseAnnual - annualEq;
      if (periods===1){ perPeriod=0; annualEq=0; diff=0; }
      if (periods!==1) { totalPer += perPeriod; totalAnnEq += annualEq; }
      totalBase += baseAnnual; totalDiff += diff;
      html += `<tr>
        <td class="p-2 border">${personName}</td>
        <td class="p-2 border">${prodName}</td>
        <td class="p-2 border text-right">${stbhDisplay||'—'}</td>
        <td class="p-2 border text-center">${years||'—'}</td>`;
      if (periods!==1) html += `<td class="p-2 border text-right">${fmt(perPeriod)}</td><td class="p-2 border text-right">${fmt(annualEq)}</td>`;
      html += `<td class="p-2 border text-right">${fmt(baseAnnual)}</td>`;
      if (periods!==1) html += `<td class="p-2 border text-right">${diff?` + "`<span class=\"text-red-600 font-bold\">${fmt(diff)}</span>`" + r":'0'}</td>`;
      html += `</tr>`;
    }

    for (const p of persons){
      const containerEl = p.container || document.getElementById('main-person-container');
      const selections = getSuppSelections(containerEl);

      html += `<tr><td class="p-2 border font-semibold">${sanitizeHtml(p.name|| (p===mainPerson?'NĐBH Chính':'NĐBH Bổ sung'))}</td><td class="p-2 border font-semibold">Tổng</td>
               <td class="p-2 border">—</td><td class="p-2 border">—</td>`;
      if (periods!==1) html += `<td class="p-2 border text-right"></td><td class="p-2 border text-right"></td>`;
      html += `<td class="p-2 border text-right"></td>`;
      if (periods!==1) html += `<td class="p-2 border text-right"></td>`;
      html += `</tr>`;

      // SP chính & đóng thêm (chỉ main)
      if (p===mainPerson && state?.mainProduct?.key){
        const baseAnnual = calculateMainPremium(p, state.mainProduct, p.age);
        const perPeriod = periods===1 ? 0 : Math.round(baseAnnual/periods/1000)*1000;
        const annualEq = periods===1 ? 0 : perPeriod*periods;
        addRow(p.name,'SP chính', formatCurrency(state.mainProduct.stbh||0), state.mainProduct.paymentTerm||'—', perPeriod, annualEq, baseAnnual);
      }
      if (p===mainPerson && (state?.mainProduct?.extraPremium||0)>0){
        const baseAnnual = state.mainProduct.extraPremium||0;
        const perPeriod = periods===1 ? 0 : Math.round(baseAnnual/periods/1000)*1000;
        const annualEq = periods===1 ? 0 : perPeriod*periods;
        addRow(p.name,'Phí đóng thêm','—', state.mainProduct.paymentTerm||'—', perPeriod, annualEq, baseAnnual);
      }

      // SCL
      if (selections['health_scl']){
        const sec = containerEl.querySelector('.health-scl-section');
        const program = sec?.querySelector('.health-scl-program')?.value;
        const scope = sec?.querySelector('.health-scl-scope')?.value;
        const outpatient = !!sec?.querySelector('.health-scl-outpatient')?.checked;
        const dental = !!sec?.querySelector('.health-scl-dental')?.checked;
        const name = `Sức khoẻ Bùng Gia Lực – ${program?.replace(/_/g,' ') || ''} (${scope==='abroad'?'Nước ngoài':'Trong nước'}${outpatient?', Ngoại trú':''}${dental?', Nha khoa':''})`;
        const baseAnnual = calculateHealthSclPremium(p, state?.fees?.baseMain||0, 0, p.age);
        const perPeriod = periods===1 ? 0 : Math.round(baseAnnual * suppFactor / periods /1000)*1000;
        const annualEq = periods===1 ? 0 : perPeriod*periods;
        addRow(p.name,name, formatCurrency(CONFIG.supplementaryProducts[0]?.stbhByProgram?.[program]||0),  state?.mainProduct?.paymentTerm||'—', perPeriod, annualEq, baseAnnual);
      }
      // BHN
      if (selections['bhn']){
        const sec = containerEl.querySelector('.bhn-section');
        const stbh = num(sec?.querySelector('.bhn-stbh')?.value||'0');
        const baseAnnual = calculateBhnPremium(p, state?.fees?.baseMain||0, 0, p.age);
        const perPeriod = periods===1 ? 0 : Math.round(baseAnnual * suppFactor / periods /1000)*1000;
        const annualEq = periods===1 ? 0 : perPeriod*periods;
        addRow(p.name,'Bệnh Hiểm Nghèo 2.0', formatCurrency(stbh), state?.mainProduct?.paymentTerm||'—', perPeriod, annualEq, baseAnnual);
      }
      // Tai nạn
      if (selections['accident']){
        const sec = containerEl.querySelector('.accident-section');
        const stbh = num(sec?.querySelector('.accident-stbh')?.value||'0');
        const baseAnnual = calculateAccidentPremium(p, state?.fees?.baseMain||0, 0, p.age);
        const perPeriod = periods===1 ? 0 : Math.round(baseAnnual * suppFactor / periods /1000)*1000;
        const annualEq = periods===1 ? 0 : perPeriod*periods;
        addRow(p.name,'Bảo hiểm Tai nạn', formatCurrency(stbh), state?.mainProduct?.paymentTerm||'—', perPeriod, annualEq, baseAnnual);
      }
      // Hỗ trợ nằm viện
      if (selections['hospital_support']){
        const sec = containerEl.querySelector('.hospital-support-section');
        const stbh = num(sec?.querySelector('.hospital-support-stbh')?.value||'0');
        const baseAnnual = calculateHospitalSupportPremium(p, state?.fees?.baseMain||0, 0, p.age);
        const perPeriod = periods===1 ? 0 : Math.round(baseAnnual * suppFactor / periods /1000)*1000;
        const annualEq = periods===1 ? 0 : perPeriod*periods;
        addRow(p.name,'Hỗ trợ chi phí nằm viện', formatCurrency(stbh), state?.mainProduct?.paymentTerm||'—', perPeriod, annualEq, baseAnnual);
      }
    }
    html += `<tr class="font-semibold bg-gray-50"><td class="p-2 border">Tổng cộng</td><td class="p-2 border"></td><td class="p-2 border"></td><td class="p-2 border"></td>`;
    if (periods!==1) html += `<td class="p-2 border text-right">${fmt(totalPer)}</td><td class="p-2 border text-right">${fmt(totalAnnEq)}</td>`;
    html += `<td class="p-2 border text-right">${fmt(totalBase)}</td>`;
    if (periods!==1) html += `<td class="p-2 border text-right">${totalDiff?fmt(totalDiff):'0'}</td>`;
    html += `</tr></tbody></table>`;

    // ==== PHẦN 2 ====
    html += `<h3 class="text-lg font-bold mt-6 mb-2">Phần 2 · Bảng phí</h3>`;
    const suppCols = persons.map(p => sanitizeHtml(p.name||'NĐBH'));
    html += `<table class="w-full border-collapse text-sm"><thead><tr>
      <th class="p-2 border">Năm HĐ</th>
      <th class="p-2 border">Tuổi NĐBH chính</th>
      <th class="p-2 border">Phí chính</th>
      <th class="p-2 border">Phí đóng thêm</th>`;
    suppCols.forEach((n)=>{ html += `<th class="p-2 border">Phí bổ sung (${n})</th>`; });
    if (periods!==1) html += `<th class="p-2 border">Tổng (theo kỳ)</th>`;
    html += `<th class="p-2 border">${periods===1?'Tổng (năm)':'Nếu đóng theo năm'}</th>`;
    if (periods!==1) html += `<th class="p-2 border">Chênh lệch</th>`;
    html += `</tr></thead><tbody>`;
    ...
    for (let year=1; mainPerson.age + year -1 <= targetAge; year++){
      const ageNow = mainPerson.age + year -1;

      const inPayTerm = (year <= paymentTerm);
      const baseAnnualMain = inPayTerm ? calculateMainPremium(mainPerson, state.mainProduct, ageNow) : 0;
      const extra = inPayTerm ? (state?.mainProduct?.extraPremium||0) : 0;
      ...
      for (let pi=0; pi<persons.length; pi++){
        const p = persons[pi];
        const containerEl = p.container || document.getElementById('main-person-container');
        let y = ageNow + (p===mainPerson ? 0 : (p.age - mainPerson.age));
        let sPeriod=0, sYear=0;

        const secS = containerEl.querySelector('.health-scl-section');
        if (secS && secS.querySelector('.health-scl-checkbox')?.checked){
          const base = calculateHealthSclPremium(p, baseAnnualMain, 0, y);
          sYear += base;
          sPeriod += Math.round(base * suppFactor /1000)*1000;
        }
        const secB = containerEl.querySelector('.bhn-section');
        if (secB && secB.querySelector('.bhn-checkbox')?.checked){
          const base = calculateBhnPremium(p, baseAnnualMain, 0, y);
          sYear += base;
          sPeriod += Math.round(base * suppFactor /1000)*1000;
        }
        const secA = containerEl.querySelector('.accident-section');
        if (secA && secA.querySelector('.accident-checkbox')?.checked){
          const base = calculateAccidentPremium(p, baseAnnualMain, 0, y);
          sYear += base;
          sPeriod += Math.round(base * suppFactor /1000)*1000;
        }
        const secH = containerEl.querySelector('.hospital-support-section');
        if (secH && secH.querySelector('.hospital-support-checkbox')?.checked){
          const base = calculateHospitalSupportPremium(p, baseAnnualMain, 0, y);
          sYear += base;
          sPeriod += Math.round(base * suppFactor /1000)*1000;
        }
        if (mdpEnabled && mdpTargetId === (p.container ? p.container.id : p.id)){
          const fee = (y <= 65) ? mdpFeeYear : 0;
          sYear += fee;
          sPeriod += periods===1 ? fee : Math.round(fee*suppFactor/1000)*1000;
        }

        perPersonSuppYear.push(sYear);
        perPersonSuppPeriod.push(periods===1 ? 0 : sPeriod);
      }

      const totalSuppPeriod = perPersonSuppPeriod.reduce((a,b)=>a+b,0);
      const totalSuppYear = perPersonSuppYear.reduce((a,b)=>a+b,0);
      const totalPeriod = periods===1 ? 0 : (baseAnnualMain + extra + totalSuppPeriod);
      const totalYear = baseAnnualMain + extra + totalSuppYear;
      const diff = totalYear - totalPeriod;

      sumMain += baseAnnualMain;
      sumExtra += extra;
      for (let i=0;i<persons.length;i++){
        sumSuppCols[i] += (periods===1 ? perPersonSuppYear[i] : perPersonSuppPeriod[i]);
      }
      if (periods!==1){ sumTotalPeriod += totalPeriod; sumDiff += diff; }
      sumTotalYear += totalYear;

      html += `<tr>
        <td class="p-2 border text-center">${year}</td>
        <td class="p-2 border text-center">${ageNow}</td>
        <td class="p-2 border text-right">${fmt(baseAnnualMain)}</td>
        <td class="p-2 border text-right">${fmt(extra)}</td>`;
      for (let i=0;i<persons.length;i++){
        const val = (periods===1 ? perPersonSuppYear[i] : perPersonSuppPeriod[i]);
        html += `<td class="p-2 border text-right">${fmt(val)}</td>`;
      }
      if (periods!==1) html += `<td class="p-2 border text-right">${fmt(totalPeriod)}</td>`;
      html += `<td class="p-2 border text-right">${fmt(totalYear)}</td>`;
      if (periods!==1) html += `<td class="p-2 border text-right">${diff?` + "`<span class=\"text-red-600 font-bold\">${fmt(diff)}</span>`" + r":'0'}</td>`;
      html += `</tr>`;
    }
    ...
    html += `<tr class="font-semibold bg-gray-50">
      ...
      <td class="p-2 border text-right">${fmt(sumExtra)}</td>`;
    for (let i=0;i<persons.length;i++){
      html += `<td class="p-2 border text-right">${fmt(sumSuppCols[i])}</td>`;
    }
    if (periods!==1) html += `<td class="p-2 border text-right">${fmt(sumTotalPeriod)}</td>`;
    html += `<td class="p-2 border text-right">${fmt(sumTotalYear)}</td>`;
    if (periods!==1) html += `<td class="p-2 border text-right">${fmt(sumDiff)}</td>`;
    html += `</tr></tbody></table>`;
