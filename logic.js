...
// Luôn hiển thị phí, validation chỉ để gắn lỗi input
const fees = appState.fees;

const fmt = (v) => formatDisplayCurrency(v);
const setEl = (id, val) => { const el = document.getElementById(id); if (el) el.textContent = fmt(val); };

setEl('summary-total', fees.total);
setEl('main-insured-main-fee', fees.baseMain);
setEl('main-insured-extra-fee', fees.extra);
setEl('summary-supp-fee', fees.totalSupp);
...
function renderSupplementaryProductsForPerson(customer, mainProductKey, mainPremium, container) {
    ...
    // Default program 'nang_cao' when empty
    if (sclSection) {
      const progSel = sclSection.querySelector('.health-scl-program');
      if (progSel && !progSel.value) progSel.value = 'nang_cao';
