  ...
  try {
    ...
    for (const p of persons){
      ...
      if (p.supplements && p.supplements.health_scl){
        ...
        const scope = scl.scope || 'domestic';
        ...
        const name = `Sức khoẻ Bùng Gia Lực – ${programName} (${scope==='abroad'?'Nước ngoài':'Trong nước'}${outpatient?', Ngoại trú':''}${dental?', Nha khoa':''})`;
        const stbh = (CONFIG?.supplementaryProducts?.find(p=>p.id==='health_scl')?.stbhByProgram?.[program]) || 0;
      ...
      if (p.supplements && p.supplements.bhn){
        const stbh = parseFormattedNumber((p.supplements.bhn.stbh)||0);
      ...
      if (p.supplements && p.supplements.accident){
        const stbh = parseFormattedNumber((p.supplements.accident.stbh)||0);
      ...
      if (p.supplements && p.supplements.hospital_support){
        const stbh = parseFormattedNumber((p.supplements.hospital_support.stbh)||0);
