...
function generateSummaryTable() {
  ...
  try {
    ...
    // Tổng tích lũy cho toàn bảng 1
    let totalPerPeriodAll = 0, totalAnnualEqAll = 0, totalBaseAll = 0, totalDiffAll = 0;

    // Dòng tổng theo từng người và các dòng sản phẩm
    personsList.forEach(person => {
      const colspan = showDiffColumn ? 6 : 5;
      summaryHtml += `<tr><td class="p-2 border font-semibold">${sanitizeHtml(person.name || '')}</td>`;
      summaryHtml += `<td class="p-2 border text-right">—</td>`;
      ...
      summaryHtml += `<td class="p-2 border text-right"></td>`;
      summaryHtml += `<td class="p-2 border text-right"></td>`;
      if (showDiffColumn) summaryHtml += `<td class="p-2 border text-right"></td>`;
      summaryHtml += `</tr>`;

      // Các dòng sản phẩm
      person.products.forEach(pr => {
        const diffVal = (pr.baseAnnual || 0) - (pr.annualEq || 0);
        totalPerPeriodAll += (pr.perPeriod || 0);
        totalAnnualEqAll += (pr.annualEq || 0);
        totalBaseAll += (pr.baseAnnual || 0);
        totalDiffAll += diffVal;
        ...
        if (showDiffColumn) {
          summaryHtml += `<td class="p-2 border text-right">${diffVal ? `<span class="text-red-600 font-semibold text-danger">${formatCurrency(diffVal)}</span>` : ''}</td>`;
        }
        summaryHtml += `</tr>`;
      });
    });

    // Dòng tổng cộng cuối bảng 1
    summaryHtml += `<tr><td class="p-2 border font-semibold">Tổng cộng</td>` +
                   `<td class="p-2 border"></td>` +
                   `<td class="p-2 border"></td>` +
                   `<td class="p-2 border text-right font-semibold">${formatCurrency(totalPerPeriodAll)}</td>` +
                   `<td class="p-2 border text-right font-semibold">${formatCurrency(totalAnnualEqAll)}</td>` +
                   `<td class="p-2 border text-right font-semibold">${formatCurrency(totalBaseAll)}</td>` +
                   (showDiffColumn ? `<td class="p-2 border text-right font-semibold">${totalDiffAll ? formatCurrency(totalDiffAll) : '0'}</td>` : '') +
                   `</tr>`;
    ...
    // ==== Accumulators for Part 2 totals (ensure defined in function scope)
    let sumMainBase=0, sumExtra=0, sumMainSupp=0;
    let sumSuppCols=[]; let sumTotalAnnualEq=0, sumBaselineTotal=0, sumTotalDiff=0;
    ...
    if (periods !== 1) { tableHtml += `<th class=\"p-2 border\">Tổng (theo kỳ)</th><th class=\"p-2 border\">Nếu đóng theo năm</th><th class=\"p-2 border\">Chênh lệch</th>`; } else { tableHtml += `<th class=\"p-2 border\">Nếu đóng theo năm</th>`; }
    ...
    // Accumulators for totals in Part 2
    let sumMainBase=0, sumExtra=0, sumMainSupp=0, sumSuppCols=new Array(tablePersons.length).fill(0), sumTotalAnnualEq=0, sumBaselineTotal=0, sumTotalDiff=0;
...
for (let i = 0; (mainPersonInfo.age + i) <= targetAge; i++) {
      ...
      const baselineTotal = baseAnnualTotal; // baseline tổng theo năm đã tính ở trên
      const totalDiff = baselineTotal - totalAnnualEq;
      // Cộng dồn totals
      sumMainBase += annualEqMainBase; sumExtra += annualEqMainExtra; if (hasSuppMain) sumMainSupp += annualEqMainSupp;
      annualEqSupps.forEach((v,idx)=>{ sumSuppCols[idx] += (v||0); });
      if (periods !== 1) { sumTotalAnnualEq += totalAnnualEq; sumBaselineTotal += baselineTotal; sumTotalDiff += totalDiff; }
      // Render row
      tableHtml += `<tr>`;
      ...
      if (periods !== 1) { tableHtml += `<td class="p-2 border text-right font-semibold">${formatCurrency(totalAnnualEq)}</td><td class="p-2 border text-right font-semibold">${formatCurrency(baselineTotal)}</td><td class="p-2 border text-right">${totalDiff ? `<span class=\"text-red-600 font-semibold text-danger\">${formatCurrency(totalDiff)}</span>` : ""}</td>`; } else { tableHtml += `<td class="p-2 border text-right font-semibold">${formatCurrency(baselineTotal)}</td>`; }
      tableHtml += `</tr>`;
    }
    tableHtml += `</tbody>`;
    // Dòng tổng cộng Part 2
    tableHtml += `<tfoot><tr><td class="p-2 border font-semibold">Tổng cộng</td><td class="p-2 border"></td>` +
      `<td class="p-2 border text-right font-semibold">${formatCurrency(sumMainBase)}</td>` +
      (hasExtraMain ? `<td class="p-2 border text-right font-semibold">${formatCurrency(sumExtra)}</td>` : '') +
      (hasSuppMain ? `<td class="p-2 border text-right font-semibold">${formatCurrency(sumMainSupp)}</td>` : '') +
      tablePersons.map((_,idx)=>`<td class="p-2 border text-right font-semibold">${formatCurrency(sumSuppCols[idx]||0)}</td>`).join('') +
      (periods !== 1 ? `<td class="p-2 border text-right font-semibold">${formatCurrency(sumTotalAnnualEq)}</td><td class="p-2 border text-right font-semibold">${formatCurrency(sumBaselineTotal)}</td><td class="p-2 border text-right font-semibold">${sumTotalDiff ? formatCurrency(sumTotalDiff) : '0'}</td>` : `<td class="p-2 border text-right font-semibold">${formatCurrency(sumMainBase + sumExtra + sumMainSupp + sumSuppCols.reduce((a,b)=>a+b,0))}</td>`) +
      `</tr></tfoot>`;
tableHtml += `</table></div>`;
    ...
    function buildSummaryExportDocument(){
      const modal = document.querySelector('#summary-modal, .summary-modal, .modal-summary');
      const host = modal || document.querySelector('#summary-content-container') || document.body;
      const clone = host.cloneNode(true);
      clone.querySelectorAll('#export-html-btn,#export-pdf-btn,.export-html-btn,.export-pdf-btn').forEach(el=>el.remove());
      ...
      const baseCss = `
        ...
        .title{font-size:20px;font-weight:700;color:#b30000;margin-bottom:12px}
      ...
      const ymd = `${y}-${m}-${d}`;
      const html = `<!doctype html><html lang="vi"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Bảng tóm tắt quyền lợi & phí - ${ymd}</title><style>${baseCss}${printCss}</style></head><body><div class="title">Bảng Tóm Tắt Quyền Lợi và Phí</div>${clone.innerHTML}</body></html>`;
      return { html, ymd };
    }
    const htmlBtn = document.getElementById('export-html-btn');
    if (htmlBtn && !htmlBtn.__bind) {
      htmlBtn.__bind = true;
      htmlBtn.addEventListener('click', ()=>{
        const {html, ymd} = buildSummaryExportDocument();
        const blob = new Blob([html], {type:'text/html;charset=utf-8'});
        const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href=url; a.download=`bang-tom-tat-quyen-loi-phi_${ymd}.html`; document.body.appendChild(a); a.click(); setTimeout(()=>{URL.revokeObjectURL(url); a.remove();},0);
      });
    }
    const pdfBtn = document.getElementById('export-pdf-btn');
    if (pdfBtn && !pdfBtn.__bind) {
      pdfBtn.__bind = true;
      pdfBtn.addEventListener('click', ()=>{
        const {html} = buildSummaryExportDocument();
