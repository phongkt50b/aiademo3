  ...
  (function() {
    function customGenerateSummary() {
      ...
      try {
        ...
        summaryHtml += `</tbody></table></div>`;
            // === Part 2: Bảng phí ===
            // We rebuild the fee table to dynamically include columns per category and per person based on non-zero premiums and payment frequency.
            let tableHtml = `<div class="mb-4"><div class="text-lg font-semibold mb-2">Phần 2 · Bảng phí (${freq === 'year' ? 'năm' : (freq === 'half' ? 'nửa năm' : 'theo quý')})</div>`;
            // Determine MDP3 configuration
            let mdp3SelectedId = null;
            let mdp3Fee = 0;
            let mdp3OtherName = '';
            try {
            ...
            // Prepare arrays to track data for each year and column visibility
            const yearRows = [];
            let hasExtra = false;
            let hasSuppMain = false;
            const hasSuppEach = suppPersonsInfo.map(() => false);
            let hasMdp3Other = false;
            // Loop through each contract year to compute per-period values
            for (let i = 0; (mainInfo.age + i) <= targetAge; i++) {
              ...
              // Supplementary premium for main person in this year
              let suppMainYear = 0;
              ...
              // Supplementary premiums for each supplementary person in this year
              const suppEachYear = [];
              suppPersonsInfo.forEach(pInfo => {
              ...
              // Allocate MDP3 premium to appropriate person
              let mdp3Other = 0;
              if (mdp3Fee > 0) {
                if (mdp3SelectedId === mainContainer.id) {
                  suppMainYear += mdp3Fee;
                } else if (mdp3SelectedId === 'other') {
                  mdp3Other = mdp3Fee;
                } else {
                  const idx = suppPersonsInfo.findIndex(p => p.container?.id === mdp3SelectedId);
                  if (idx >= 0) suppEachYear[idx] += mdp3Fee;
                  else {
                    // if mdp3SelectedId is null or unknown, default assign to main
                    suppMainYear += mdp3Fee;
                  }
                }
              }
              // Determine original annual sum for skipping zero rows
              const annualSumOriginal = mainThisYear + extraThisYear + suppMainYear + suppEachYear.reduce((a,b) => a+b,0) + mdp3Other;
              if (annualSumOriginal <= 0) continue;
              // Calculate per-period values for each category
              let perMainThis = 0;
              let perExtraThis = 0;
              let perSuppMain = 0;
              const perSuppEach = [];
              let perMdp3Other = 0;
              if (periods === 1) {
                perMainThis = mainThisYear;
                perExtraThis = extraThisYear;
                perSuppMain = suppMainYear;
                suppEachYear.forEach(v => perSuppEach.push(v));
                perMdp3Other = mdp3Other;
              } else {
                perMainThis = mainThisYear > 0 ? Math.floor(mainThisYear / periods / 1000) * 1000 : 0;
                perExtraThis = extraThisYear > 0 ? Math.floor(extraThisYear / periods / 1000) * 1000 : 0;
                perSuppMain = suppMainYear > 0 ? Math.round((suppMainYear / 1000 * factor / periods)) * 1000 : 0;
                suppEachYear.forEach(v => {
                  const perVal = v > 0 ? Math.round((v / 1000 * factor / periods)) * 1000 : 0;
                  perSuppEach.push(perVal);
                });
                perMdp3Other = mdp3Other > 0 ? Math.round((mdp3Other / 1000 * factor / periods)) * 1000 : 0;
              }
              // Compute totals and difference
              const perTotal = perMainThis + perExtraThis + perSuppMain + perSuppEach.reduce((a,b) => a+b,0) + perMdp3Other;
              const convertedAnnual = perTotal * periods;
              const diff = convertedAnnual - annualSumOriginal;
              // Update visibility flags
              if (perExtraThis > 0) hasExtra = true;
              if (perSuppMain > 0) hasSuppMain = true;
              perSuppEach.forEach((v, idx) => {
                if (v > 0) hasSuppEach[idx] = true;
              });
              if (perMdp3Other > 0) hasMdp3Other = true;
              // Store row data
              yearRows.push({
                contractYear,
                ageThisYear,
            ...
            tableHtml += `<th class="p-2 border">Phí sản phẩm chính</th>`;
            if (hasExtra) {
              tableHtml += `<th class="p-2 border">Phí đóng thêm</th>`;
            }
            if (hasSuppMain) {
              tableHtml += `<th class="p-2 border">Phí sản phẩm bổ sung<br>(${sanitizeHtml(mainInfo.name)})</th>`;
            }
            suppPersonsInfo.forEach((p, idx) => {
              if (hasSuppEach[idx]) {
                tableHtml += `<th class="p-2 border">Phí sản phẩm bổ sung<br>(${sanitizeHtml(p.name)})</th>`;
              }
            });
            if (hasMdp3Other) {
              const displayName = sanitizeHtml(mdp3OtherName || 'Người khác');
              tableHtml += `<th class="p-2 border">Phí sản phẩm bổ sung<br>(${displayName})</th>`;
            }
            tableHtml += `<th class="p-2 border">Tổng cộng</th>`;
            if (freq !== 'year') {
              tableHtml += `<th class="p-2 border">Chênh lệch so với năm</th>`;
            ...
            yearRows.forEach(row => {
              tableHtml += `<tr>`;
              tableHtml += `<td class="p-2 border text-center">${row.contractYear}</td>`;
              tableHtml += `<td class="p-2 border text-center">${row.ageThisYear}</td>`;
              tableHtml += `<td class="p-2 border text-right">${formatCurrency(row.perMainThis)}</td>`;
              if (hasExtra) {
                tableHtml += `<td class="p-2 border text-right">${formatCurrency(row.perExtraThis)}</td>`;
              }
              if (hasSuppMain) {
                tableHtml += `<td class="p-2 border text-right">${formatCurrency(row.perSuppMain)}</td>`;
              }
              suppPersonsInfo.forEach((p, idx) => {
                if (hasSuppEach[idx]) {
                  const val = row.perSuppEach[idx] || 0;
                  tableHtml += `<td class="p-2 border text-right">${formatCurrency(val)}</td>`;
                }
              });
              if (hasMdp3Other) {
                tableHtml += `<td class="p-2 border text-right">${formatCurrency(row.perMdp3Other)}</td>`;
              }
              tableHtml += `<td class="p-2 border text-right">${formatCurrency(row.perTotal)}</td>`;
              if (freq !== 'year') {
                tableHtml += `<td class="p-2 border text-right">${formatCurrency(row.diff)}</td>`;
              }
              tableHtml += `</tr>`;
            });
            tableHtml += `</tbody></table></div>`;
