...
const MAX_ENTRY_AGE = {
  PUL_TRON_DOI: 70, PUL_15NAM: 70, PUL_5NAM: 70, KHOE_BINH_AN: 70, VUNG_TUONG_LAI: 70,
...
// Sử dụng ngày hiện tại thay vì một ngày cố định để tính tuổi linh động
...
function updatePaymentFrequencyOptions(baseMainAnnual) {
  ...
  const allowHalf = baseMainAnnual >= 7000000;
  const allowQuarter = baseMainAnnual >= 8000000;
...
function calculateBhnPremium(customer, container, ageOverride = null) {
  ...
  if (stbh < 200000000 || stbh > MAX_STBH.bhn) {
...
function calculateAccidentPremium(customer, container, ageOverride = null) {
  ...
  if (stbh < 10000000 || stbh > MAX_STBH.accident) {
...
function calculateHospitalSupportPremium(customer, mainPremium, container, totalHospitalSupportStbh = 0, ageOverride = null) {
  ...
  const maxSupportByAge = ageToUse >= 18 ? 1000000 : 300000;
...
function generateSummaryTable() {
  ...
  try {
    ...
    personsList.forEach(p => {
      ...
      let totalAnnualEq = 0;
      let totalBaseAnnual = 0;
      let totalDiff = 0;
      p.products.forEach(pr => {
        totalPerPeriod += pr.perPeriod;
        totalAnnualEq += pr.annualEq;
        totalBaseAnnual += pr.baseAnnual;
        totalDiff += (pr.baseAnnual - pr.annualEq);
      });
      p.totalPerPeriod = totalPerPeriod;
      p.totalAnnualEq = totalAnnualEq;
      p.totalBaseAnnual = totalBaseAnnual;
      p.totalDiff = totalDiff;
    });

    // ===== XÂY DỰNG PHẦN 1: TÓM TẮT SẢN PHẨM =====
    // Chuẩn bị HTML cho bảng tóm tắt. Bảng gồm các cột:
    // Sản phẩm | STBH | Số năm đóng phí | Phí đóng (theo kỳ) | Phí đóng quy năm | Phí đóng theo năm | Chênh lệch
    ...
    // Cột phí đóng theo kỳ (năm, nửa năm, quý)
    const periodLabel = (periods === 1) ? 'năm' : (periods === 2 ? 'nửa năm' : 'quý');
    summaryHtml += `<th class="p-2 border">Phí đóng (${periodLabel})</th>`;
    // Cột phí đóng quy đổi về năm (quy năm)
    summaryHtml += `<th class="p-2 border">Phí đóng quy năm</th>`;
    // Cột phí đóng theo năm (baseline annual)
    summaryHtml += `<th class="p-2 border">Phí đóng theo năm</th>`;
    // Cột chênh lệch (hiển thị khi không phải trả theo năm)
    if (showDiffColumn) summaryHtml += `<th class="p-2 border">Chênh lệch</th>`;
    summaryHtml += `</tr></thead><tbody>`;

    // Dòng tổng cho từng người
    personsList.forEach(person => {
      // Tổng hợp dòng đầu tiên cho người này
      // Cột đầu hiển thị tên người, sau đó các cột số liệu. Số cột số liệu = 4 (kỳ, quy năm, theo năm, chênh lệch) hoặc 3 nếu không hiển thị chênh lệch
      summaryHtml += `<tr>`;
      summaryHtml += `<td class="p-2 border font-semibold">${sanitizeHtml(person.name || '')}</td>`;
      // STBH cột trống ở dòng tổng (kí tự en dash)
      summaryHtml += `<td class="p-2 border text-right font-semibold">—</td>`;
      // Số năm đóng phí cột trống
      summaryHtml += `<td class="p-2 border text-center font-semibold">—</td>`;
      // Phí theo kỳ (tổng perPeriod)
      summaryHtml += `<td class="p-2 border text-right font-semibold">${formatCurrency(person.totalPerPeriod)}</td>`;
      // Phí quy năm (tổng annualEq)
      summaryHtml += `<td class="p-2 border text-right font-semibold">${formatCurrency(person.totalAnnualEq)}</td>`;
      // Phí theo năm (tổng baseAnnual)
      summaryHtml += `<td class="p-2 border text-right font-semibold">${formatCurrency(person.totalBaseAnnual)}</td>`;
      // Chênh lệch (tổng baseAnnual - totalAnnualEq)
      if (showDiffColumn) {
        const diffVal = person.totalBaseAnnual - person.totalAnnualEq;
        summaryHtml += `<td class="p-2 border text-right font-semibold" style="${(periods !== 1 && diffVal !== 0) ? 'color:#d00;font-weight:700;' : ''}">${diffVal ? formatCurrency(diffVal) : '0'}</td>`;
      }
      summaryHtml += `</tr>`;
      ...
      person.products.forEach(pr => {
        ...
        // Phí theo năm (baseline)
        summaryHtml += `<td class="p-2 border text-right">${formatCurrency(pr.baseAnnual)}</td>`;
        // Chênh lệch: baseAnnual - annualEq
        if (showDiffColumn) {
          const prDiff = pr.baseAnnual - pr.annualEq;
          summaryHtml += `<td class="p-2 border text-right" style="${(periods !== 1 && prDiff !== 0) ? 'color:#d00;font-weight:700;' : ''}">${prDiff ? formatCurrency(prDiff) : ''}</td>`;
        }
        summaryHtml += `</tr>`;
      });
    });
    ...
    // ===== PHẦN 2: BẢNG PHÍ =====
    // Chuẩn bị bảng phí với các cột tách biệt cho NĐBH chính (sản phẩm chính, phí đóng thêm, phí sản phẩm bổ sung)
    // và cột riêng cho từng NĐBH bổ sung, bao gồm cả "Người khác" nếu có MDP3 áp dụng.
    let tableHtml = `<div class="mb-4"><div class="text-lg font-semibold mb-2">Phần 2 · Bảng phí</div>`;
    // Xác định các cột cần hiển thị cho NĐBH chính
    const hasExtraMain = extraPremium > 0;
    // Kiểm tra xem NĐBH chính có sản phẩm bổ sung hoặc MDP3 hay không
    let hasSuppMain = false;
    {
      const mainSuppContainer = document.querySelector('#main-supp-container .supplementary-products-container');
      if (mainSuppContainer) {
        // Kiểm tra mỗi sản phẩm bổ sung xem phí năm đầu có > 0
        if (mainSuppContainer.querySelector('.health-scl-checkbox')?.checked &&
            calculateHealthSclPremium(mainPersonInfo, mainSuppContainer, mainPersonInfo.age) > 0) {
          hasSuppMain = true;
        }
        if (!hasSuppMain && mainSuppContainer.querySelector('.bhn-checkbox')?.checked &&
            calculateBhnPremium(mainPersonInfo, mainSuppContainer, mainPersonInfo.age) > 0) {
          hasSuppMain = true;
        }
        if (!hasSuppMain && mainSuppContainer.querySelector('.accident-checkbox')?.checked &&
            calculateAccidentPremium(mainPersonInfo, mainSuppContainer, mainPersonInfo.age) > 0) {
          hasSuppMain = true;
        }
        if (!hasSuppMain && mainSuppContainer.querySelector('.hospital-support-checkbox')?.checked &&
            calculateHospitalSupportPremium(mainPersonInfo, initialBaseMainPremium, mainSuppContainer, 0, mainPersonInfo.age) > 0) {
          hasSuppMain = true;
        }
      }
      // Kiểm tra MDP3 cho NĐBH chính
      if (mdp3Data && mdp3Data.personId === 'main-person-container' && (mdp3Data.row?.baseAnnual || 0) > 0) {
        hasSuppMain = true;
      }
    }
    // Xây dựng danh sách các NĐBH bổ sung cho bảng phí
    const tablePersons = [];
    suppPersons.forEach(p => {
      // Lưu trữ toàn bộ personInfo để tính phí cho từng năm chính xác (bao gồm giới tính, riskGroup, mainProduct, v.v.)
      tablePersons.push({ person: p, id: p.container?.id || '', name: p.name || '', isOther: false });
    });
    // Nếu MDP3 áp dụng cho người khác, thêm người khác vào danh sách
    if (mdp3Data && mdp3Data.personId === 'other') {
      tablePersons.push({ person: null, id: 'other', name: mdp3Data.personName || 'Người khác', isOther: true });
    }
    // Phân tích thông tin MDP3 nếu có để sử dụng trong tính phí từng năm
    let mdp3PersonId = null, mdp3Years = 0, mdp3AnnualBase = 0, mdp3PerPeriod = 0, mdp3AnnualEq = 0;
    if (mdp3Data && mdp3Data.row) {
      mdp3PersonId = mdp3Data.personId;
      mdp3Years = mdp3Data.row.years || 0;
      mdp3AnnualBase = mdp3Data.row.baseAnnual || 0;
      if (periods === 1) {
        mdp3PerPeriod = mdp3AnnualBase;
      } else {
        mdp3PerPeriod = Math.round((mdp3AnnualBase / 1000 * factor / periods)) * 1000;
      }
      mdp3AnnualEq = mdp3PerPeriod * periods;
    }
    ...
    // Cột cho NĐBH chính: luôn có cột phí sản phẩm chính
    tableHtml += `<th class="p-2 border">Phí sản phẩm chính<br>(${sanitizeHtml(mainPersonInfo.name)})</th>`;
    // Cột phí đóng thêm nếu có
    if (hasExtraMain) {
      tableHtml += `<th class="p-2 border">Phí đóng thêm<br>(${sanitizeHtml(mainPersonInfo.name)})</th>`;
    }
    // Cột phí sản phẩm bổ sung (bao gồm MDP3) nếu có
    if (hasSuppMain) {
      tableHtml += `<th class="p-2 border">Phí sản phẩm bổ sung<br>(${sanitizeHtml(mainPersonInfo.name)})</th>`;
    }
    // Cột cho từng NĐBH bổ sung
    tablePersons.forEach(p => {
    ...
    for (let i = 0; (mainPersonInfo.age + i) <= targetAge; i++) {
      ...
      // Xác định phí sản phẩm chính và phí đóng thêm trong năm này
      const mainBaseThisYear = (contractYear <= paymentTerm) ? initialBaseMainPremium : 0;
      const extraThisYear = (contractYear <= paymentTerm) ? extraPremium : 0;
      // Tính phí sản phẩm bổ sung cho NĐBH chính (chưa bao gồm MDP3)
      ...
      // Tính phí sản phẩm bổ sung cho từng NĐBH bổ sung (chưa bao gồm MDP3)
      const suppAnnuals = tablePersons.map(tp => {
        // Người khác không có sản phẩm bổ sung
        if (tp.isOther) return 0;
        const ageNow = tp.person.age + i;
        const suppCont = tp.person.container?.querySelector('.supplementary-products-container');
        let suppAnnual = 0;
        if (suppCont) {
          // Sử dụng toàn bộ thông tin person (bao gồm gender, riskGroup, mainProduct...) để tính phí
          suppAnnual += calculateHealthSclPremium({ ...tp.person, age: ageNow }, suppCont, ageNow);
          suppAnnual += calculateBhnPremium({ ...tp.person, age: ageNow }, suppCont, ageNow);
          suppAnnual += calculateAccidentPremium({ ...tp.person, age: ageNow }, suppCont, ageNow);
          suppAnnual += calculateHospitalSupportPremium({ ...tp.person, age: ageNow }, initialBaseMainPremium, suppCont, 0, ageNow);
      ...
      const baseAnnualTotalMain = mainBaseThisYear + extraThisYear + suppAnnualMain;
      const baseAnnualTotalSupps = suppAnnuals.reduce((a,b) => a + b, 0);
      const baseAnnualTotalAllWithoutMDP3 = baseAnnualTotalMain + baseAnnualTotalSupps;
      // Kiểm tra MDP3 áp dụng trong năm này để cộng thêm vào phí sản phẩm bổ sung của người được áp dụng
      let mainMdp3Annual = 0;
      const suppMdp3Annuals = new Array(tablePersons.length).fill(0);
      if (mdp3PersonId && i < mdp3Years) {
        if (mdp3PersonId === 'main-person-container') {
          mainMdp3Annual = mdp3AnnualBase;
        } else {
          const idx = tablePersons.findIndex(p => p.id === mdp3PersonId);
          if (idx !== -1) {
            suppMdp3Annuals[idx] = mdp3AnnualBase;
          }
        }
      }
      const baseAnnualTotal = baseAnnualTotalAllWithoutMDP3 + mainMdp3Annual + suppMdp3Annuals.reduce((a,b) => a + b, 0);
      if (baseAnnualTotal === 0) continue;
      // Tính phí theo kỳ cho NĐBH chính
      // Phí sản phẩm chính (baseMain)
      const annualEqMainBase = mainBaseThisYear;
      // Phí đóng thêm (extra)
      const annualEqMainExtra = extraThisYear;
      // Phí sản phẩm bổ sung (bao gồm MDP3) cho NĐBH chính
      const baseMainSuppAnnual = suppAnnualMain + mainMdp3Annual;
      let perPeriodMainSupp, annualEqMainSupp, diffMainSupp;
      if (periods === 1) {
        perPeriodMainSupp = baseMainSuppAnnual;
        annualEqMainSupp = baseMainSuppAnnual;
        diffMainSupp = 0;
      } else {
        perPeriodMainSupp = Math.round((baseMainSuppAnnual / 1000 * factor / periods)) * 1000;
        annualEqMainSupp = perPeriodMainSupp * periods;
        diffMainSupp = annualEqMainSupp - baseMainSuppAnnual;
      }
      // Tính phí cho từng NĐBH bổ sung
      const annualEqSupps = [];
      const diffSupps = [];
      suppAnnuals.forEach((baseAnnual, idx) => {
        const totalBase = baseAnnual + suppMdp3Annuals[idx];
        if (periods === 1) {
          annualEqSupps[idx] = totalBase;
          diffSupps[idx] = 0;
        } else {
          const perP = Math.round((totalBase / 1000 * factor / periods)) * 1000;
          const annEq = perP * periods;
          annualEqSupps[idx] = annEq;
          diffSupps[idx] = annEq - totalBase;
        }
      });
      // Tổng cộng và chênh lệch
      const totalAnnualEq = annualEqMainBase + annualEqMainExtra + annualEqMainSupp + annualEqSupps.reduce((a,b) => a + b, 0);
      const totalDiff = diffMainSupp + diffSupps.reduce((a,b) => a + b, 0);
      // Render row
      tableHtml += `<tr>`;
      tableHtml += `<td class="p-2 border text-center">${contractYear}</td>`;
      tableHtml += `<td class="p-2 border text-center">${currentAgeMain}</td>`;
      // Cột phí sản phẩm chính cho NĐBH chính (ẩn nếu bằng 0)
      tableHtml += `<td class="p-2 border text-right">${annualEqMainBase ? formatCurrency(annualEqMainBase) : ''}</td>`;
      // Cột phí đóng thêm cho NĐBH chính (ẩn nếu không cần hiển thị)
      if (hasExtraMain) {
        tableHtml += `<td class="p-2 border text-right">${annualEqMainExtra ? formatCurrency(annualEqMainExtra) : ''}</td>`;
      }
      // Cột phí bổ sung cho NĐBH chính (ẩn nếu không cần hiển thị)
      if (hasSuppMain) {
        tableHtml += `<td class="p-2 border text-right">${annualEqMainSupp ? formatCurrency(annualEqMainSupp) : ''}</td>`;
      }
      // Cột phí cho từng NĐBH bổ sung (bao gồm MDP3 nếu có)
      annualEqSupps.forEach(val => {
    ...
    try {
      // Post-process Part 1 table to ensure baseline and difference columns are correct.
      if (window.__fixPart1Table) {
        window.__fixPart1Table(container, periods, factor);
      }
    } catch (err) {
      console.error('fixPart1Table error', err);
    }
...
(() => {
  ...
  (function() {
      ...
      try {
        ...
        function computeSupplementProducts(personInfo, containerEl) {
          ...
          if (sclSec && sclSec.querySelector('.health-scl-checkbox')?.checked) {
            const program = sclSec.querySelector('.health-scl-program')?.value || '';
          const programLabel = { co_ban: 'Cơ bản', nang_cao: 'Nâng cao', toan_dien: 'Toàn diện', hoan_hao: 'Hoàn hảo' }[program] || '';
          // Determine selected scope and optional benefits for Sức khỏe Bùng Gia Lực
          let optionsLabel = '';
          try {
            const scopeSelect = sclSec.querySelector('.health-scl-scope');
            const scopeVal = scopeSelect?.value || '';
            const labels = [];
            if (scopeVal === 'main_vn' || scopeVal === 'vn') labels.push('Trong nước');
            else if (scopeVal === 'main_global' || scopeVal === 'global') labels.push('Nước ngoài');
            const outpatientCheckbox = sclSec.querySelector('.health-scl-outpatient');
            if (outpatientCheckbox?.checked) labels.push('Ngoại trú');
            const dentalCheckbox = sclSec.querySelector('.health-scl-dental');
            if (dentalCheckbox?.checked) labels.push('Nha khoa');
            ...
            if (annual > 0) {
              list.push({
                name: `Sức khoẻ Bùng Gia Lực${programLabel ? ' - ' + programLabel : ''}${optionsLabel}`,
                stbh: stbh,
                years: years,
                annual: annual,
        ...
        persons.forEach(p => {
          // Remove products with zero annual premium
          p.products = p.products.filter(pr => pr.annual > 0);
          // Baseline (annual) total for this person
          p.totalAnnual = p.products.reduce((sum, pr) => sum + pr.annual, 0);
          // Per-period total by summing each product's perPeriod
          p.totalPerPeriod = p.products.reduce((sum, pr) => sum + pr.perPeriod, 0);
          // Annualised total derived from per-period total
          p.totalAnnualFromPeriod = p.totalPerPeriod * periods;
          // Difference: baseline minus annualised (negative when paying per period is more expensive)
          p.totalDiff = p.totalAnnual - p.totalAnnualFromPeriod;
        });
        const filteredPersons = persons.filter(p => p.products.length > 0);
        // Build Part 1 HTML with custom columns
        // Build Part 1 summary table. Column structure differs depending on payment frequency:
        // - If frequency is not year, include: per-period premium, annualised premium (per-period × periods),
        //   baseline annual premium, and difference (baseline minus annualised).
        // - If frequency is year, only include the baseline annual premium column.
        ...
        summaryHtml += `<th class="p-2 border">Số năm đóng phí</th>`;
        if (freq !== 'year') {
          const periodLabel = (freq === 'half') ? 'nửa năm' : (freq === 'quarter' ? 'theo quý' : 'năm');
          summaryHtml += `<th class="p-2 border">Phí đóng (${periodLabel})</th>`;
          summaryHtml += `<th class="p-2 border">Phí đóng (quy năm)</th>`;
          summaryHtml += `<th class="p-2 border">Phí đóng theo năm</th>`;
          summaryHtml += `<th class="p-2 border">Chênh lệch</th>`;
        } else {
          summaryHtml += `<th class="p-2 border">Phí đóng theo năm</th>`;
        }
        summaryHtml += `</tr></thead><tbody>`;
        filteredPersons.forEach(person => {
          ...
          const totalPer = person.totalPerPeriod;
          const totalAnnualized = person.totalAnnualFromPeriod;
          const totalBaseline = person.totalAnnual;
          const diffVal = totalBaseline - totalAnnualized;
          summaryHtml += `<tr>`;
          summaryHtml += `<td class="p-2 border font-semibold">${sanitizeHtml(person.name || '')}</td>`;
          ...
          if (freq !== 'year') {
            summaryHtml += `<td class="p-2 border text-right font-semibold">${formatCurrency(totalPer)}</td>`;
            summaryHtml += `<td class="p-2 border text-right font-semibold">${formatCurrency(totalAnnualized)}</td>`;
            summaryHtml += `<td class="p-2 border text-right font-semibold">${formatCurrency(totalBaseline)}</td>`;
            summaryHtml += `<td class="p-2 border text-right font-semibold" style="${(diffVal !== 0) ? 'color:#d00;font-weight:700;' : ''}">${diffVal ? formatCurrency(diffVal) : '0'}</td>`;
          } else {
            summaryHtml += `<td class="p-2 border text-right font-semibold">${formatCurrency(totalBaseline)}</td>`;
          }
          summaryHtml += `</tr>`;
          ...
          person.products.forEach(prod => {
            const perP = prod.perPeriod;
            const annualized = prod.annualFromPeriod;
            const baseline = prod.annual;
            const diff = baseline - annualized;
            summaryHtml += `<tr>`;
            summaryHtml += `<td class="p-2 border"></td>`;
            ...
            if (freq !== 'year') {
              summaryHtml += `<td class="p-2 border text-right">${formatCurrency(perP)}</td>`;
              summaryHtml += `<td class="p-2 border text-right">${formatCurrency(annualized)}</td>`;
              summaryHtml += `<td class="p-2 border text-right">${formatCurrency(baseline)}</td>`;
              summaryHtml += `<td class="p-2 border text-right" style="${(diff !== 0) ? 'color:#d00;font-weight:700;' : ''}">${diff ? formatCurrency(diff) : ''}</td>`;
            } else {
              summaryHtml += `<td class="p-2 border text-right">${formatCurrency(baseline)}</td>`;
            }
            summaryHtml += `</tr>`;
        ...
        tableHtml += `<th class="p-2 border">Phí bổ sung<br>(${sanitizeHtml(mainPerson.name)})</th>`;
        suppPersonsInfo.forEach(p => {
          tableHtml += `<th class="p-2 border">Phí bổ sung<br>(${sanitizeHtml(p.name)})</th>`;
        });
        // Append total and difference columns. Show per-period total and difference
        // only when frequency is not annual. The baseline total column is always shown.
        if (freq !== 'year') {
          tableHtml += `<th class="p-2 border">Tổng (theo kỳ)</th>`;
        }
        tableHtml += `<th class="p-2 border">Nếu đóng theo năm</th>`;
        if (freq !== 'year') {
          tableHtml += `<th class="p-2 border">Chênh lệch</th>`;
        }
        tableHtml += `</tr></thead><tbody>`;
        // Compute rows per contract year and accumulate totals
        let sumPerTotal = 0;
        let sumBaseline = 0;
        let sumDiff = 0;
        for (let i = 0; (mainInfo.age + i) <= targetAge; i++) {
          ...
          const contractYear = i + 1;
          // Baseline main and extra premiums for this year
          const baselineMain = (contractYear <= paymentTerm) ? mainAnnual : 0;
          const baselineExtra = (contractYear <= paymentTerm) ? extraAnnual : 0;
          // Supplementary annual premium for main person this year
          let suppMainYear = 0;
          ...
          // Supplementary annual premium for each supplementary person this year
          const suppEach = [];
          suppPersonsInfo.forEach(pInfo => {
            ...
            let s = 0;
            if (cont) {
              s += calculateHealthSclPremium({ ...pInfo, age: pAge }, cont, pAge);
              s += calculateBhnPremium({ ...pInfo, age: pAge }, cont, pAge);
              s += calculateAccidentPremium({ ...pInfo, age: pAge }, cont, pAge);
              s += calculateHospitalSupportPremium({ ...pInfo, age: pAge }, mainAnnual, cont, 0, pAge);
            }
            suppEach.push(s);
          });
          // MDP3 contribution for this year (add to appropriate person's supplementary)
          let mdp3Fee = 0;
          let mdp3Sel = null;
          try {
            if (window.MDP3) {
          ...
          let mdp3Other = 0;
          if (mdp3Fee > 0) {
            if (mdp3Sel === mainContainer.id || !mdp3Sel) {
              suppMainYear += mdp3Fee;
            } else if (mdp3Sel === 'other') {
              mdp3Other = mdp3Fee;
            } else {
              const idx = suppPersonsInfo.findIndex(p => p.container?.id === mdp3Sel);
              if (idx >= 0) suppEach[idx] += mdp3Fee;
            }
          }
          // Baseline total for this row (per year)
          const baselineTotalThisYear = baselineMain + baselineExtra + suppMainYear + suppEach.reduce((a,b) => a+b,0) + mdp3Other;
          if (baselineTotalThisYear <= 0) continue;
          // Compute per period totals
          const perMainExtra = (periods === 1) ? (baselineMain + baselineExtra) : Math.floor((baselineMain + baselineExtra) / periods / 1000) * 1000;
          const baselineSupp = suppMainYear + suppEach.reduce((a,b) => a+b,0) + mdp3Other;
          const perSupp = (periods === 1) ? baselineSupp : Math.round((baselineSupp / 1000 * factor / periods)) * 1000;
          const perTotal = perMainExtra + perSupp;
          // Annualised total converted from per-period
          const convertedTotal = (periods === 1) ? baselineTotalThisYear : perTotal * periods;
          // Difference: converted minus baseline
          const diffVal2 = convertedTotal - baselineTotalThisYear;
          // Build row
          tableHtml += `<tr>`;
          tableHtml += `<td class="p-2 border text-center">${contractYear}</td>`;
          tableHtml += `<td class="p-2 border text-center">${ageThisYear}</td>`;
          tableHtml += `<td class="p-2 border text-right">${formatCurrency(baselineMain)}</td>`;
          tableHtml += `<td class="p-2 border text-right">${formatCurrency(baselineExtra)}</td>`;
          tableHtml += `<td class="p-2 border text-right">${formatCurrency(suppMainYear)}</td>`;
          suppEach.forEach(val => {
            tableHtml += `<td class="p-2 border text-right">${formatCurrency(val)}</td>`;
          });
          if (freq !== 'year') {
            tableHtml += `<td class="p-2 border text-right">${formatCurrency(perTotal)}</td>`;
            tableHtml += `<td class="p-2 border text-right">${formatCurrency(baselineTotalThisYear)}</td>`;
            tableHtml += `<td class="p-2 border text-right" style="${diffVal2 !== 0 ? 'color:#d00;font-weight:700;' : ''}">${formatCurrency(diffVal2)}</td>`;
          } else {
            // When paying annually, only show baseline total
            tableHtml += `<td class="p-2 border text-right">${formatCurrency(baselineTotalThisYear)}</td>`;
          }
          tableHtml += `</tr>`;
          // Accumulate sums (using per period total, baseline and diff)
          if (!isNaN(perTotal)) sumPerTotal += perTotal;
          sumBaseline += baselineTotalThisYear;
          sumDiff += diffVal2;
        }
        tableHtml += `</tbody>`;
        // Add totals row for Part 2
        if (sumBaseline > 0) {
          tableHtml += `<tfoot><tr>`;
          // The first column shows the label 'Tổng cộng'; subsequent initial columns are empty
          const initialCols = 5 + suppPersonsInfo.length; // Năm HĐ, Tuổi NĐBH, Phí chính, Phí đóng thêm, Phí bổ sung main, plus one for each supplementary person
          tableHtml += `<td class="p-2 border font-semibold">Tổng cộng</td>`;
          for (let i = 1; i < initialCols; i++) {
            tableHtml += `<td class="p-2 border"></td>`;
          }
          if (freq !== 'year') {
            tableHtml += `<td class="p-2 border text-right font-semibold">${formatCurrency(sumPerTotal)}</td>`;
            tableHtml += `<td class="p-2 border text-right font-semibold">${formatCurrency(sumBaseline)}</td>`;
            tableHtml += `<td class="p-2 border text-right font-semibold" style="${sumDiff !== 0 ? 'color:#d00;font-weight:700;' : ''}">${formatCurrency(sumDiff)}</td>`;
          } else {
            // When paying annually, only show the baseline total
            tableHtml += `<td class="p-2 border text-right font-semibold">${formatCurrency(sumBaseline)}</td>`;
          }
          tableHtml += `</tr></tfoot>`;
        }
        tableHtml += `</table></div>`;
        // Render the summary and table. We intentionally avoid calling fixPart1Table here
        // because the summary table already includes the correct columns for per-period,
        // annualised (quy năm), baseline (theo năm) and the difference. Invoking
        // fixPart1Table would modify these columns and recompute values, which
        // interferes with our custom logic.
        if (container) {
          container.innerHTML = summaryHtml + tableHtml + `<div class="mt-4 text-center"><button id="export-html-btn" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">Xuất HTML</button></div>`;
        }
        document.getElementById('export-html-btn')?.addEventListener('click', () => window.print());
    ...
    /**
     * Fixes the Part 1 summary table in the detailed illustration modal by ensuring only
     * one baseline column exists and computing baseline (annual) values from the
     * available per-period, annualised and difference columns. It also recalculates
     * the difference as baseline minus annualised and applies red bold styling when
     * appropriate. This function operates on the DOM after the summary HTML has
     * been rendered. It does not modify the underlying premium calculations.
     *
     * @param {HTMLElement} container The container element where the summary
     *   table resides.
     * @param {number} periods The number of payment periods per year (1, 2 or 4).
     * @param {number} factor The factor applied to supplementary premiums for half-year or quarter payments (1.02 or 1.04).
     */
    function fixPart1Table(container, periods, factor) {
      if (!container) return;
      // Find the first table in the container (should be Part 1 summary table). We
      // only operate on the first table because subsequent tables correspond to
      // Part 2 and other sections.
      const table = container.querySelector('table');
      if (!table) return;
      const thead = table.tHead || table.querySelector('thead');
      if (!thead || !thead.rows[0]) return;
      const headCells = Array.from(thead.rows[0].cells).map(cell => (cell.textContent || '').trim());
      // Determine the indices of relevant columns. We'll look for columns by
      // canonicalising their labels to lower-case without accents or extra
      // whitespace. Vietnamese diacritics can vary, so we normalise by removing
      // accents.
      const normalize = (str) => str.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/\s+/g, ' ').trim();
      const canonHeads = headCells.map(h => normalize(h));
      const findIndex = (candidates) => {
        for (const candidate of candidates) {
          const idx = canonHeads.findIndex(h => h === normalize(candidate));
          if (idx >= 0) return idx;
        }
        return -1;
      };
      // Candidate labels for each column type
      const periodCandidates = ['phí đóng (nửa năm)', 'phí đóng nửa năm', 'phí đóng (quy)', 'phí đóng (quý)', 'phí đóng theo quý', 'phí đóng (năm)', 'phí đóng nam'];
      const annualEqCandidates = ['phí đóng quy năm', 'phí đóng (quy năm)'];
      const baselineCandidates = ['phí đóng theo năm', 'phí đóng (năm)', 'phí đóng (quy năm)'];
      const diffCandidates = ['chênh lệch', 'chênh lệch so với năm'];
      const idxPeriod = findIndex(periodCandidates);
      const idxAnnualEq = findIndex(annualEqCandidates);
      let idxBaseline = findIndex(['phí đóng theo năm']);
      let idxBaselineAlt = findIndex(['phí đóng (năm)']);
      // If baseline column isn't present under our preferred name but exists under
      // an alternate label, record it.
      if (idxBaseline < 0 && idxBaselineAlt >= 0) {
        idxBaseline = idxBaselineAlt;
        idxBaselineAlt = -1;
      }
      const idxDiff = findIndex(diffCandidates);
      // Remove duplicate baseline columns: if both 'Phí đóng theo năm' and 'Phí đóng (năm)'
      // exist, drop the latter. We assume the first baseline column (preferred
      // label) is idxBaseline and the duplicate is idxBaselineAlt.
      if (idxBaseline >= 0 && idxBaselineAlt >= 0 && idxBaseline !== idxBaselineAlt) {
        // Remove the duplicate baseline column (idxBaselineAlt)
        const removeIdx = idxBaselineAlt;
        // Remove from header
        thead.rows[0].deleteCell(removeIdx);
        // Remove from each body row
        Array.from(table.tBodies).forEach(tb => {
          Array.from(tb.rows).forEach(r => {
            if (removeIdx < r.cells.length) {
              r.deleteCell(removeIdx);
            }
          });
        });
        // Recompute canonical header indices after removal
        headCells.splice(removeIdx, 1);
        canonHeads.splice(removeIdx, 1);
        if (removeIdx < idxPeriod) idxPeriod--;
        if (removeIdx < idxAnnualEq) idxAnnualEq--;
        if (removeIdx < idxBaseline) idxBaseline--;
        if (removeIdx < idxDiff) idxDiff--;
      }
      // If no baseline column exists, insert one after the annualised column
      if (idxBaseline < 0) {
        // Insert new header cell
        const th = document.createElement('th');
        th.className = 'p-2 border';
        th.textContent = 'Phí đóng theo năm';
        let insertPos = idxAnnualEq + 1;
        if (insertPos < 0 || insertPos > thead.rows[0].cells.length) {
          insertPos = thead.rows[0].cells.length;
        }
        thead.rows[0].insertBefore(th, thead.rows[0].cells[insertPos] || null);
        idxBaseline = insertPos;
        // Insert blank cells into each row
        Array.from(table.tBodies).forEach(tb => {
          Array.from(tb.rows).forEach(r => {
            const td = document.createElement('td');
            td.className = 'p-2 border text-right';
            r.insertBefore(td, r.cells[insertPos] || null);
          });
        });
        // If baseline inserted before diff, adjust diff index
        if (idxDiff >= insertPos) idxDiff++;
      } else {
        // Ensure baseline column uses the preferred label
        thead.rows[0].cells[idxBaseline].textContent = 'Phí đóng theo năm';
      }
      // Identify updated positions after potential modifications
      const headerCells2 = Array.from(thead.rows[0].cells).map(cell => (cell.textContent || '').trim());
      const canon2 = headerCells2.map(h => normalize(h));
      const idxPeriod2 = canon2.findIndex(h => periodCandidates.map(c => normalize(c)).includes(h));
      const idxAnnualEq2 = canon2.findIndex(h => annualEqCandidates.map(c => normalize(c)).includes(h));
      const idxBaseline2 = canon2.findIndex(h => baselineCandidates.map(c => normalize(c)).includes(h));
      const idxDiff2 = canon2.findIndex(h => diffCandidates.map(c => normalize(c)).includes(h));
      // Determine if difference column exists; if not, add one after baseline column
      if (idxDiff2 < 0) {
        const th = document.createElement('th');
        th.className = 'p-2 border';
        th.textContent = 'Chênh lệch';
        const insertPos = idxBaseline2 + 1;
        thead.rows[0].insertBefore(th, thead.rows[0].cells[insertPos] || null);
        // Insert blank diff cells for each row
        Array.from(table.tBodies).forEach(tb => {
          Array.from(tb.rows).forEach(r => {
            const td = document.createElement('td');
            td.className = 'p-2 border text-right';
      ...
      // Recompute indices again to reflect any inserted columns
      const hdr = Array.from(thead.rows[0].cells).map(c => (c.textContent || '').trim());
      const canonHdr = hdr.map(h => normalize(h));
      const iPeriod = canonHdr.findIndex(h => periodCandidates.map(c => normalize(c)).includes(h));
      const iAnnual = canonHdr.findIndex(h => annualEqCandidates.map(c => normalize(c)).includes(h));
      const iBaseline = canonHdr.findIndex(h => normalize('phí đóng theo năm') === h);
      const iDiff = canonHdr.findIndex(h => normalize('chênh lệch') === h);
      // Determine if we are in year frequency to decide highlighting
      const shouldHighlight = (periods !== 1);
      // Process each body row: compute baseline and diff
      Array.from(table.tBodies).forEach(tb => {
        Array.from(tb.rows).forEach(r => {
          // Skip rows that have fewer cells than expected
          if (r.cells.length <= Math.max(iAnnual, iBaseline, iDiff)) return;
          // Determine if this row is a total row (we still compute baseline/diff but style differently)
          const isTotal = /^\s*total|^\s*tổng/i.test((r.cells[0]?.textContent || '').trim());
          // Parse values
          const valAnnualEq = parseInt((r.cells[iAnnual]?.textContent || '').replace(/[^\d\-]/g,'')) || 0;
          const valPeriod = parseInt((r.cells[iPeriod]?.textContent || '').replace(/[^\d\-]/g,'')) || 0;
          // If baseline cell already has a value, parse it; otherwise we'll compute using diff or period
          let valBaseline = parseInt((r.cells[iBaseline]?.textContent || '').replace(/[^\d\-]/g,'')) || 0;
          // Parse diff if available
          let valDiff = parseInt((r.cells[iDiff]?.textContent || '').replace(/[^\d\-]/g,'')) || 0;
          // If baseline is zero, compute baseline using existing diff or by reversing the rounding logic:
          // baseline = annualEq + diff (since diff = baseAnnual - annualEq). If diff is zero and baseline is zero,
          // derive baseline from per-period fee by dividing out factor and adjusting rounding.
          if (!valBaseline) {
            if (valDiff !== 0) {
              valBaseline = valAnnualEq + valDiff;
            } else if (valPeriod) {
              // approximate baseline: per-period fee is rounded with factor and periods; reverse by dividing by factor
              const rawPer = valPeriod / (periods === 1 ? 1 : factor);
              valBaseline = Math.round(rawPer * periods);
            } else {
              valBaseline = valAnnualEq;
            }
          }
          // If diff cell exists but shows zero or blank, recompute difference as baseline - annualEq
          valDiff = valBaseline - valAnnualEq;
          // Set baseline cell text
          r.cells[iBaseline].textContent = (valBaseline === 0) ? '' : valBaseline.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
          // Set annualised cell text (trim if 0)
          r.cells[iAnnual].textContent = (valAnnualEq === 0) ? '' : valAnnualEq.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
          // Set diff cell text with highlight if necessary
          r.cells[iDiff].textContent = (!valDiff) ? '' : valDiff.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
          if (!isTotal && shouldHighlight && valDiff !== 0) {
            r.cells[iDiff].style.color = '#d00';
            r.cells[iDiff].style.fontWeight = '700';
          } else {
            r.cells[iDiff].style.color = '';
            r.cells[iDiff].style.fontWeight = '';
          }
        });
      });
      // Compute totals row: sum baseline, annualEq, diff for non-total rows and update the last row labelled 'Tổng cộng'
      const tbody = table.tBodies[0];
      if (!tbody) return;
      let sumBaseline = 0, sumAnnual = 0, sumDiffVal = 0;
      Array.from(tbody.rows).forEach(r => {
        const first = (r.cells[0]?.textContent || '').trim().toLowerCase();
        const isTotal = /tổng/i.test(first);
        const baseVal = parseInt((r.cells[iBaseline]?.textContent || '').replace(/[^\d\-]/g,'')) || 0;
        const annVal = parseInt((r.cells[iAnnual]?.textContent || '').replace(/[^\d\-]/g,'')) || 0;
        const diffVal = parseInt((r.cells[iDiff]?.textContent || '').replace(/[^\d\-]/g,'')) || 0;
        if (!isTotal) {
          sumBaseline += baseVal;
          sumAnnual += annVal;
          sumDiffVal += diffVal;
        ...
        if (isTotal) {
          ...
          r.cells[iBaseline].textContent = sumBaseline ? sumBaseline.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') : '';
          r.cells[iAnnual].textContent = sumAnnual ? sumAnnual.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') : '';
          r.cells[iDiff].textContent = sumDiffVal ? sumDiffVal.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') : '';
          if (shouldHighlight && sumDiffVal !== 0) {
            r.cells[iDiff].style.color = '#d00';
            r.cells[iDiff].style.fontWeight = '700';
          } else {
            r.cells[iDiff].style.color = '';
    ...
    // Also store a reference to this implementation so that any later overrides
    // can delegate back to it. Without preserving this reference, subsequent
    // assignments to window.generateSummaryTable would replace this version
    // and make it inaccessible. We'll use __generateSummaryTableOld as a global
    // variable to point to our comprehensive implementation.
    window.__generateSummaryTableOld = customGenerateSummary;
  })();
...
(function(){
  ...
  function customGenerateSummary() {
    // If a comprehensive summary function has been defined earlier (stored on window),
    // delegate to it and skip this implementation. This ensures the latest override
    // does not accidentally revert back to an outdated version.
    if (window.__generateSummaryTableOld) {
      return window.__generateSummaryTableOld();
    }

    // Expose fixPart1Table so that other summary functions (if any) can call it safely.
    // We assign on window under a unique property to avoid clobbering any existing
    // names. This allows legacy generateSummaryTable implementations to call
    // fixPart1Table if needed.
    window.__fixPart1Table = fixPart1Table;
    const container = document.getElementById('summary-content-container');
    const modal = document.getElementById('summary-modal');
    if (container) container.innerHTML = '';
    try {
  ...
  window.generateSummaryTable = customGenerateSummary;
  // Also override the original generateSummaryTable reference used by event listeners
  // so that clicking the "Xem Bảng Minh Họa Chi Tiết" button calls our custom version.
  // Without this assignment, the early generateSummaryTable function defined above remains bound
  // to the click event and our changes would not take effect.
  if (typeof generateSummaryTable === 'function') {
...
/**
 * Business Enhancer (safe build)
 * - Highlights diff (bold, red) when freq is half/quarter (non-annual) in summaries
 * - Detailed table: adds "Phí đóng (năm)", computes "Phí đóng quy năm" (half*2 or quarter*4), recomputes "Chênh lệch"
 * - Part 2: converts supplementary product fees per chosen frequency (x1.02 for half, x1.04 for quarter) and adds Total & Grand Total rows
 * - Non-invasive: runs AFTER original generators; only post-processes DOM if tables/headers present
 */
(function BusinessEnhancerSafe(){
  // Disable BusinessEnhancer functionality. The summary generation now computes
  // per-period, annualised, baseline and difference values correctly, and
  // includes totals rows, so post-processing of the tables is no longer
  // necessary. To avoid interfering with the custom logic, return early.
  return;
  const $$ = (sel, root=document) => root.querySelector(sel);
  const $$$ = (sel, root=document) => Array.from(root.querySelectorAll(sel));

  function getFreq(){
    // detect radio/selector commonly used
    const r = $$('input[name="payment-frequency"]:checked') || $$('#payment-frequency') || $$('.payment-frequency:checked');
    if (r) {
      const v = r.value || r.getAttribute('value') || r.textContent;
      if (!v) return 'year';
      const t = String(v).toLowerCase();
      if (t.includes('half') || t.includes('nửa')) return 'half';
      if (t.includes('quarter') || t.includes('quý')) return 'quarter';
      if (t.includes('year') || t.includes('năm')) return 'year';
    }
    // fallback: look for active class
    const active = $$('.payment-frequency.active');
    if (active) {
      const t = active.textContent.toLowerCase();
      if (t.includes('nửa') || t.includes('half')) return 'half';
      if (t.includes('quý') || t.includes('quarter')) return 'quarter';
      if (t.includes('năm') || t.includes('year')) return 'year';
    }
    return 'year';
  }
  ...
  function parseVnd(s){
    ...
    const txt = String(s).replace(/[^\d\-]/g, '');
    if (!txt) return 0;
  ...
  function ensureHeader(table, label, insertAfterIdx=null){
    // Ensure header cell with specific label exists; returns its column index
    const thead = table.tHead || table.querySelector('thead');
    if (!thead) return -1;
    const row = thead.rows[0];
    const headers = Array.from(row.cells).map(c => c.textContent.trim());
    let idx = headers.findIndex(h => h.toLowerCase() === label.toLowerCase());
    if (idx !== -1) return idx;
    // insert new th
    const th = document.createElement('th');
    th.className = 'p-2 border';
    th.textContent = label;
    if (insertAfterIdx == null || insertAfterIdx >= row.cells.length - 1) {
      row.appendChild(th);
      idx = row.cells.length - 1;
    } else {
      row.insertBefore(th, row.cells[insertAfterIdx + 1]);
      idx = insertAfterIdx + 1;
    }
    // add empty td to each body row at same column
    const tbody = table.tBodies[0];
    if (tbody) {
      for (const r of tbody.rows) {
        const td = document.createElement('td');
        td.className = 'p-2 border text-right';
        if (insertAfterIdx == null || insertAfterIdx >= r.cells.length - 1) {
          r.appendChild(td);
        } else {
          r.insertBefore(td, r.cells[insertAfterIdx + 1]);
        }
      }
    }
    return idx;
  ...
  function headerIndexMap(table){
    ...
    cells.forEach((c, i) => {
      map[c.textContent.trim().toLowerCase()] = i;
    });
    return map;
  }
  ...
  function highlightDiffs(root){
    const freq = getFreq();
    const shouldHighlight = (freq === 'half' || freq === 'quarter');
    // 1) Known #freq-diff spot
    const dEl = $$('#freq-diff', root) || $$('#freq-diff');
    if (dEl) {
      const val = parseVnd(dEl.textContent);
      const parent = dEl.closest('div') || dEl;
      parent.classList.toggle('hidden', val === 0);
      if (shouldHighlight && val !== 0) {
        dEl.style.color = '#d00';
    ...
    // 2) Any column named "Chênh lệch"
    $$$('table').forEach(t => {
      const map = headerIndexMap(t);
      const diffIdx = map['chênh lệch'];
      if (diffIdx == null) return;
      const tbody = t.tBodies[0];
      if (!tbody) return;
      Array.from(tbody.rows).forEach(r => {
        const cell = r.cells[diffIdx];
        if (!cell) return;
        const v = parseVnd(cell.textContent);
        // hide zero rows only if not a total row
        const label = (r.cells[0]?.textContent || '').toLowerCase();
        if (label && !/tổng|grand/i.test(label)) {
          r.classList.toggle('hidden', v === 0);
        }
        if (shouldHighlight && v !== 0) {
  ...
  function enhanceDetailedTables(root){
    const freq = getFreq();
    const factor = (freq === 'half') ? 2 : (freq === 'quarter') ? 4 : 1;
    const suppFactor = (freq === 'half') ? 1.02 : (freq === 'quarter') ? 1.04 : 1;

    const tables = $$$('table', root);
    tables.forEach(table => {
      const map = headerIndexMap(table);
      // Heuristic: find any fee column by header text
      const idxHalf = map['phí đóng (nửa năm)'];
      const idxQuarter = map['phí đóng (theo quý)'] ?? map['phí đóng (quý)'];
      // baseline column may be labelled as 'Phí đóng theo năm', 'Phí đóng (năm)' or 'Phí đóng (quy năm)' depending on upstream
      const idxBaseline = map['phí đóng theo năm'] ?? map['phí đóng (năm)'] ?? map['phí đóng (quy năm)'];
      const idxDiff = map['chênh lệch'];

      // Skip Part 1 (summary) tables: those whose first header is "Sản phẩm" (case-insensitive)
      {
        const thead = table.tHead || table.querySelector('thead');
        if (thead && thead.rows[0] && thead.rows[0].cells.length) {
          const firstHeader = thead.rows[0].cells[0].textContent.trim().toLowerCase();
          // If first header is 'sản phẩm' then this is the summary table; skip enhancements
          if (firstHeader === 'sản phẩm') {
            return;
          }
        }
      }
      // Ensure we have target columns
      const insertAfter = Math.max(idxHalf ?? -1, idxQuarter ?? -1, idxBaseline ?? -1, idxDiff ?? -1);
      // baseline annual column index: reuse if exists, otherwise insert a column with label 'Phí đóng theo năm'
      const annualIdx = (idxBaseline != null && idxBaseline >= 0) ? idxBaseline : ensureHeader(table, 'Phí đóng theo năm', insertAfter);
      const eqIdx = ensureHeader(table, 'Phí đóng quy năm', annualIdx);
      const diffIdx = (idxDiff != null && idxDiff >= 0) ? idxDiff : ensureHeader(table, 'Chênh lệch', eqIdx);

      const tbody = table.tBodies[0];
      if (!tbody) return;

      let sumAnnual = 0, sumEq = 0, sumDiff = 0, sumSupp = 0, sumTotal = 0;

      Array.from(tbody.rows).forEach(r => {
        // Skip total rows (we'll recompute at end)
        const first = (r.cells[0]?.textContent || '').trim().toLowerCase();
        const isTotalRow = /tổng/i.test(first);

        // Read per-period fee
        const vHalf = (idxHalf != null && r.cells[idxHalf]) ? parseVnd(r.cells[idxHalf].textContent) : null;
        const vQuarter = (idxQuarter != null && r.cells[idxQuarter]) ? parseVnd(r.cells[idxQuarter].textContent) : null;
        let vAnnual = (annualIdx != null && r.cells[annualIdx]) ? parseVnd(r.cells[annualIdx].textContent) : null;

        // Compute annual if missing
        if ((vAnnual == null || isNaN(vAnnual)) && (vHalf != null || vQuarter != null)) {
          if (vHalf != null) vAnnual = vHalf * 2;
          if (vQuarter != null) vAnnual = vQuarter * 4;
        }

        // Compute annual-equivalent (quy năm) based on *chosen* frequency
        let vEq = null;
        if (freq === 'year') vEq = vAnnual || 0;
        else if (freq === 'half') vEq = (vHalf != null) ? vHalf * 2 : (vAnnual != null ? vAnnual : 0);
        else if (freq === 'quarter') vEq = (vQuarter != null) ? vQuarter * 4 : (vAnnual != null ? vAnnual : 0);

        // Write back
        if (r.cells[annualIdx]) r.cells[annualIdx].textContent = fmtVnd(vAnnual || 0);
        if (r.cells[eqIdx]) r.cells[eqIdx].textContent = fmtVnd(vEq || 0);

        // Difference
        const vDiff = (vAnnual || 0) - (vEq || 0);
        if (r.cells[diffIdx]) r.cells[diffIdx].textContent = fmtVnd(vDiff);

        // Supplementary columns (Part 2) heuristic: any header containing 'bổ sung'
        // multiply by 1.02 / 1.04 depending on frequency to reflect period fees
        Object.keys(map).forEach(key => {
          if (key.includes('bổ sung')) {
            const cIdx = map[key];
            const cell = r.cells[cIdx];
            if (!cell) return;
            const base = parseVnd(cell.textContent);
            const adjusted = Math.round(base * suppFactor);
            cell.textContent = fmtVnd(adjusted);
            sumSupp += adjusted;
          }
        });

        // Running sums (skip total rows)
        if (!isTotalRow) {
          sumAnnual += vAnnual || 0;
          sumEq += vEq || 0;
          sumDiff += vDiff || 0;
          // sumTotal will be computed later if table has an explicit 'Tổng' column
        }
      });

      // Add/refresh a Grand Total row
      const gt = document.createElement('tr');
      const cols = table.tHead ? table.tHead.rows[0].cells.length : (table.rows[0]?.cells.length || 1);
      ...
      // Label in first col
      gt.cells[0].textContent = 'Tổng cộng';
      // Fill sums if indexes exist
      if (annualIdx != null && gt.cells[annualIdx]) gt.cells[annualIdx].textContent = fmtVnd(sumAnnual);
      if (eqIdx != null && gt.cells[eqIdx]) gt.cells[eqIdx].textContent = fmtVnd(sumEq);
      if (diffIdx != null && gt.cells[diffIdx]) {
        gt.cells[diffIdx].textContent = fmtVnd(sumDiff);
        if (sumDiff !== 0 && (getFreq() !== 'year')) {
          gt.cells[diffIdx].style.color = '#d00';
          gt.cells[diffIdx].style.fontWeight = '700';
        }
      }
      const tbody2 = table.tBodies[0];
      if (tbody2) tbody2.appendChild(gt);
    });
  }

  function enhanceWaiverOther(root){
    // If there is a waiver-person selection not equal to main, ensure Part 2 shows a column for that person's fee
    // This is heuristic & non-breaking: only adds if we can detect it
    const waiverSel = $$('select[name="waiver-person"], #waiver-person, .waiver-person');
    if (!waiverSel) return;
    const val = waiverSel.value || '';
    if (!val || /main|chính/i.test(val)) return; // only when 'người khác'
    // Find the detailed table and ensure a column exists for that person
    const tables = $$$('table');
    tables.forEach(table => {
      const map = headerIndexMap(table);
      // If a column for this person is missing but table has per-person columns, add an empty one to be filled by upstream logic
      const personLabel = 'Phí ' + waiverSel.options[waiverSel.selectedIndex]?.text;
      if (map[personLabel?.toLowerCase()]) return;
      // Insert after main product col if exists, otherwise at the end
      const keys = Object.keys(map);
      let afterIdx = -1;
      for (let k of keys) {
        if (k.includes('nđbh chính') || k.includes('sản phẩm chính')) { afterIdx = map[k]; break; }
      }
      ensureHeader(table, personLabel || 'Phí miễn đóng (người khác)', afterIdx);
    });
  }
  ...
  function runEnhancements(){
    const root = document;
    highlightDiffs(root);
    enhanceDetailedTables(root);
    enhanceWaiverOther(root);
  }

  // Hook after original summary generation if available
  const tryHook = () => {
    const btn = $$('#view-summary-btn');
    if (btn && !btn.__bizEnhancerBound) {
      btn.__bizEnhancerBound = true;
      btn.addEventListener('click', () => setTimeout(runEnhancements, 0));
    }
    // Also re-run when frequency changes
    $$$('input[name="payment-frequency"], .payment-frequency').forEach(el => {
      if (!el.__bizEnhancerBound) {
        el.__bizEnhancerBound = true;
        el.addEventListener('change', () => setTimeout(runEnhancements, 0));
        el.addEventListener('click', () => setTimeout(runEnhancements, 0));
      }
    });
  };

  // Initial run + observer to catch dynamic content
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => { tryHook(); runEnhancements(); });
  } else {
    tryHook(); runEnhancements();
  }

  // Mutation observer to re-apply when modal content changes
...
/* ==== summary_agent_patch merged below (do not remove) ==== */

/*! summary_agent_patch.js — DOM-level patcher to fix Part 1 & Part 2 and add export buttons
   Safe: does not touch your calculation logic. Load this AFTER your original JS file.
*/
(function(){
  const CURRENCY = {
    parse: (s) => {
      if (!s) return 0;
      if (typeof s === 'number') return s;
      s = String(s).replace(/[^\d\-]/g,''); // keep digits and minus
  ...
  function getPeriods(){
    // Try to read from UI controls; fallback to header text
    const checked = document.querySelector('[name="payment-frequency"]:checked');
    if (checked && checked.value) {
      if (/year/i.test(checked.value) || checked.value==='year') return 1;
      if (/half|semi|2/i.test(checked.value)) return 2;
      if (/quarter|quy|4/i.test(checked.value)) return 4;
    }
    // Fallback by reading Part 1 header
    const h = document.querySelector('#summary-content-container table thead');
    if (h){
      const txt = h.textContent.toLowerCase();
      if (txt.includes('nửa năm')) return 2;
      if (txt.includes('quý')) return 4;
      if (txt.includes('năm')) return 1;
    }
    return 1;
  }
  function suppFactor(periods){
    if (periods===2) return 1.02;
    if (periods===4) return 1.04;
    return 1;
  }
  function findTables(){
    const root = document.getElementById('summary-content-container') || document;
    const tables = Array.from(root.querySelectorAll('table'));
    if (tables.length<2) return {part1: tables[0]||null, part2: tables[1]||null};
    // Heuristic: Part 1 table appears first
    return {part1: tables[0], part2: tables[1]};
  }

  // Add export buttons at footer (safe to call multiple times)
  function ensureExportButtons(){
    const root = document.getElementById('summary-content-container') || document.body;
    let footer = root.querySelector('.summary-export-footer');
    if (!footer){
      footer = document.createElement('div');
      footer.className = 'summary-export-footer mt-4 text-center';
      footer.innerHTML = '<button id="export-html-btn" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 mr-2">Xuất HTML</button><button id="export-pdf-btn" class="bg-stone-600 text-white px-4 py-2 rounded hover:bg-stone-700">Xuất PDF</button>';
      root.appendChild(footer);
    }
    if (footer && !footer.__bind){
      footer.__bind = true;
      function buildDoc(){
        const host = document.getElementById('summary-content-container') || document.body;
        const clone = host.cloneNode(true);
        clone.querySelectorAll('#export-html-btn,#export-pdf-btn,.summary-export-footer').forEach(el=>el.remove());
        // Show hidden
        clone.querySelectorAll('[hidden]').forEach(el => el.hidden = false);
        clone.querySelectorAll('[aria-hidden="true"]').forEach(el => el.setAttribute('aria-hidden','false'));
        ...
        .text-danger{color:#d00;font-weight:700}`;
        ...
        const ymd = `${y}-${m}-${d}`;
        const html = `<!doctype html><html lang="vi"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Bảng tóm tắt quyền lợi & phí - ${ymd}</title><style>${baseCss}${printCss}</style></head><body>${clone.innerHTML}</body></html>`;
        return {html, ymd};
      }
      footer.querySelector('#export-html-btn').addEventListener('click', ()=>{
        const {html, ymd} = buildDoc();
        const blob = new Blob([html], {type:'text/html;charset=utf-8'});
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a'); a.href=url; a.download=`bang-tom-tat_${ymd}.html`; document.body.appendChild(a); a.click(); setTimeout(()=>{ URL.revokeObjectURL(url); a.remove(); },0);
      });
      footer.querySelector('#export-pdf-btn').addEventListener('click', ()=>{
        const {html} = buildDoc();
        const iframe = document.createElement('iframe');
        Object.assign(iframe.style,{position:'fixed',right:0,bottom:0,width:0,height:0,border:0});
        document.body.appendChild(iframe);
        iframe.onload = ()=> setTimeout(()=>{ try{ iframe.contentWindow.focus(); iframe.contentWindow.print(); } finally { setTimeout(()=>iframe.remove(), 60000); } },300);
        iframe.srcdoc = html;
      });
    }
  }

  function fixPart1(table, periods){
    if (!table) return;
    const thead = table.tHead || table.querySelector('thead');
    if (!thead) return;
    const headers = Array.from(thead.querySelectorAll('th')).map(th=>th.textContent.trim().toLowerCase());
    const idxPer = headers.findIndex(h=>/phí đóng.*(nửa năm|quý|kỳ|năm)/i.test(h));
    let idxAnnualEq = headers.findIndex(h=>h.includes('quy năm'));
    let idxBase = headers.findIndex(h=>h.includes('phí đóng theo năm'));
    let idxDiff = headers.findIndex(h=>h.includes('chênh lệch'));

    // If baseline column missing, insert one before 'chênh lệch'
    if (idxBase === -1){
      const th = document.createElement('th'); th.className='p-2 border'; th.textContent='Phí đóng theo năm';
      const ref = idxDiff>-1 ? thead.rows[0].children[idxDiff] : null;
      thead.rows[0].insertBefore(th, ref);
      // Adjust indices after insertion
      const hs = Array.from(thead.rows[0].children).map(x=>x.textContent.toLowerCase());
      idxBase = hs.findIndex(h=>h.includes('phí đóng theo năm'));
      idxDiff = hs.findIndex(h=>h.includes('chênh lệch'));
      idxAnnualEq = hs.findIndex(h=>h.includes('quy năm'));
    }

    let totalPer=0, totalAnnEq=0, totalBase=0, totalDiff=0;

    const tbody = table.tBodies[0];
    Array.from(tbody.rows).forEach(tr => {
      const tds = Array.from(tr.children);
      if (!tds.length) return;
      const isTotal = /tổng cộng/i.test(tds[0].textContent);
      const isPersonHeader = tds[0].classList.contains('font-semibold') && (tds[1].textContent.trim()==='—');
      if (isTotal || isPersonHeader) return; // skip, we'll rebuild total later

      function getNum(idx){ return idx>-1 ? CURRENCY.parse(tds[idx].textContent) : 0; }
      const per = getNum(idxPer);
      const annEq = getNum(idxAnnualEq);
      let base = idxBase>-1 ? getNum(idxBase) : 0;
      // If baseline empty but we can infer: for supplemental rows baseline = annEq / factor
      if (!base && annEq){
        const f = suppFactor(periods);
        base = Math.round(annEq / f);
        // write baseline back
        if (idxBase>-1) tds[idxBase].textContent = CURRENCY.fmt(base);
      }
      const diff = base - annEq;
      if (idxDiff>-1){
        tds[idxDiff].innerHTML = diff ? `<span class="text-danger">${CURRENCY.fmt(diff)}</span>` : '';
      }
      totalPer += per; totalAnnEq += annEq; totalBase += base; totalDiff += diff;
    });

    // Remove existing total row, then append our total
    const oldTotal = Array.from(tbody.rows).find(r => /tổng cộng/i.test(r.cells[0]?.textContent||''));
    if (oldTotal) oldTotal.remove();
    const tr = document.createElement('tr');
    function td(txt, cls){ const e=document.createElement('td'); e.className='p-2 border ' + (cls||''); e.innerHTML=txt; return e; }
    tr.appendChild(td('Tổng cộng','font-semibold'));
    // two dummy cols for STBH & số năm
    tr.appendChild(td(''));
    tr.appendChild(td(''));
    tr.appendChild(td(CURRENCY.fmt(totalPer),'text-right font-semibold'));
    tr.appendChild(td(CURRENCY.fmt(totalAnnEq),'text-right font-semibold'));
    tr.appendChild(td(CURRENCY.fmt(totalBase),'text-right font-semibold'));
    if (idxDiff>-1) tr.appendChild(td(totalDiff ? CURRENCY.fmt(totalDiff) : '0','text-right font-semibold'));
    tbody.appendChild(tr);
  }

  function fixPart2(table, periods){
    if (!table) return;
    const thead = table.tHead || table.querySelector('thead');
    if (!thead) return;
    const headers = Array.from(thead.querySelectorAll('th')).map(th=>th.textContent.trim().toLowerCase());
    // Identify key columns
    const idxMain = headers.findIndex(h=>/phí chính/i.test(h));
    const idxExtra = headers.findIndex(h=>/đóng thêm/i.test(h));
    const suppStart = (()=>{
      let i = headers.findIndex(h=>/phí bổ sung/i.test(h));
      return i;
    })();
    // Determine where final trio should be
    let idxTotalPeriod = headers.findIndex(h=>/tổng \(theo kỳ\)/i.test(h));
    let idxBaseline = headers.findIndex(h=>/nếu đóng theo năm/i.test(h));
    let idxDiff = headers.findIndex(h=>/chênh lệch/i.test(h));

    const tbody = table.tBodies[0];
    const f = suppFactor(periods);

    // Recompute trio per row
    Array.from(tbody.rows).forEach(tr=>{
      const tds = Array.from(tr.children);
      if (!tds.length) return;
      const isFooter = /tổng cộng/i.test(tds[0].textContent||'');
      if (isFooter) return;
      // Read baseline totals (year)
      let baseline = 0;
      if (idxMain>-1) baseline += CURRENCY.parse(tds[idxMain].textContent);
      if (idxExtra>-1) baseline += CURRENCY.parse(tds[idxExtra].textContent);
      // Sum supplemental baseline shown in columns (assume baseline displayed)
      let suppBaselineSum = 0;
      for (let c=suppStart; c>=0 && c<tds.length; c++){
        // stop before trio columns if present
        if (c===idxTotalPeriod || c===idxBaseline || c===idxDiff) break;
        // identify only supplemental columns by header text
        const h = headers[c]||'';
        if (/phí bổ sung/i.test(h)) {
          const val = CURRENCY.parse(tds[c].textContent);
          suppBaselineSum += val;
          // also overwrite this cell with per-period value when periods≠1
          const perPeriodVal = Math.round(val * f);
          if (periods!==1){
            tds[c].textContent = perPeriodVal ? CURRENCY.fmt(perPeriodVal) : '';
          }
        }
      }
      baseline += suppBaselineSum;

      // Compute total(theo kỳ) as baseline main+extra + sum supplements (after applying factor)
      let totalPeriod = 0;
      // main & extra are annual baseline (not multiplied)
      if (idxMain>-1) totalPeriod += CURRENCY.parse(tds[idxMain].textContent);
      if (idxExtra>-1) totalPeriod += CURRENCY.parse(tds[idxExtra].textContent);
      // supplemental columns now show per-period (after rewrite above)
      for (let c=suppStart; c>=0 && c<tds.length; c++){
        if (c===idxTotalPeriod || c===idxBaseline || c===idxDiff) break;
        const h = headers[c]||'';
        if (/phí bổ sung/i.test(h)) totalPeriod += CURRENCY.parse(tds[c].textContent);
      }

      // Write trio
      if (periods===1){
        // Only baseline column should exist/show
        if (idxBaseline>-1) tds[idxBaseline].textContent = CURRENCY.fmt(baseline);
        if (idxTotalPeriod>-1) tds[idxTotalPeriod].textContent = '';
        if (idxDiff>-1) tds[idxDiff].textContent = '';
      } else {
        if (idxTotalPeriod>-1) tds[idxTotalPeriod].textContent = CURRENCY.fmt(totalPeriod);
        if (idxBaseline>-1) tds[idxBaseline].textContent = CURRENCY.fmt(baseline);
        const diff = baseline - totalPeriod;
        if (idxDiff>-1) tds[idxDiff].innerHTML = diff ? `<span class="text-danger">${CURRENCY.fmt(diff)}</span>` : '';
      }
    });

    // Rebuild footer totals (sum all visible numeric columns)
    const tfoot = table.tFoot || table.createTFoot();
    tfoot.innerHTML='';
    const tr = document.createElement('tr');
    function td(txt, cls){ const e=document.createElement('td'); e.className='p-2 border ' + (cls||''); e.innerHTML=txt; return e; }
    tr.appendChild(td('Tổng cộng','font-semibold'));
    // second column may be 'Năm HĐ' or similar → leave blank
    tr.appendChild(td(''));

    // Sum each numeric column except the first two
    const colCount = (table.tBodies[0].rows[0]||{cells:[]}).cells.length;
    for (let c=2; c<colCount; c++){
      // sum column c
      let sum=0;
      Array.from(tbody.rows).forEach(r=>{
        if (/tổng cộng/i.test(r.cells[0]?.textContent||'')) return;
        sum += CURRENCY.parse(r.cells[c]?.textContent||'');
      });
      tr.appendChild(td(sum ? CURRENCY.fmt(sum) : (c===colCount-1 ? '0' : ''), 'text-right font-semibold'));
    }
    tfoot.appendChild(tr);
  }

  function patch(){
    // Call original renderer if exists to populate DOM
    const old = window.generateSummaryTable;
    const periods = getPeriods();
    const res = old ? old() : undefined;
    setTimeout(()=>{
      try{
        const {part1, part2} = findTables();
        fixPart1(part1, periods);
        fixPart2(part2, periods);
        ensureExportButtons();
      }catch(e){ console.error('summary_agent_patch error', e); }
    }, 0);
    return res;
  }
  ...
  if (typeof window !== 'undefined'){
    const old = window.generateSummaryTable;
    if (!old || old.__patched) return;
    function wrapped(){ return patch(); }
    wrapped.__patched = true;
    window.generateSummaryTable = wrapped;
  }
})();
