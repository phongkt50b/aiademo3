...
function updateTargetAge() {
    ...
    try {
      let hint = document.getElementById('target-age-hint');
      if (!hint) {
        hint = document.createElement('p');
        hint.id = 'target-age-hint';
        hint.className = 'text-sm text-gray-500 mt-1';
        const holder = document.getElementById('target-age-input')?.parentElement;
        if (holder) holder.appendChild(hint);
      }
      const v = parseInt(document.getElementById('target-age-input')?.value||'0',10) || 0;
      hint.textContent = `Tối thiểu là ${v} tuổi`;
    } catch(e) {}
}
else if (mainProduct === 'AN_BINH_UU_VIET') {
...
function renderSuppList(){
  ...
  if (main) persons.push({id: main.id, name: main.name || 'NĐBH Chính'});
  document.querySelectorAll('#supplementary-insured-container .person-container').forEach(c=>{
    const p = collectPersonData(c,false); if (p) persons.push({id:p.id, name:p.name || 'NĐBH bổ sung'});
  });
  // MDP3 chọn người khác
  try {
    const enabled = window.MDP3 && MDP3.isEnabled && MDP3.isEnabled();
    const selId = enabled ? (MDP3.getSelectedId && MDP3.getSelectedId()) : null;
    if (enabled && selId === 'other') {
      const cont = document.getElementById('person-container-mdp3-other');
      const name = cont?.querySelector('.name-input')?.value || 'Người được miễn đóng phí';
      persons.push({id: 'mdp3_other', name});
    }
  } catch(e){}
  
  const byPerson = (appState && appState.fees && appState.fees.byPerson) ? appState.fees.byPerson : {};
  const html = persons.map(p=>{
    const f = byPerson[p.id] || {};
    const supp = Math.round(((f.supp||0))/1000)*1000; // chỉ phí bổ sung
    return `<div class="flex justify-between"><span>${sanitizeHtml(p.name)}</span><span>${formatDisplayCurrency(supp)}</span></div>`;
  }).join('');
  box.innerHTML = html;
}
);
