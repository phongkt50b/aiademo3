...
/* logic.js – patch bundle (override các lỗi cố hữu) */
/* ====================== TIỆN ÍCH CƠ BẢN ====================== */
const _fmtVN = (n) => {
  n = Number(n || 0);
...
/* Collect dữ liệu người – dùng bản gốc nếu có */
const collectPersonData = _fn('collectPersonData', (container, isMain=false) => {
  // fallback tối giản để tránh lỗi runtime (khuyến nghị giữ bản gốc của bạn)
  ...
  // bổ sung supplements nếu có
  return { id, isMain, name, dob, gender, age, supplements: {} };
});

/* Bộ tính phí – dùng bản gốc nếu có */
...
/* appState – trỏ về biến đang dùng trong hệ thống cũ */
window.appState = window.appState || {
  mainProduct: { key: '', stbh: 0, paymentTerm: 0, extraPremium: 0 },
  fees: { baseMain: 0, total: 0, totalSupp: 0, byPerson: {} }
};

/* MĐP3 module – dùng module cũ nếu có; nếu không, stub an toàn (phí=0) */
window.MDP3 = window.MDP3 || (() => {
  const isEnabled = () => !!$get('mdp3-enable')?.checked;
  const getSelectedId = () => {
    // tìm radio/select trong #mdp3-section (#mdp3-select-container)
  ...
  const getPremium = () => {
    // phí đã tính sẵn trong hệ thống của bạn; nếu không có, trả 0
    const text = $get('mdp3-fee-display')?.textContent || '0';
    return parseFormattedNumber(text);
  };
  const getSelectedAge = () => null; // hệ thống của bạn có thể override
...
function _validatePaymentTermAndShowError() {
  try {
    ...
    const minByKey = (k)=>{
      ...
      if (k === 'AN_BINH_UU_VIET') return 5;   // ABƯV chỉ cho 5/10/15
...
/* Giữ danh sách “Xem từng người” luôn cập nhật khi thêm/xóa người bổ sung, đổi MĐP3 */
function _ensureSuppListReRender() {
  try {
    ...
    $get('toggle-supp-list-btn')?.addEventListener('click', ()=>{
      setTimeout(()=>renderSuppList(), 0);
    });
  } catch {}
}
document.addEventListener('DOMContentLoaded', _ensureSuppListReRender, {once:true});

/* ====================== OVERRIDE renderSuppList ====================== */
/* Chỉ cộng Riders + MĐP3, không cộng phí chính/đóng thêm */
function renderSuppList(){
  const box = $get('supp-insured-summaries');
  if (!box) return;
...
/* ====================== PATCH performCalculations ====================== */
/* Gọi hàm gốc (nếu có) rồi phân bổ MĐP3 vào per-person breakdown */
(function patchPerformCalculations(){
  const orig = _fn('performCalculations', null);
  if (!orig) return; // nếu không có, bỏ qua (tránh crash)
  window.performCalculations = function(state){
    const fees = orig(state);
    try {
      // tổng riders đã + mdp3Fee ở hàm gốc; nhưng phân bổ về byPerson có thể chưa
      const mdpEnabled = !!(window.MDP3 && MDP3.isEnabled && MDP3.isEnabled());
      const mdp3Fee = mdpEnabled ? (MDP3.getPremium && MDP3.getPremium()) : 0;
      const targetId = mdpEnabled ? (MDP3.getSelectedId && MDP3.getSelectedId()) : null;
      if (mdpEnabled && mdp3Fee > 0) {
        state.fees = state.fees || {};
        state.fees.byPerson = state.fees.byPerson || {};
        if (targetId && state.fees.byPerson[targetId]) {
          state.fees.byPerson[targetId].supp = (state.fees.byPerson[targetId].supp||0) + mdp3Fee;
          state.fees.byPerson[targetId].suppDetails = state.fees.byPerson[targetId].suppDetails || {};
          state.fees.byPerson[targetId].suppDetails.mdp3 = mdp3Fee;
        } else if (targetId === 'other') {
          if (!state.fees.byPerson['mdp3_other']) state.fees.byPerson['mdp3_other'] = { main: 0, supp: 0, total: 0, suppDetails: {} };
          state.fees.byPerson['mdp3_other'].supp += mdp3Fee;
          state.fees.byPerson['mdp3_other'].suppDetails.mdp3 = mdp3Fee;
        }
      }
    } catch(e){}
    return fees;
...
function generateSummaryTable() {
  ...
  try {
    ...
    for (let year=1; mainInfo.age + year -1 <= targetAge; year++){
      const ageNow = mainInfo.age + year -1;
      const inPayTerm = (year <= paymentTerm);
      const baseAnnualMain = inPayTerm ? (window.appState.fees?.baseMain||0) : 0;  // cố định theo pay-term
    ...
    for (const r of rowsData){
      ...
      sumExtra += r.extra;
      for (let i=0;i<persons.length;i++){
        sumSuppCols[i] += r.perPersonSuppYear[i];   // hiển thị riders = quy năm
      }
      if (!isAnnual){ sumTotalPeriod += r.totalPeriod; sumDiff += r.diff; }
      sumTotalYear += r.totalYear;
