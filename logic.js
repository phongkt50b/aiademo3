...
function updateSummaryUI(fees) {
  ...
  try {
    ...
    if (suppBox) {
      suppBox.innerHTML = (fees.totalSupp||0) ? `<div class="flex justify-between items-center py-2 border-b">
    ...
    const perMain  = periods===1 ? 0 : safe((fees.baseMain||0) / periods);
    const perExtra = periods===1 ? 0 : safe((fees.extra||0) / periods);
    const perSupp  = periods===1 ? 0 : safe(((fees.totalSupp||0) * factor) / periods);
