  ...
  try {
    ...
    personsList.forEach(person => {
      ...
      person.products.forEach(pr => {
        ...
        if (showDiffColumn) {
          summaryHtml += `<td class="p-2 border text-right">${pr.diff ? formatCurrency(pr.diff) : ''}</td>`;
        }
        summaryHtml += `</tr>`;
      });
    });
...
(function BusinessEnhancerSafe(){
  ...
  function enhanceDetailedTables(root){
    ...
    tables.forEach(table => {
      ...
      const idxAnnual = map['phí đóng (năm)'];
      const idxDiff = map['chênh lệch'];

      // Ensure we have target columns
      const insertAfter = Math.max(idxHalf ?? -1, idxQuarter ?? -1, idxAnnual ?? -1, idxDiff ?? -1);
      const annualIdx = (idxAnnual != null && idxAnnual >= 0) ? idxAnnual : ensureHeader(table, 'Phí đóng (năm)', insertAfter);
...
(function(){
  ...
  // Nhận diện tên cột (chấp nhận alias)
  const NAMES = {
    half: ['phí đóng (nửa năm)','phí đóng nửa năm'],
    quarter: ['phí đóng (theo quý)','phí đóng (quý)'],
    baseline: ['phí đóng theo năm','phí đóng (quy năm)','phí đóng (năm)'], // baseline annual
    annualized: ['phí đóng quy năm'], // annualized theo kỳ đang chọn
    diff: ['chênh lệch']
  };

  function getFreq(){
    const r = $$('input[name="payment-frequency"]:checked') || $$('#payment-frequency') || $$('.payment-frequency:checked');
    const v = (r && (r.value || r.getAttribute('value') || r.textContent) || '').toLowerCase();
    if (v.includes('nửa') || v.includes('half')) return 'half';
    if (v.includes('quý') || v.includes('quarter')) return 'quarter';
    if (v.includes('năm') || v.includes('year')) return 'year';
    const active = $$('.payment-frequency.active');
    if (active) {
      const t = active.textContent.toLowerCase();
      if (t.includes('nửa') || t.includes('half')) return 'half';
  ...
  function headerIndexMap(table){
    const map = {};
    const thead = table.tHead || table.querySelector('thead');
    if (!thead || !thead.rows[0]) return map;
    Array.from(thead.rows[0].cells).forEach((c,i)=>{ map[norm(c.textContent)] = i; });
    return map;
  }
  function findIdx(map, names){
    for (const n of names) if (map.hasOwnProperty(n)) return map[n];
    return -1;
  }
  function ensureHeader(table, label, afterIdx){
    const thead = table.tHead || table.querySelector('thead');
    if (!thead || !thead.rows[0]) return -1;
    const row = thead.rows[0];
    // Nếu đã có header đúng nhãn thì thôi
    const exist = Array.from(row.cells).findIndex(c => norm(c.textContent) === norm(label));
    ...
    let insertPos = (typeof afterIdx === 'number' && afterIdx >= 0 && afterIdx < row.cells.length)
      ? afterIdx + 1 : row.cells.length;

    if (insertPos >= row.cells.length) {
      row.appendChild(th);
      insertPos = row.cells.length - 1;
    } else {
      row.insertBefore(th, row.cells[insertPos]);
    }

    // thêm ô trống cho tbody
    const tbody = table.tBodies[0];
    if (tbody) {
      Array.from(tbody.rows).forEach(r=>{
        const td = document.createElement('td');
        td.className = 'p-2 border text-right';
        if (insertPos >= r.cells.length) r.appendChild(td);
        else r.insertBefore(td, r.cells[insertPos]);
      });
    }
    return insertPos;
  }
  function renameHeader(table, idx, newText){
    const thead = table.tHead || table.querySelector('thead');
    if (thead && thead.rows[0] && thead.rows[0].cells[idx]) {
      thead.rows[0].cells[idx].textContent = newText;
    }
  }
  function deleteColumn(table, idx){
    const thead = table.tHead || table.querySelector('thead');
    if (thead) for (const row of thead.rows) if (idx < row.cells.length) row.deleteCell(idx);
    for (const tb of table.tBodies) for (const row of tb.rows) if (idx < row.cells.length) row.deleteCell(idx);
  }

  // Chỉ can thiệp các bảng Part 1 (nhận diện qua có cột nửa năm/quý/quy năm)
  function enhancePart1(){
    const freq = getFreq();
    const shouldHighlight = (freq !== 'year');

    $$$('table').forEach(table=>{
      const thead0 = table.tHead || table.querySelector('thead');
      if (!thead0 || !thead0.rows[0]) return;

      // bàn tay nhẹ: chỉ xử lý bảng nào có ít nhất một trong các cột kỳ/annual
      const rawHeaders = Array.from(thead0.rows[0].cells).map(c=>norm(c.textContent));
      const relevant = rawHeaders.some(h =>
        NAMES.half.concat(NAMES.quarter, NAMES.baseline, NAMES.annualized, NAMES.diff).includes(h)
      );
      if (!relevant) return;

      // map ban đầu
      let map = headerIndexMap(table);
      let idxHalf = findIdx(map, NAMES.half);
      let idxQuarter = findIdx(map, NAMES.quarter);
      let idxBaseline = findIdx(map, NAMES.baseline);
      let idxAnnualized = findIdx(map, NAMES.annualized);
      let idxDiff = findIdx(map, NAMES.diff);

      // gộp cột baseline: nếu tồn tại cả “Phí đóng (quy năm)” và “Phí đóng theo năm”, giữ “theo năm”
      const idxParenAnnual = rawHeaders.indexOf('phí đóng (quy năm)');
      const idxTheoNam = rawHeaders.indexOf('phí đóng theo năm');
      if (idxParenAnnual !== -1 && idxTheoNam !== -1 && idxParenAnnual !== idxTheoNam) {
        deleteColumn(table, idxParenAnnual);
      }

      // cập nhật lại map
      map = headerIndexMap(table);
      idxHalf = findIdx(map, NAMES.half);
      idxQuarter = findIdx(map, NAMES.quarter);
      idxBaseline = findIdx(map, NAMES.baseline);
      idxAnnualized = findIdx(map, NAMES.annualized);
      idxDiff = findIdx(map, NAMES.diff);

      // đảm bảo baseline tồn tại & đúng nhãn “Phí đóng theo năm”
      let afterIdx = Math.max(idxHalf, idxQuarter);
      if (idxBaseline < 0) idxBaseline = ensureHeader(table, 'Phí đóng theo năm', afterIdx);
      else renameHeader(table, idxBaseline, 'Phí đóng theo năm');

      // đảm bảo cột annualized & chênh lệch
      if (idxAnnualized < 0) idxAnnualized = ensureHeader(table, 'Phí đóng quy năm', idxBaseline);
      if (idxDiff < 0) idxDiff = ensureHeader(table, 'Chênh lệch', idxAnnualized);

      const tbody = table.tBodies[0];
      if (!tbody) return;

      let sumBaseline=0, sumAnnualized=0, sumDiff=0;

      Array.from(tbody.rows).forEach(r=>{
        const first = (r.cells[0]?.textContent || '');
        const isTotal = /tổng/i.test(first);

        const vHalf = (idxHalf>=0 && r.cells[idxHalf]) ? parseVnd(r.cells[idxHalf].textContent) : null;
        const vQuarter = (idxQuarter>=0 && r.cells[idxQuarter]) ? parseVnd(r.cells[idxQuarter].textContent) : null;
        let vBaseline = (idxBaseline>=0 && r.cells[idxBaseline]) ? parseVnd(r.cells[idxBaseline].textContent) : 0;

        // nếu baseline rỗng/0 mà có kỳ nửa năm/quý thì suy ngược
        if (!vBaseline) {
          if (vHalf != null && vHalf > 0) vBaseline = vHalf * 2;
          else if (vQuarter != null && vQuarter > 0) vBaseline = vQuarter * 4;
        }

        // quy năm theo kỳ chọn
        let vEq = vBaseline;
        if (freq === 'half') vEq = (vHalf != null && vHalf > 0) ? vHalf*2 : vBaseline;
        else if (freq === 'quarter') vEq = (vQuarter != null && vQuarter > 0) ? vQuarter*4 : vBaseline;

        // ghi lại
        if (idxBaseline>=0 && r.cells[idxBaseline]) r.cells[idxBaseline].textContent = fmtVnd(vBaseline);
        if (idxAnnualized>=0 && r.cells[idxAnnualized]) r.cells[idxAnnualized].textContent = fmtVnd(vEq);

        const diff = (vBaseline||0) - (vEq||0);
        if (idxDiff>=0 && r.cells[idxDiff]) {
          r.cells[idxDiff].textContent = fmtVnd(diff);
          if (!isTotal && shouldHighlight && diff !== 0) {
            r.cells[idxDiff].style.color = '#d00';
            r.cells[idxDiff].style.fontWeight = '700';
          } else {
            r.cells[idxDiff].style.color = '';
            r.cells[idxDiff].style.fontWeight = '';
          }
        }

        if (!isTotal) {
          sumBaseline += vBaseline||0;
          sumAnnualized += vEq||0;
          sumDiff += diff||0;
        }
      });
      ...
      const hasTotal = last && /tổng/i.test((last.cells[0]?.textContent||''));
      const row = hasTotal ? last : document.createElement('tr');
      if (!hasTotal){
        const cols = (table.tHead && table.tHead.rows[0]) ? table.tHead.rows[0].cells.length : (tbody.rows[0]?.cells.length || 1);
        for (let i=0;i<cols;i++){
          const td = document.createElement('td');
          td.className = 'p-2 border text-right';
          row.appendChild(td);
      ...
      if (row.cells[0]) row.cells[0].textContent = 'Tổng cộng';
      if (idxBaseline>=0 && row.cells[idxBaseline]) row.cells[idxBaseline].textContent = fmtVnd(sumBaseline);
      if (idxAnnualized>=0 && row.cells[idxAnnualized]) row.cells[idxAnnualized].textContent = fmtVnd(sumAnnualized);
      if (idxDiff>=0 && row.cells[idxDiff]) {
        row.cells[idxDiff].textContent = fmtVnd(sumDiff);
        if (shouldHighlight && sumDiff !== 0) {
          row.cells[idxDiff].style.color = '#d00';
          row.cells[idxDiff].style.fontWeight = '700';
        } else {
          row.cells[idxDiff].style.color = '';
          row.cells[idxDiff].style.fontWeight = '';
        }
      }
    });
  }

  function run(){ enhancePart1(); }
  function bind(){
    const btn = $$('#view-summary-btn');
    if (btn && !btn.__p1Enh) { btn.__p1Enh = true; btn.addEventListener('click', ()=>setTimeout(run,0)); }
    $$$('input[name="payment-frequency"], .payment-frequency').forEach(el=>{
      if (!el.__p1Enh) {
        el.__p1Enh = true;
        el.addEventListener('change', ()=>setTimeout(run,0));
        el.addEventListener('click', ()=>setTimeout(run,0));
      }
    });
  }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', ()=>{ bind(); run(); });
  else { bind(); run(); }

  // nếu modal render động, bind lại
