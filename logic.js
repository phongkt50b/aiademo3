...
const MAX_STBH = {
  // Số tiền bảo hiểm tối đa cho các sản phẩm; dùng số nguyên thuần để tránh lỗi cú pháp do numeric separators
  bhn: 5000000000,
  accident: 8000000000
};
...
function updatePaymentFrequencyOptions(baseMainAnnual) {
  ...
  // Dùng số nguyên thuần (không dùng numeric separators) để tránh lỗi cú pháp trên một số môi trường trình duyệt cũ
...
function calculateBhnPremium(customer, container, ageOverride = null) {
  ...
  // So sánh giới hạn STBH bằng số nguyên thuần để tránh lỗi do numeric separators
...
function calculateAccidentPremium(customer, container, ageOverride = null) {
  ...
  // Sử dụng số nguyên thuần thay cho numeric separators (10_000_000 -> 10000000)
...
function calculateHospitalSupportPremium(customer, mainPremium, container, totalHospitalSupportStbh = 0, ageOverride = null) {
  ...
  // Không dùng numeric separators ở đây để tương thích với trình duyệt cũ
...
function generateSummaryTable() {
  ...
  try {
    ...
    try {
      if (window.MDP3 && (document.getElementById('mdp3-enable')?.checked)) {
        ...
        if (fee > 0) {
          ...
          // Nếu chọn 'Người khác', sử dụng tuổi của NĐBH "khác" để tính toán như một NĐBH bổ sung
          let mdpStartAge;
          if (selId === 'main-person-container') {
            mdpStartAge = mainPersonInfo.age;
          } else if (selId === 'other') {
            const otherInfo = getCustomerInfo(document.getElementById('person-container-mdp3-other'), false) || {};
            mdpStartAge = otherInfo.age || mainPersonInfo.age;
          } else {
            const supp = suppPersons.find(x => x.container?.id === selId);
            mdpStartAge = supp ? supp.age : mainPersonInfo.age;
          }
    ...
    // Cột phí đóng quy năm (gấp số kỳ lên) luôn hiển thị
    summaryHtml += `<th class="p-2 border">Phí đóng (quy năm)</th>`;
    // Cột phí đóng theo năm (base annual) luôn hiển thị
    summaryHtml += `<th class="p-2 border">Phí đóng (năm)</th>`;
    // Luôn hiển thị cột chênh lệch, áp dụng màu đỏ cho giá trị dương
    summaryHtml += `<th class="p-2 border">Chênh lệch</th>`;
    summaryHtml += `</tr></thead><tbody>`;

    // Dòng tổng cho từng người
    personsList.forEach(person => {
      // Tổng cộng cho từng người: hiển thị tổng phí đóng theo kỳ (perPeriod) ở cột thứ hai
      // Mặc định tổng perPeriod có 6 cột còn lại: STBH, Số năm đóng phí, Phí theo kỳ, Phí quy năm, Phí theo năm, Chênh lệch
      const colspan = 6;
      ...
      person.products.forEach(pr => {
        ...
        // Phí đóng theo kỳ (perPeriod)
        summaryHtml += `<td class="p-2 border text-right">${formatCurrency(pr.perPeriod)}</td>`;
        // Phí đóng quy năm = phí theo kỳ nhân số kỳ
        summaryHtml += `<td class="p-2 border text-right">${formatCurrency(pr.annualEq)}</td>`;
        // Phí đóng theo năm (base annual)
        summaryHtml += `<td class="p-2 border text-right">${formatCurrency(pr.baseAnnual)}</td>`;
        // Cột chênh lệch: bôi đậm và đỏ nếu có chênh lệch dương
        summaryHtml += `<td class="p-2 border text-right">${pr.diff ? `<span class="font-semibold text-red-600">${formatCurrency(pr.diff)}</span>` : ''}</td>`;
        summaryHtml += `</tr>`;
      });
    });
    ...
    // Khởi tạo biến cộng dồn cho dòng tổng cuối bảng phí
    let totalAnnualEqSum = 0;
    let totalBaseAnnualSum = 0;
    let totalDiffSum = 0;
    for (let i = 0; (mainPersonInfo.age + i) <= targetAge; i++) {
      ...
      // ===== Tính phí theo kỳ và quy năm cho từng đối tượng =====
      // Phí sản phẩm chính và phí đóng thêm cho NĐBH chính (theo kỳ)
      const perPeriodMainBase = (periods === 1) ? mainBaseThisYear : (mainBaseThisYear / periods);
      const perPeriodMainExtra = (periods === 1) ? extraThisYear : (extraThisYear / periods);
      ...
      let perPeriodMainSupp = 0, annualEqMainSupp = 0, diffMainSupp = 0;
      if (periods === 1) {
      ...
      // Phí cho từng NĐBH bổ sung (bao gồm MDP3)
      const perPeriodSupps = [];
      const annualEqSupps = [];
      const diffSupps = [];
      suppAnnuals.forEach((baseAnnual, idx) => {
        const totalBase = baseAnnual + suppMdp3Annuals[idx];
        if (periods === 1) {
          perPeriodSupps[idx] = totalBase;
          annualEqSupps[idx] = totalBase;
          diffSupps[idx] = 0;
        } else {
          const perP = Math.round((totalBase / 1000 * factor / periods)) * 1000;
          perPeriodSupps[idx] = perP;
          const annEq = perP * periods;
          annualEqSupps[idx] = annEq;
          diffSupps[idx] = annEq - totalBase;
        }
      });
      // Tổng hợp phí quy năm và theo năm cho cả năm hợp đồng
      const totalAnnualEq = annualEqMainBase + annualEqMainExtra + annualEqMainSupp + annualEqSupps.reduce((a,b) => a + b, 0);
      const baseAnnualTotal = mainBaseThisYear + extraThisYear + baseMainSuppAnnual + suppAnnuals.reduce((a,b) => a + b, 0) + suppMdp3Annuals.reduce((a,b) => a + b, 0);
      const totalDiff = totalAnnualEq - baseAnnualTotal;
      // Render row với phí theo kỳ cho từng đối tượng, và tổng/quy năm/chênh lệch
      tableHtml += `<tr>`;
      tableHtml += `<td class="p-2 border text-center">${contractYear}</td>`;
      tableHtml += `<td class="p-2 border text-center">${currentAgeMain}</td>`;
      // Phí sản phẩm chính (theo kỳ)
      tableHtml += `<td class="p-2 border text-right">${perPeriodMainBase ? formatCurrency(perPeriodMainBase) : ''}</td>`;
      // Phí đóng thêm (theo kỳ) nếu có
      if (hasExtraMain) {
        tableHtml += `<td class="p-2 border text-right">${perPeriodMainExtra ? formatCurrency(perPeriodMainExtra) : ''}</td>`;
      }
      // Phí sản phẩm bổ sung (bao gồm MDP3) cho NĐBH chính (theo kỳ) nếu có
      if (hasSuppMain) {
        tableHtml += `<td class="p-2 border text-right">${perPeriodMainSupp ? formatCurrency(perPeriodMainSupp) : ''}</td>`;
      }
      // Phí cho từng NĐBH bổ sung (theo kỳ)
      perPeriodSupps.forEach(val => {
        tableHtml += `<td class="p-2 border text-right">${val ? formatCurrency(val) : ''}</td>`;
      });
      // Tổng cộng quy năm cho tất cả NĐBH
      tableHtml += `<td class="p-2 border text-right font-semibold">${formatCurrency(totalAnnualEq)}</td>`;
      // Tổng cộng nếu đóng theo năm (base annual)
      tableHtml += `<td class="p-2 border text-right font-semibold">${formatCurrency(baseAnnualTotal)}</td>`;
      // Chênh lệch: bôi đỏ nếu dương
      tableHtml += `<td class="p-2 border text-right">${totalDiff > 0 ? `<span class="font-semibold text-red-600">${formatCurrency(totalDiff)}</span>` : ''}</td>`;
      tableHtml += `</tr>`;

      // Cộng dồn cho dòng tổng
      totalAnnualEqSum += totalAnnualEq;
      totalBaseAnnualSum += baseAnnualTotal;
      totalDiffSum += totalDiff;
    }
    // Sau khi lặp xong các năm hợp đồng, thêm dòng tổng cuối
    // Tính số cột trước các cột tổng (Năm HĐ, Tuổi, các cột phí theo kỳ)
    let colBeforeTotals = 2 + 1; // Năm HĐ, Tuổi, Phí sản phẩm chính
    if (hasExtraMain) colBeforeTotals += 1;
    if (hasSuppMain) colBeforeTotals += 1;
    colBeforeTotals += tablePersons.length; // cột cho từng NĐBH bổ sung
    tableHtml += `<tr class="bg-gray-100 font-semibold">`;
    tableHtml += `<td class="p-2 border text-center" colspan="${colBeforeTotals}">Tổng cộng</td>`;
    tableHtml += `<td class="p-2 border text-right font-semibold">${formatCurrency(totalAnnualEqSum)}</td>`;
    tableHtml += `<td class="p-2 border text-right font-semibold">${formatCurrency(totalBaseAnnualSum)}</td>`;
    tableHtml += `<td class="p-2 border text-right">${totalDiffSum > 0 ? `<span class="font-semibold text-red-600">${formatCurrency(totalDiffSum)}</span>` : ''}</td>`;
    tableHtml += `</tr>`;
...
function getHealthSclStbhByProgram(program) {
  switch (program) {
    case 'co_ban': return 100000000;
    case 'nang_cao': return 250000000;
    case 'toan_dien': return 500000000;
    case 'hoan_hao': return 1000000000;
    default: return 0;
  }
}
...
(function() {
  ...
  function updatePeriodBreakdown() {
    ...
    // Ẩn chênh lệch nếu 0 và làm nổi bật chênh lệch dương
    const diffEl = $$("#freq-diff");
    if (diffEl) {
      const row = diffEl.closest("div");
      if (row) row.classList.toggle("hidden", diff === 0);
      // Nếu có chênh lệch dương, tô đỏ và bôi đậm, ngược lại bỏ định dạng
      if (diff > 0) {
        diffEl.classList.add('font-semibold');
        diffEl.classList.add('text-red-600');
      } else {
        diffEl.classList.remove('font-semibold');
