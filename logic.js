...
function calculateAll() {
  try {
    ...
    // Add Miễn đóng phí 3.0 premium into corresponding person's supplement
    if (window.MDP3) {
      const mdp3Fee = Number(MDP3.getPremium() || 0);
      const selId = (MDP3.getSelectedId && MDP3.getSelectedId()) ||
                    (document.getElementById('mdp3-person-select')?.value || '');
      if (mdp3Fee > 0) {
        if (selId && selId !== 'other') {
          // assign MDP3 fee to selected person's supplementary fees
          if (!window.personFees[selId]) {
            window.personFees[selId] = { main: 0, mainBase: 0, supp: 0, total: 0 };
          }
          window.personFees[selId].supp += mdp3Fee;
          window.personFees[selId].total = (window.personFees[selId].main || 0) + window.personFees[selId].supp;
          totalSupplementaryPremium += mdp3Fee;
        } else {
          // "other": fee counted only in total, not added to any person
          totalSupplementaryPremium += mdp3Fee;
        }
      }
    }
...
function renderSuppListSimple() {
  ...
  // Main insured supplementary fee
  ...
  // Supplementary insured persons – show only if supplementary fee > 0
  ...
  // If Miễn đóng phí 3.0 is assigned to “other”, display its row
  try {
    if (window.MDP3) {
      ...
      if (selId === 'other' && fee > 0) {
        // show the name from the MDP3 other person input if available
        let otherName = 'Người khác';
        const otherForm = document.getElementById('person-container-mdp3-other');
        if (otherForm) {
          const nm = otherForm.querySelector('.name-input')?.value;
          if (nm && nm.trim()) otherName = nm.trim();
        }
        const row = document.createElement('div');
        row.className = 'flex justify-between items-center py-1 text-sm';
        row.innerHTML = `<span>Miễn đóng phí 3.0 (${sanitizeHtml(otherName)})</span><span class="font-semibold">${formatCurrency(fee)}</span>`;
        wrap.appendChild(row);
      }
    }
  } catch (e) {}
...
// === Custom summary generator (Phần 1 & Phần 2) ===
// This override builds a concise product summary and fee table according to user requirements.
// It computes premiums per selected payment frequency, hides rows with zero values, and includes Miễn đóng phí 3.0 details.
window.generateSummaryTable = function() {
  ...
  try {
    ...
    const baseAnnual = Number(last.baseMainPremium || 0);
    const extraAnnual = Number(last.extraPremium || 0);
    const mainSuppAnnual = pf['main-person-container'] ? Number(pf['main-person-container'].supp || 0) : 0;
    ...
    // Target age
    let targetAge = parseInt(document.getElementById('target-age-input')?.value || '0', 10);
    if (!targetAge || targetAge <= 0) {
      targetAge = mainInfo.age + paymentTerm - 1;
    }
    const freqSel = document.getElementById('payment-frequency');
    const freq = freqSel ? freqSel.value : 'year';
    const periods = freq === 'half' ? 2 : (freq === 'quarter' ? 4 : 1);
    ...
    let mdp3Fee = 0;
    let mdp3Enabled = false;
    try {
      mdp3Enabled = !!(document.getElementById('mdp3-enable')?.checked);
      if (mdp3Enabled && window.MDP3) {
    ...
    // Compute STBH base for MDP3
    let mdp3StbhBase = 0;
    if (mdp3Enabled && mdp3Fee > 0) {
      for (const pid in pf) {
        const obj = pf[pid];
        mdp3StbhBase += Number(obj.mainBase || 0) + Number(obj.supp || 0);
      }
    }
    // Build persons list
    const persons = [];
    persons.push({
      ...
      name: mainInfo.name || 'NĐBH chính',
      age: mainInfo.age,
      baseAnnual: baseAnnual,
      extraAnnual: extraAnnual,
      suppAnnual: mainSuppAnnual,
      mdp3Annual: (mdp3Enabled && mdp3SelectedId === 'main-person-container') ? mdp3Fee : 0,
    ...
    document.querySelectorAll('#supplementary-insured-container .person-container').forEach(cont => {
      const info = getCustomerInfo(cont, false);
      const supp = pf[cont.id] ? Number(pf[cont.id].supp || 0) : 0;
      const mdpPart = (mdp3Enabled && mdp3SelectedId === cont.id) ? mdp3Fee : 0;
      persons.push({
        id: cont.id,
        name: info.name || 'NĐBH bổ sung',
        age: info.age,
        baseAnnual: 0,
        extraAnnual: 0,
        suppAnnual: supp,
    ...
    // mdp3-other
    if (mdp3Enabled && mdp3SelectedId === 'other' && mdp3Fee > 0) {
      let otherName = 'Người khác';
      const otherForm = document.getElementById('person-container-mdp3-other');
      if (otherForm) {
        const nm = otherForm.querySelector('.name-input')?.value;
    ...
    // Helper functions
    function calcSuppPerPeriod(annual) {
      if (periods === 1) return annual;
      return Math.round(((annual / 1000) * factor / periods)) * 1000;
    }
    function calcBasePerPeriod(annual) {
      if (periods === 1) return annual;
      return Math.floor((annual / periods) / 1000) * 1000;
    }
    // Build Phần 1 summary table
    let summaryHtml = `<div class="mb-4"><div class="text-lg font-semibold mb-2">Phần 1 · Tóm tắt sản phẩm</div>`;
    summaryHtml += `<div class="text-sm mb-2 italic text-gray-600">Các phí dưới đây được tính theo kỳ: ${freq === 'year' ? 'Năm' : (freq === 'half' ? 'Nửa năm' : 'Quý')}</div>`;
    summaryHtml += `<table class="w-full text-left border-collapse"><thead class="bg-gray-100"><tr>`;
    summaryHtml += `<th class="p-2 border">Họ tên</th><th class="p-2 border">Sản phẩm</th><th class="p-2 border text-center">Số năm đóng phí</th><th class="p-2 border text-right">Phí đóng</th>`;
    summaryHtml += `</tr></thead><tbody>`;
    // For each person
    persons.forEach(person => {
      const suppAnnual = Number(person.suppAnnual || 0);
      const mdpAnnual = Number(person.mdp3Annual || 0);
      const basePer = calcBasePerPeriod(person.baseAnnual);
      const extraPer = calcBasePerPeriod(person.extraAnnual);
      const suppPer = calcSuppPerPeriod(suppAnnual + mdpAnnual);
      const aggregatedPer = basePer + extraPer + suppPer;
      // Aggregated row
      summaryHtml += `<tr><td class="p-2 border font-semibold" colspan="3">${sanitizeHtml(person.name)}</td><td class="p-2 border font-semibold text-right">${formatCurrency(aggregatedPer)}</td></tr>`;
      if (!person.isOtherMdp) {
        // Main person specific rows
        if (person.isMain) {
          // Main product row
          let stbh = 0;
          const stbhEl = document.getElementById('main-stbh');
          if (stbhEl && stbhEl.value) stbh = parseFormattedNumber(stbhEl.value || '0');
          const productLabel = getProductLabel(mainInfo.mainProduct) || 'Sản phẩm chính';
          const baseRowPremium = basePer;
          const years = paymentTerm;
          summaryHtml += `<tr><td class="p-2 border"></td><td class="p-2 border">${sanitizeHtml(productLabel)}</td><td class="p-2 border text-center">${years}</td><td class="p-2 border text-right">${formatCurrency(baseRowPremium)}</td></tr>`;
          // Extra row
          if (extraAnnual > 0) {
            const extraRowPremium = extraPer;
            summaryHtml += `<tr><td class="p-2 border"></td><td class="p-2 border">Phí đóng thêm</td><td class="p-2 border text-center">${years}</td><td class="p-2 border text-right">${formatCurrency(extraRowPremium)}</td></tr>`;
          }
          // Supplementary products for main
          const suppContainer = document.querySelector('#main-supp-container .supplementary-products-container');
          if (suppContainer) {
            // Health SCL
            const sclSec = suppContainer.querySelector('.health-scl-section');
            if (sclSec && sclSec.querySelector('.health-scl-checkbox')?.checked) {
              const program = sclSec.querySelector('.health-scl-program')?.value || '';
              const programLabel = {co_ban:'Cơ bản', nang_cao:'Nâng cao', toan_dien:'Toàn diện', hoan_hao:'Hoàn hảo'}[program] || '';
              const feeAnn = calculateHealthSclPremium(mainInfo, suppContainer, mainInfo.age);
              const feePer = calcSuppPerPeriod(feeAnn);
              const yearsScl = Math.max(0, Math.min(targetAge, 75) - mainInfo.age + 1);
              summaryHtml += `<tr><td class="p-2 border"></td><td class="p-2 border">Sức khoẻ Bùng Gia Lực${programLabel? ' - ' + programLabel:''}</td><td class="p-2 border text-center">${yearsScl}</td><td class="p-2 border text-right">${formatCurrency(feePer)}</td></tr>`;
            }
            // BHN
            const bhnSec = suppContainer.querySelector('.bhn-section');
            if (bhnSec && bhnSec.querySelector('.bhn-checkbox')?.checked) {
              const feeAnn = calculateBhnPremium(mainInfo, suppContainer, mainInfo.age);
              const feePer = calcSuppPerPeriod(feeAnn);
              const yearsBhn = Math.max(0, Math.min(targetAge, 85) - mainInfo.age + 1);
              summaryHtml += `<tr><td class="p-2 border"></td><td class="p-2 border">Bệnh hiểm nghèo 2.0</td><td class="p-2 border text-center">${yearsBhn}</td><td class="p-2 border text-right">${formatCurrency(feePer)}</td></tr>`;
            }
            // Accident
            const accSec = suppContainer.querySelector('.accident-section');
            if (accSec && accSec.querySelector('.accident-checkbox')?.checked) {
              const feeAnn = calculateAccidentPremium(mainInfo, suppContainer, mainInfo.age);
              const feePer = calcSuppPerPeriod(feeAnn);
              const yearsAcc = Math.max(0, Math.min(targetAge, 65) - mainInfo.age + 1);
              summaryHtml += `<tr><td class="p-2 border"></td><td class="p-2 border">Bảo hiểm Tai nạn</td><td class="p-2 border text-center">${yearsAcc}</td><td class="p-2 border text-right">${formatCurrency(feePer)}</td></tr>`;
            }
            // Hospital support
            const hsSec = suppContainer.querySelector('.hospital-support-section');
            if (hsSec && hsSec.querySelector('.hospital-support-checkbox')?.checked) {
              const feeAnn = calculateHospitalSupportPremium(mainInfo, baseAnnual, suppContainer, 0, mainInfo.age);
              const feePer = calcSuppPerPeriod(feeAnn);
              const yearsHs = Math.max(0, Math.min(targetAge, 65) - mainInfo.age + 1);
              summaryHtml += `<tr><td class="p-2 border"></td><td class="p-2 border">Hỗ trợ chi phí nằm viện (đ/ngày)</td><td class="p-2 border text-center">${yearsHs}</td><td class="p-2 border text-right">${formatCurrency(feePer)}</td></tr>`;
            }
          }
          // MDP3 for main
          if (mdp3Enabled && mdp3SelectedId === 'main-person-container' && mdp3Fee > 0) {
            let mdpStbh = mdp3StbhBase;
            mdpStbh -= mainSuppAnnual;
            const mdpYears = Math.max(0, Math.min(targetAge, 65) - mainInfo.age + 1);
            const mdpPer = calcSuppPerPeriod(mdp3Fee);
            summaryHtml += `<tr><td class="p-2 border"></td><td class="p-2 border">Miễn đóng phí 3.0</td><td class="p-2 border text-center">${mdpYears}</td><td class="p-2 border text-right">${formatCurrency(mdpPer)}</td></tr>`;
          }
        } else {
          // Supplementary person rows
          const info = getCustomerInfo(person.container, false);
          const cont = person.container ? person.container.querySelector('.supplementary-products-container') : null;
          if (cont) {
            // Health SCL
            const sclSec2 = cont.querySelector('.health-scl-section');
            if (sclSec2 && sclSec2.querySelector('.health-scl-checkbox')?.checked) {
              const program = sclSec2.querySelector('.health-scl-program')?.value || '';
              const programLabel = {co_ban:'Cơ bản', nang_cao:'Nâng cao', toan_dien:'Toàn diện', hoan_hao:'Hoàn hảo'}[program] || '';
              const feeAnn = calculateHealthSclPremium(info, cont, info.age);
              const feePer = calcSuppPerPeriod(feeAnn);
              const yearsScl = Math.max(0, Math.min(targetAge, 75) - info.age + 1);
              summaryHtml += `<tr><td class="p-2 border"></td><td class="p-2 border">Sức khoẻ Bùng Gia Lực${programLabel? ' - ' + programLabel:''}</td><td class="p-2 border text-center">${yearsScl}</td><td class="p-2 border text-right">${formatCurrency(feePer)}</td></tr>`;
            }
            // BHN
            const bhnSec2 = cont.querySelector('.bhn-section');
            if (bhnSec2 && bhnSec2.querySelector('.bhn-checkbox')?.checked) {
              const feeAnn = calculateBhnPremium(info, cont, info.age);
              const feePer = calcSuppPerPeriod(feeAnn);
              const yearsBhn = Math.max(0, Math.min(targetAge, 85) - info.age + 1);
              summaryHtml += `<tr><td class="p-2 border"></td><td class="p-2 border">Bệnh hiểm nghèo 2.0</td><td class="p-2 border text-center">${yearsBhn}</td><td class="p-2 border text-right">${formatCurrency(feePer)}</td></tr>`;
            }
            // Accident
            const accSec2 = cont.querySelector('.accident-section');
            if (accSec2 && accSec2.querySelector('.accident-checkbox')?.checked) {
              const feeAnn = calculateAccidentPremium(info, cont, info.age);
              const feePer = calcSuppPerPeriod(feeAnn);
              const yearsAcc = Math.max(0, Math.min(targetAge, 65) - info.age + 1);
              summaryHtml += `<tr><td class="p-2 border"></td><td class="p-2 border">Bảo hiểm Tai nạn</td><td class="p-2 border text-center">${yearsAcc}</td><td class="p-2 border text-right">${formatCurrency(feePer)}</td></tr>`;
            }
            // Hospital
            const hsSec2 = cont.querySelector('.hospital-support-section');
            if (hsSec2 && hsSec2.querySelector('.hospital-support-checkbox')?.checked) {
              const feeAnn = calculateHospitalSupportPremium(info, baseAnnual, cont, 0, info.age);
              const feePer = calcSuppPerPeriod(feeAnn);
              const yearsHs = Math.max(0, Math.min(targetAge, 65) - info.age + 1);
              summaryHtml += `<tr><td class="p-2 border"></td><td class="p-2 border">Hỗ trợ chi phí nằm viện (đ/ngày)</td><td class="p-2 border text-center">${yearsHs}</td><td class="p-2 border text-right">${formatCurrency(feePer)}</td></tr>`;
            }
          }
          // MDP3 for supplementary person
          if (mdp3Enabled && mdp3SelectedId === person.id && mdp3Fee > 0) {
            let mdpStbh2 = mdp3StbhBase;
            mdpStbh2 -= Number(person.suppAnnual || 0);
            const mdpYears2 = Math.max(0, Math.min(targetAge, 65) - person.age + 1);
            const mdpPer2 = calcSuppPerPeriod(mdp3Fee);
            summaryHtml += `<tr><td class="p-2 border"></td><td class="p-2 border">Miễn đóng phí 3.0</td><td class="p-2 border text-center">${mdpYears2}</td><td class="p-2 border text-right">${formatCurrency(mdpPer2)}</td></tr>`;
          }
        }
      }
      // MDP3 other row (list product row) handled separately below
      persons.forEach(p => {
        if (p.isOtherMdp) {
          const mdpYearsOther = Math.max(0, Math.min(targetAge, 65) - mainInfo.age + 1);
          const mdpPerOther = calcSuppPerPeriod(p.mdp3Annual || 0);
          summaryHtml += `<tr><td class="p-2 border"></td><td class="p-2 border">Miễn đóng phí 3.0</td><td class="p-2 border text-center">${mdpYearsOther}</td><td class="p-2 border text-right">${formatCurrency(mdpPerOther)}</td></tr>`;
        }
      });
    summaryHtml += `</tbody></table></div>`;
    // Build Phần 2 fee table
    let tableHtml = `<div class="mb-4"><div class="text-lg font-semibold mb-2">Phần 2 · Bảng phí</div>`;
    tableHtml += `<table class="w-full text-left border-collapse"><thead class="bg-gray-100"><tr>`;
    tableHtml += `<th class="p-2 border">Năm HĐ</th>`;
    tableHtml += `<th class="p-2 border">Tuổi NĐBH chính<br>(${sanitizeHtml(mainInfo.name || 'NĐBH chính')})</th>`;
    tableHtml += `<th class="p-2 border">Phí chính</th>`;
    tableHtml += `<th class="p-2 border">Phí đóng thêm</th>`;
    tableHtml += `<th class="p-2 border">Phí bổ sung<br>(${sanitizeHtml(mainInfo.name || 'NĐBH chính')})</th>`;
    persons.forEach(p => {
      if (!p.isMain && !p.isOtherMdp) {
    ...
    tableHtml += `</tr></thead><tbody>`;
    // Determine maximum contract years
    const maxContractAge = targetAge;
    const mainSuppContainer2 = document.querySelector('#main-supp-container .supplementary-products-container');
    for (let i = 0; (mainInfo.age + i) <= maxContractAge; i++) {
      const currentAgeMain = mainInfo.age + i;
      const contractYear = i + 1;
      const mainThisYearBase = (contractYear <= paymentTerm) ? baseAnnual : 0;
      const extraThisYear = (contractYear <= paymentTerm) ? extraAnnual : 0;
      // Supplementary for main
      let suppMainYear = 0;
      let totalHospitalSupportStbh = 0;
      if (mainSuppContainer2) {
        suppMainYear += calculateHealthSclPremium({ ...mainInfo, age: currentAgeMain }, mainSuppContainer2, currentAgeMain);
        suppMainYear += calculateBhnPremium({ ...mainInfo, age: currentAgeMain }, mainSuppContainer2, currentAgeMain);
        suppMainYear += calculateAccidentPremium({ ...mainInfo, age: currentAgeMain }, mainSuppContainer2, currentAgeMain);
        suppMainYear += calculateHospitalSupportPremium({ ...mainInfo, age: currentAgeMain }, baseAnnual, mainSuppContainer2, totalHospitalSupportStbh, currentAgeMain);
      }
      // Supplementary for each supplementary person
      const suppYearValues = [];
      persons.forEach(p => {
        if (!p.isMain && !p.isOtherMdp) {
          const info = getCustomerInfo(p.container, false);
          const cont = p.container.querySelector('.supplementary-products-container');
          let suppVal = 0;
          if (cont) {
            suppVal += calculateHealthSclPremium({ ...info, age: info.age + i }, cont, info.age + i);
            suppVal += calculateBhnPremium({ ...info, age: info.age + i }, cont, info.age + i);
            suppVal += calculateAccidentPremium({ ...info, age: info.age + i }, cont, info.age + i);
            suppVal += calculateHospitalSupportPremium({ ...info, age: info.age + i }, baseAnnual, cont, totalHospitalSupportStbh, info.age + i);
          }
          suppYearValues.push(suppVal);
        }
      });
      const suppAnnualAll = suppMainYear + suppYearValues.reduce((s,v)=>s+v,0);
      const perMainExtra = (periods === 1) ? (mainThisYearBase + extraThisYear) : (calcBasePerPeriod(mainThisYearBase) + calcBasePerPeriod(extraThisYear));
      const perSupp = (periods === 1) ? suppAnnualAll : Math.round(((suppAnnualAll / 1000) * factor / periods)) * 1000;
      const perPeriodTotal = perMainExtra + perSupp;
      const totalFromPeriod = perPeriodTotal * periods;
      const diff = totalFromPeriod - (mainThisYearBase + extraThisYear + suppAnnualAll);
      tableHtml += `<tr><td class="p-2 border text-center">${contractYear}</td><td class="p-2 border text-center">${currentAgeMain}</td><td class="p-2 border text-right">${formatCurrency(mainThisYearBase)}</td><td class="p-2 border text-right">${formatCurrency(extraThisYear)}</td><td class="p-2 border text-right">${formatCurrency(suppMainYear)}</td>`;
      let idx = 0;
      persons.forEach(p => {
        if (!p.isMain && !p.isOtherMdp) {
          const v = suppYearValues[idx] || 0;
          tableHtml += `<td class="p-2 border text-right">${formatCurrency(v)}</td>`;
          idx++;
        }
      });
      tableHtml += `<td class="p-2 border text-right font-semibold">${formatCurrency(totalFromPeriod)}</td>`;
    ...
    tableHtml += `</tbody></table></div>`;
    // Combine summary and table and export button
    container.innerHTML = summaryHtml + tableHtml + `<div class="mt-4 text-center"><button id="export-html-btn" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">Xuất HTML</button></div>`;
    document.getElementById('export-html-btn').addEventListener('click', () => window.print());
  } catch (err) {
    container.innerHTML = `<p class="text-red-600 font-semibold text-center">${sanitizeHtml(err.message || String(err))}</p>`;
  } finally {
...
(function(){
  ...
  window.renderSection6 = function renderSection6(){
    try{
      ...
      // Determine if extra and supplementary rows should be hidden (value zero)
      const mainSuppVal = window.personFees?.['main-person-container'] ? (window.personFees['main-person-container'].supp || 0) : 0;
      const hideExtra = Number(extra) === 0;
      const hideMainSupp = Number(mainSuppVal) === 0;
      ...
      html += `<div class="py-2 border-b">
        ...
        <div class="mt-2 text-sm text-gray-700 pl-2">
          ...
          <div class="flex justify-between ${hideExtra ? 'hidden' : ''}"><span>Phí đóng thêm:</span><span>${formatCurrency(extra)}</span></div>
          <div class="flex justify-between ${hideMainSupp ? 'hidden' : ''}"><span>Phí sản phẩm bổ sung (NĐBH chính):</span><span>${formatCurrency(mainSuppVal)}</span></div>
        </div>
      </div>`;
      ...
      suppPersons.forEach((p, idx) => {
        ...
        if (Number(fee) > 0) {
          suppLines.push(`<div class="flex justify-between items-center py-1"><span class="text-sm">${sanitizeHtml(name)}</span><span class="font-semibold">${formatCurrency(fee)}</span></div>`);
        }
      });
      // If MDP3 is assigned to "other", append a row for that too
      try {
        if (window.MDP3) {
          const selId = window.MDP3.getSelectedId && window.MDP3.getSelectedId();
          const fee = Number(window.MDP3.getPremium ? (window.MDP3.getPremium() || 0) : 0);
          if (selId === 'other' && fee > 0) {
            ...
            suppLines.push(`<div class="flex justify-between items-center py-1"><span class="text-sm">Miễn đóng phí 3.0 (${sanitizeHtml(otherName)})</span><span class="font-semibold">${formatCurrency(fee)}</span></div>`);
          }
        }
      } catch (e) {}
      ...
      const hideExtraTotal = Number(extra) === 0;
      const hideSuppTotal = Number(suppTotal) === 0;
      html += `<div class="py-2 border-b mt-2">
        ...
        <div class="text-sm text-gray-600 mt-2">
          ...
          <div class="${hideExtraTotal ? 'hidden' : ''}">+ Phí đóng thêm: ${formatCurrency(extra)}</div>
          <div class="${hideSuppTotal ? 'hidden' : ''}">+ Phí sản phẩm bổ sung: ${formatCurrency(suppTotal)}</div>
        </div>
      </div>`;
      ...
      if(freqInfo.periods === 1){
        ...
        // Do not show the label "Kỳ:"; directly render the breakdown grid
...
(() => {
  ...
  function readAnnuals(){
    /*
     * Compute annual premium components from window.personFees.
     * Base and extra premiums come directly from the main insured.
     * Supplementary premiums are the sum of each person's supplementary fees (already
     * including Miễn đóng phí 3.0 where assigned to a specific person).
     * If MDP3 is assigned to "other" (id === 'other'), its premium is not included in
     * any person's supplementary total, so we need to add it to the overall
     * supplementary total here. Do not double-count MDP3 when assigned to
     * main or supplementary persons since calculateAll already merged it into
     * their pf.supp values.
     */
    const pf = (window.personFees)||{};
    const mainPf = pf['main-person-container']||{mainBase:0, supp:0};
    const base = Number(mainPf.mainBase||0);
    const extra = (typeof getExtraPremiumValue==='function') ? Number(getExtraPremiumValue()||0) : 0;
    // Start with main person's supplementary
    let suppTotal = Number(mainPf.supp||0);
    // accumulate supplementary persons' supplementary fees
    document.querySelectorAll('#supplementary-insured-container .person-container').forEach(cont => {
      const id = cont.id;
      const fee = pf[id] ? Number(pf[id].supp||0) : 0;
      suppTotal += fee;
    });
    // If MDP3 is assigned to "other", add its premium to the total here
    try{
      if(window.MDP3){
        const selId = window.MDP3.getSelectedId ? window.MDP3.getSelectedId() :
                      (document.getElementById('mdp3-person-select')?.value||null);
        const mdp3Fee = Number(window.MDP3.getPremium ? (window.MDP3.getPremium()||0) : 0);
        if(mdp3Fee>0 && selId === 'other'){
          suppTotal += mdp3Fee;
        }
      }
    }catch(e){}
  ...
  function renderSection6V2(){
    try{
      ...
      if(list){
        list.innerHTML = '';
        // Build rows only for supplementary persons with non-zero supplementary fees
        ...
        // If Miễn đóng phí 3.0 (MDP3) is assigned to "other", include it as a separate line
        try{
          if(window.MDP3){
            const selId = window.MDP3.getSelectedId ? window.MDP3.getSelectedId() :
                          (document.getElementById('mdp3-person-select')?.value||null);
            const fee = Number(window.MDP3.getPremium ? (window.MDP3.getPremium()||0) : 0);
            if(selId==='other' && fee>0){
              // Get the name of the "other" person from the temporary form if available
              let otherName = 'Người khác';
              const otherForm = document.getElementById('person-container-mdp3-other');
              if(otherForm){
                const nm = otherForm.querySelector('.name-input')?.value;
              ...
              const row = document.createElement('div');
              row.className = 'flex justify-between items-center py-1 text-sm';
              row.innerHTML = `<span>Phí MDP 3.0 (${sanitizeHtml(otherName)}):</span><span class="font-semibold">${fmt(fee)}</span>`;
              list.appendChild(row);
            }
          }
        }catch(e){}
      ...
      // Hide rows for extra premium and supplementary fee when value is zero
      try {
        const extraEl = document.getElementById('main-insured-extra-fee');
        if(extraEl){
          const row = extraEl.closest('li');
          if(row) row.classList.toggle('hidden', Number(extra) === 0);
        }
        // For main supplementary fee row in group (if exists)
        const mainSuppEl = document.getElementById('main-insured-supp-fee');
        if(mainSuppEl){
          const row = mainSuppEl.closest('li');
          const pf = (window.personFees||{})['main-person-container']||{supp:0};
          const suppVal = Number(pf.supp||0);
          if(row) row.classList.toggle('hidden', suppVal === 0);
        }
        // Hide total supplementary line in summary totals when suppTotal is zero
        const sumSuppRow = document.getElementById('summary-supp-fee')?.closest('li');
        if(sumSuppRow) sumSuppRow.classList.toggle('hidden', Number(suppTotal) === 0);
      } catch(e) {}
