...
function generateSummaryTable() {
  ...
  try {
    ...
    const freqLabel = freq === 'half' ? 'nửa năm' : (freq === 'quarter' ? 'theo quý' : 'năm');
    const factor = freq === 'half' ? 1.02 : (freq === 'quarter' ? 1.04 : 1.0);

    const MAX_AGE_BY_PRODUCT = { scl: 74, bhn: 85, accident: 64, hospital: 64, mdp3: 64 };

    const roundDownTo1000_local = (n)=>{ n=Number(n)||0; if(n<=0) return 0; return Math.floor(n/1000)*1000; };

    function perPeriodMainExtra(annual){
      if(periods===1) return annual;
      return roundDownTo1000_local(annual/periods);
    }
    function perPeriodSupp(annual){
      if(periods===1) return annual;
      const perUnits = Math.round((Number(annual)||0)/1000 * factor / periods);
      return perUnits*1000;
    }
    function diffFromAnnual(perPeriod, annual){
      return perPeriod*periods - (Number(annual)||0);
    }
    function yearsForSupplement(isMain, personAge, productKey, mainAge){
      const maxAge = MAX_AGE_BY_PRODUCT[productKey];
      if(isMain){
        return Math.max(0, Math.min(targetAge, maxAge) - personAge + 1);
      } else {
        const transAgeTarget = targetAge - mainAge + personAge; // chuyển về mốc tuổi của NĐBH bổ sung
        return Math.max(0, Math.min(transAgeTarget, maxAge) - personAge + 1);
      }
    }

    function computeSuppRows(personInfo, isMain){
      const rows = [];
      const cont = isMain ? document.querySelector('#main-supp-container .supplementary-products-container') : personInfo.container.querySelector('.supplementary-products-container');
      if(!cont) return { rows, total: 0 };
      let total = 0;

      // Sức khỏe Bùng Gia Lực
      const scl = cont.querySelector('.health-scl-section');
      if(scl && scl.querySelector('.health-scl-checkbox')?.checked){
        ...
        const annual = calculateHealthSclPremium(personInfo, cont, personInfo.age);
        const yrs = yearsForSupplement(isMain, personInfo.age, 'scl', mainPersonInfo.age);
        total += annual;
        rows.push({product: `Sức khỏe Bùng Gia Lực${programLabel?` - ${programLabel}`:''}`, stbh, years: yrs, annual, isSupp:true});
      }
      // BHN
      const bhn = cont.querySelector('.bhn-section');
      if(bhn && bhn.querySelector('.bhn-checkbox')?.checked){
        const stbh = parseFormattedNumber(bhn.querySelector('.bhn-stbh')?.value || '0');
        const annual = calculateBhnPremium(personInfo, cont, personInfo.age);
        const yrs = yearsForSupplement(isMain, personInfo.age, 'bhn', mainPersonInfo.age);
        total += annual;
        rows.push({product: 'Bệnh hiểm nghèo 2.0', stbh, years: yrs, annual, isSupp:true});
      }
      // Tai nạn
      const acc = cont.querySelector('.accident-section');
      if(acc && acc.querySelector('.accident-checkbox')?.checked){
        const stbh = parseFormattedNumber(acc.querySelector('.accident-stbh')?.value || '0');
        const annual = calculateAccidentPremium(personInfo, cont, personInfo.age);
        const yrs = yearsForSupplement(isMain, personInfo.age, 'accident', mainPersonInfo.age);
        total += annual;
        rows.push({product: 'Bảo hiểm Tai nạn', stbh, years: yrs, annual, isSupp:true});
      }
      // Hỗ trợ chi phí nằm viện
      const hs = cont.querySelector('.hospital-support-section');
      if(hs && hs.querySelector('.hospital-support-checkbox')?.checked){
        const stbh = parseFormattedNumber(hs.querySelector('.hospital-support-stbh')?.value || '0');
        const annual = calculateHospitalSupportPremium(personInfo, (window.lastSummaryPrem?.baseMainPremium)||0, cont, 0, personInfo.age);
        const yrs = yearsForSupplement(isMain, personInfo.age, 'hospital', mainPersonInfo.age);
        total += annual;
        rows.push({product: 'Hỗ trợ chi phí nằm viện (đ/ngày)', stbh, years: yrs, annual, isSupp:true});
      }
      return { rows, total };
    }

    // Main annuals
    const baseAnnual = calculateMainPremium(mainPersonInfo);
    const extraAnnual = getExtraPremiumValue();
    const { rows: mainSuppRows, total: mainSuppAnnual } = computeSuppRows(mainPersonInfo, true);

    // Prepare table header
    const includeDiff = (freq !== 'year');
    const colHeads = [
      'Tên NĐBH', 'Sản phẩm', 'STBH', 'Số năm đóng phí',
      `Phí đóng (${freqLabel})`, 'Phí đóng (quy năm)'
    ];
    if (includeDiff) colHeads.push('Chênh lệch');

    let summaryHtml = `<div class="mb-4">
      <div class="text-lg font-semibold mb-2">Phần 1 · Tóm tắt sản phẩm (phí tính theo ${freqLabel})</div>
      <table class="w-full text-left border-collapse">
        <thead class="bg-gray-100"><tr>` +
        colHeads.map(h=>`<th class=\"p-2 border\">${h}</th>`).join('') +
        `</tr></thead><tbody>`;

    function renderRow(name, product, stbh, years, annual, isSupp, isSummary){
      if((annual||0)===0 && !isSummary) return '';
      const per = isSupp ? perPeriodSupp(annual) : perPeriodMainExtra(annual);
      const annFromPer = per*periods;
      const diff = includeDiff ? (annFromPer - (annual||0)) : 0;
      const stbhCell = stbh ? formatCurrency(stbh) : '—';
      const yearsCell = (years || years===0) ? years : '—';
      return `<tr>
        <td class="p-2 border">${sanitizeHtml(name||'')}</td>
        <td class="p-2 border">${sanitizeHtml(product||'')}</td>
        <td class="p-2 border text-right">${stbhCell}</td>
        <td class="p-2 border text-center">${yearsCell}</td>
        <td class="p-2 border text-right ${isSummary? 'font-bold':''}">${formatCurrency(per)}</td>
        <td class="p-2 border text-right">${formatCurrency(annFromPer)}</td>
        ${includeDiff? `<td class=\"p-2 border text-right\">${formatCurrency(diff)}</td>`:''}
      </tr>`;
    }

    // === MAIN PERSON BLOCK ===
    const mainName = mainPersonInfo.name || 'NĐBH chính';
    ...
    const mainTerm = (mainProduct === 'TRON_TAM_AN') ? 10 : (mainProduct === 'AN_BINH_UU_VIET' ? parseInt(document.getElementById('abuv-term')?.value || '0',10) : parseInt(document.getElementById('payment-term')?.value || '0',10));

    // Summary line for main person (total = base + extra + own supp + MDP3 if assigned to main)
    let mdp3AssignedId = null; let mdp3Fee = 0; let mdp3Stbh = 0;
    try{
      if(window.MDP3 && (document.getElementById('mdp3-enable')?.checked)){
        mdp3AssignedId = window.MDP3.getSelectedId && window.MDP3.getSelectedId();
        mdp3Fee = Number(window.MDP3.getPremium()||0);
        // reproduce stbhBase used inside MDP3.getPremium()
        let stbhBase = 0;
        if(window.personFees){
          for(let pid in window.personFees){ stbhBase += (window.personFees[pid].mainBase||0) + (window.personFees[pid].supp||0); }
          if(mdp3AssignedId !== 'other' && window.personFees[mdp3AssignedId]){
            stbhBase -= (window.personFees[mdp3AssignedId].supp||0);
          }
        }
        mdp3Stbh = stbhBase;
      }
    ...
    const mainTotalAnnual = baseAnnual + extraAnnual + mainSuppAnnual + ((mdp3AssignedId==='main-person-container')? mdp3Fee : 0);
    summaryHtml += renderRow(mainName, '— Tổng', null, null, mainTotalAnnual, false, true);

    // Main product row
    summaryHtml += renderRow(mainName, productLabel, mainStbh, mainTerm, baseAnnual, false, false);
    // Extra premium row (hide if 0)
    if(extraAnnual>0){ summaryHtml += renderRow(mainName, 'Phí đóng thêm', null, mainTerm, extraAnnual, false, false); }
    // Main person's supplementary rows
    mainSuppRows.forEach(r=>{ summaryHtml += renderRow(mainName, r.product, r.stbh, r.years, r.annual, true, false); });
    // MDP3 under main if assigned
    if(mdp3Fee>0 && mdp3AssignedId==='main-person-container'){
      const yrs = yearsForSupplement(true, mainPersonInfo.age, 'mdp3', mainPersonInfo.age);
      summaryHtml += renderRow(mainName, 'Miễn đóng phí 3.0', mdp3Stbh, yrs, mdp3Fee, true, false);
    }

    // === SUPPLEMENTARY PERSONS BLOCK ===
    suppPersons.forEach(p=>{
      const pName = p.name || 'NĐBH bổ sung';
      const { rows: suppRows, total: suppAnnual } = computeSuppRows(p, false);
      // Include mdp3 if assigned to this person
      let personAnnual = suppAnnual;
      if(mdp3Fee>0 && mdp3AssignedId===p.container?.id){ personAnnual += mdp3Fee; }
      // Summary
      summaryHtml += renderRow(pName, '— Tổng', null, null, personAnnual, true, true);
      // Detail rows
      suppRows.forEach(r=>{ summaryHtml += renderRow(pName, r.product, r.stbh, r.years, r.annual, true, false); });
      if(mdp3Fee>0 && mdp3AssignedId===p.container?.id){
        const yrs = yearsForSupplement(false, p.age, 'mdp3', mainPersonInfo.age);
        summaryHtml += renderRow(pName, 'Miễn đóng phí 3.0', mdp3Stbh, yrs, mdp3Fee, true, false);
      }
    });

    // MDP3 assigned to "Người khác"
    if(mdp3Fee>0 && mdp3AssignedId==='other'){
      // Age from form (if available)
      let otherAge = null; try{ const info = getCustomerInfo(document.getElementById('person-container-mdp3-other'), false); otherAge = info.age; }catch(e){}
      const yrs = (otherAge? Math.max(0, Math.min(otherAge, MAX_AGE_BY_PRODUCT.mdp3) - otherAge + 1) : '—');
      summaryHtml += renderRow('Người khác', 'Miễn đóng phí 3.0', mdp3Stbh, yrs, mdp3Fee, true, false);
    }

    summaryHtml += `</tbody></table></div>`;

    // ===== PHẦN 2: BẢNG PHÍ THEO NĂM (giữ nguyên nhưng ẩn cột chênh lệch khi kỳ = năm) =====
    let tableHtml = `<div class="mb-4"><div class="text-lg font-semibold mb-2">Phần 2 · Bảng phí (Kỳ: ${freqLabel})</div>`;
    ...
    suppPersons.forEach(person => { tableHtml += `<th class=\"p-2 border\">Phí bổ sung<br>(${sanitizeHtml(person.name)})</th>`; });
    tableHtml += `<th class=\"p-2 border\">Tổng cộng</th>`;
    if(includeDiff) tableHtml += `<th class=\"p-2 border\">Chênh lệch so với năm</th>`;
    tableHtml += `</tr></thead><tbody>`;

    let totalMaxSupport = Math.floor(baseAnnual / 4000000) * 100000; // Hạn mức chung Hỗ trợ viện phí
    ...
    for (let i = 0; (mainPersonInfo.age + i) <= targetAge; i++) {
      ...
      if (mainSuppContainer2) {
        suppPremiumMain += calculateHealthSclPremium({ ...mainPersonInfo, age: currentAgeMain }, mainSuppContainer2, currentAgeMain);
        suppPremiumMain += calculateBhnPremium({ ...mainPersonInfo, age: currentAgeMain }, mainSuppContainer2, currentAgeMain);
        suppPremiumMain += calculateAccidentPremium({ ...mainPersonInfo, age: currentAgeMain }, mainSuppContainer2, currentAgeMain);
        suppPremiumMain += calculateHospitalSupportPremium({ ...mainPersonInfo, age: currentAgeMain }, baseAnnual, mainSuppContainer2, totalHospitalSupportStbh, currentAgeMain);
        const hospitalSupportStbh = parseFormattedNumber(mainSuppContainer2.querySelector('.hospital-support-stbh')?.value || '0');
        if (mainSuppContainer2.querySelector('.hospital-support-checkbox')?.checked && hospitalSupportStbh > 0) {
          totalHospitalSupportStbh += hospitalSupportStbh;
        }
      }
      ...
      const suppPremiums = suppPersons.map(person => {
        ...
        if (suppProductsContainer) {
          ...
          suppPremium += calculateHospitalSupportPremium({ ...person, age: currentPersonAge }, baseAnnual, suppProductsContainer, totalHospitalSupportStbh, currentPersonAge);
      ...
      const cell = (v)=> (v>0 ? formatCurrency(v) : '');

      tableHtml += `<tr>
        <td class=\"p-2 border text-center\">${contractYear}</td>
        <td class=\"p-2 border text-center\">${currentAgeMain}</td>
        <td class=\"p-2 border text-right\">${cell(mainThisYearBase)}</td>
        <td class=\"p-2 border text-right\">${cell(extraThisYear)}</td>
        <td class=\"p-2 border text-right\">${cell(suppPremiumMain)}</td>`;
      suppPremiums.forEach(sp => { tableHtml += `<td class=\"p-2 border text-right\">${cell(sp)}</td>`; });
      tableHtml += `<td class=\"p-2 border text-right font-semibold\">${formatCurrency(totalFromPeriod)}</td>`;
      if(includeDiff) tableHtml += `<td class=\"p-2 border text-right\">${formatCurrency(diff)}</td>`;
      tableHtml += `</tr>`;
    }
