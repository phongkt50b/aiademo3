  ...
  (function() {
    function customGenerateSummary() {
      ...
      try {
        ...
        summaryHtml += `<th class="p-2 border">STBH</th>`;
        summaryHtml += `<th class="p-2 border">Số năm đóng phí</th>`;
        const periodLabel = (freq === 'year') ? 'năm' : (freq === 'half' ? 'nửa năm' : 'theo quý');
        summaryHtml += `<th class="p-2 border">Phí đóng (${periodLabel})</th>`;
        summaryHtml += `<th class="p-2 border">Phí đóng quy năm</th>`;
        summaryHtml += `<th class="p-2 border">Phí đóng theo năm</th>`;
        ...
        for (let i = 0; (mainInfo.age + i) <= targetAge; i++) {
          ...
          if (mdp3Fee > 0) {
            if (mdp3Sel === mainContainer.id || !mdp3Sel) {
              ...
              // counted in a separate person row (other) in part 1; here we add to total only
            } else {
          ...
          const totalAnnualYear = mainThisYear + extraThisYear + suppMainYear + suppYears.reduce((a, b) => a + b, 0);
          ...
          const annualSuppYear = suppMainYear + suppYears.reduce((a, b) => a + b, 0);
          ...
          if (freq !== 'year') tableHtml += `<td class="p-2 border text-right">${formatCurrency(diff)}</td>`;
          tableHtml += `</tr>`;
        }
        tableHtml += `</tbody></table></div>`;
        ...
        if (container) {
          container.innerHTML = summaryHtml + tableHtml + `<div class="mt-4 text-center"><button id="export-html-btn" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">Xuất HTML</button></div>`;
          // After injecting HTML, fix Part 1 summary table columns (merge duplicate annual columns, compute baseline correctly)
          try {
            fixPart1Table(container, periods, factor);
          } catch (e) {
            console.error('fixPart1Table error', e);
          }
        }
        document.getElementById('export-html-btn')?.addEventListener('click', () => window.print());
