...
(() => {
  ...
  function renderSection6V2(){
    try{
      ...
      if(list){
        list.innerHTML = '';
        // Determine MDP3 selection and fee
        let mdp3SelectedId = null;
        let mdp3Fee = 0;
        let mdp3OtherName = '';
        try {
        ...
        // Render supplementary insured persons
        document.querySelectorAll('#supplementary-insured-container .person-container').forEach((cont, idx) => {
          ...
          const row = document.createElement('div');
          row.className = 'flex justify-between items-center py-1 text-sm';
          // Only display name and fee (no prefix)
          row.innerHTML = `<span>${sanitizeHtml(name)}</span><span class="font-semibold">${fmt(fee)}</span>`;
          list.appendChild(row);
        });
        // If MDP3 is assigned to other, show as separate line
        if (mdp3SelectedId === 'other' && mdp3Fee > 0) {
          const row = document.createElement('div');
          row.className = 'flex justify-between items-center py-1 text-sm';
          const displayName = sanitizeHtml(mdp3OtherName || 'Người khác');
          row.innerHTML = `<span>${displayName}</span><span class="font-semibold">${fmt(mdp3Fee)}</span>`;
          list.appendChild(row);
        }
      }
