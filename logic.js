...
function performCalculations(state) {
    ...
    allPersons.forEach(p => {
        fees.byPerson[p.id] = { name: (p && (p.name||'')), main: 0, supp: 0, total: 0, suppDetails: {} };
    });
...
function updateSummaryUI(fees) {
  ...
  set('freq-diff', diff);
  set('freq-annual-eq', annualEq);
  set('freq-annualized', annualEq);
  set('freq-annual', annualEq);

  try { renderPerPersonSuppOnlyList(); } catch(e) {}
}


// === Per-person supplement-only breakdown (for 'Xem từng người') ===
function renderPerPersonSuppOnlyList() {
  try {
    const listContainer = document.getElementById('per-person-list') || document.getElementById('view-each-person-list') || document.getElementById('each-person-breakdown');
    if (!listContainer) return; // Only render if host page provides container
    const by = (appState && appState.fees && appState.fees.byPerson) ? appState.fees.byPerson : {};
    const rows = [];
    Object.keys(by).forEach(pid => {
      const item = by[pid];
      const name = (item && item.name) ? item.name : pid;
      const supp = (item && item.supp) ? item.supp : 0;
      if (supp > 0) {
        rows.push(`<div class="flex justify-between py-1"><span>${sanitizeHtml(name)}</span><span class="font-semibold">${formatDisplayCurrency(supp)}</span></div>`);
      }
    });
    listContainer.innerHTML = rows.length ? rows.join('') : '<div class="text-gray-500">Chưa có phí SP bổ sung</div>';
  } catch(e) { /* no-op */ }
}
...
// === Annualization helpers for other sections (e.g., Bảng 2) ===
function getAnnualizedRiderFactor() {
  const v = document.getElementById('payment-frequency')?.value || 'year';
  if (v === 'half') return 1.02;
  if (v === 'quarter') return 1.04;
  return 1;
}
function getAnnualizedRiderFee(n) { return Math.round(((Number(n)||0) * getAnnualizedRiderFactor())/1000)*1000; }
...
function generateSummaryTable() {
    ...
    if (mdpEnabled && mdpTargetId === 'other') {
      ...
      let nameOther = 'Người khác (MĐP3)';
      if (form) {
        const info = collectPersonData(form, false);
        ageOther = info.age || mainInfo.age;
        if (info.name) nameOther = info.name;
      }
      persons.push({ id:'mdp3_other', isMain:false, name:nameOther, gender:'Nam', age:ageOther, supplements:{} });
    }
    ...
    const pushRow = (acc, personName, prodName, stbhDisplay, years, baseAnnual, isRider)=>{
      const displayAnnual = (!isAnnual && isRider) ? Math.round((baseAnnual*riderFactor)/1000)*1000 : baseAnnual;
      const perPeriod = isAnnual ? 0 : Math.round((displayAnnual)/periods/1000)*1000;
      const annualEq = isAnnual ? 0 : perPeriod*periods;
      const diff = isAnnual ? 0 : (annualEq - displayAnnual);
      acc.per += perPeriod; acc.eq += annualEq; acc.base += displayAnnual; acc.diff += diff;
      let row = `<tr>
    ...
    html += `<tr class="bg-gray-100 font-bold"><td class="p-2 border" ${ (isAnnual?4:6) }>Tổng tất cả</td>` + (!isAnnual?`<td class=\"p-2 border text-right\">${fmt(grand.per)}</td><td class=\"p-2 border text-right\">${fmt(grand.eq)}</td>`:'') + `<td class=\"p-2 border text-right\">${fmt(grand.base)}</td>` + (!isAnnual?`<td class=\"p-2 border text-right\"><span class=\"text-red-600 font-bold\">${fmt(grand.diff)}</span></td>`:'') + `</tr>`;
      {
