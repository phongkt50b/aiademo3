    ...
    try{
      ...
      if(fb){
        if(freq==='year'){
          ...
          if(elDiff) {
            elDiff.textContent = fmt(info.diff);
            // Always highlight the difference in red and bold for non-year frequencies
            elDiff.classList.add('font-semibold', 'text-red-600');
          }
...
(function() {
  ...
  function updatePeriodBreakdown() {
    ...
    // Set the difference text
    setText("#freq-diff", diff);

    // Always highlight the difference in red and bold when the frequency is not yearly
    const diffEl = document.getElementById('freq-diff') || $$("#freq-diff");
    if (diffEl) {
      // Apply bold and red classes for emphasis
      diffEl.classList.add('font-semibold', 'text-red-600');
      const row = diffEl.closest('div');
      // Hide the entire row only if the difference is zero
...
(function(){
  ...
  function buildProductRows(personInfo, container, targetAge, freq, baseAnnualMain) {
    ...
    if (sclSec && sclSec.querySelector('.health-scl-checkbox')?.checked) {
      ...
      if (perFee > 0) {
        const periods = (freq === 'half') ? 2 : (freq === 'quarter' ? 4 : 1);
        const annualFromPer = perFee * periods;
        const diff = annualFromPer - feeAnnual;
        const showDiff = freq !== 'year';
        let row = `<tr>`;
        row += `<td class="p-2 border">Sức khoẻ Bùng Gia Lực ${programLabel ? `- ${programLabel}`:''}</td>`;
        row += `<td class="p-2 border text-right">${formatCurrency(stbh)}</td>`;
        row += `<td class="p-2 border text-center">${years}</td>`;
        row += `<td class="p-2 border text-right">${formatCurrency(perFee)}</td>`;
        // Show annual column only when paying half-year or quarterly
        if (freq !== 'year') {
          row += `<td class="p-2 border text-right">${formatCurrency(annualFromPer)}</td>`;
        }
        // Show difference column only when not paying yearly
        if (showDiff) row += `<td class="p-2 border text-right font-semibold text-red-600">${formatCurrency(diff)}</td>`;
        row += `</tr>`;
        rows.push(row);
      }
    }
    ...
    if (bhnSec && bhnSec.querySelector('.bhn-checkbox')?.checked) {
      ...
      if (perFee > 0) {
        ...
        const showDiff = freq !== 'year';
        let row = `<tr>`;
        row += `<td class="p-2 border">Bệnh hiểm nghèo 2.0</td>`;
        row += `<td class="p-2 border text-right">${formatCurrency(stbh)}</td>`;
        row += `<td class="p-2 border text-center">${years}</td>`;
        row += `<td class="p-2 border text-right">${formatCurrency(perFee)}</td>`;
        if (freq !== 'year') {
    ...
    if (accSec && accSec.querySelector('.accident-checkbox')?.checked) {
      ...
      if (perFee > 0) {
        ...
        const showDiff = freq !== 'year';
        let row = `<tr>`;
        row += `<td class="p-2 border">Bảo hiểm Tai nạn</td>`;
        row += `<td class="p-2 border text-right">${formatCurrency(stbh)}</td>`;
        row += `<td class="p-2 border text-center">${years}</td>`;
        row += `<td class="p-2 border text-right">${formatCurrency(perFee)}</td>`;
        if (freq !== 'year') {
    ...
    if (hsSec && hsSec.querySelector('.hospital-support-checkbox')?.checked) {
      ...
      if (perFee > 0) {
        ...
        const showDiff = freq !== 'year';
        let row = `<tr>`;
        row += `<td class="p-2 border">Hỗ trợ chi phí nằm viện (đ/ngày)</td>`;
        row += `<td class="p-2 border text-right">${formatCurrency(stbh)}</td>`;
        row += `<td class="p-2 border text-center">${years}</td>`;
        row += `<td class="p-2 border text-right">${formatCurrency(perFee)}</td>`;
        if (freq !== 'year') {
  ...
  function buildPersonSummary(personInfo, personId, targetAge, freq, paymentTerm, baseAnnualMain) {
    ...
    // Compute per period premiums and annual totals
    ...
    const periods = (freq === 'half') ? 2 : (freq === 'quarter' ? 4 : 1);
    const annualTotal = baseAnnual + extraAnnual + suppAnnual;
    const annualFromPer = perTotal * periods;
    const diffTotal = annualFromPer - annualTotal;
    // Skip entire section if total is zero
    if (perTotal <= 0) return '';
    // Determine whether to show the annual and difference columns (hide when paying yearly)
    const showAnnualCol = freq !== 'year';
    const showDiffCol = freq !== 'year';
    // First row: name & total premium for this person (per period and annual, diff)
    html += `<tr>`;
    html += `<td class="p-2 border font-semibold">${sanitizeHtml(name)}</td>`;
    html += `<td class="p-2 border text-right">—</td>`;
    html += `<td class="p-2 border text-center">—</td>`;
    // Per-period total always displayed
    html += `<td class="p-2 border text-right font-semibold">${formatCurrency(perTotal)}</td>`;
    // Annual total column (only when not paying yearly)
    if (showAnnualCol) {
      html += `<td class="p-2 border text-right font-semibold">${formatCurrency(annualFromPer)}</td>`;
    }
    // Difference column (only when not paying yearly)
    if (showDiffCol) {
      html += `<td class="p-2 border text-right font-semibold text-red-600">${formatCurrency(diffTotal)}</td>`;
    }
    html += `</tr>`;
    ...
    if (isMain && perBase > 0) {
      ...
      const annualFromPerBase = perBase * periods;
      const diffBase = annualFromPerBase - baseAnnual;
      html += `<tr>`;
      html += `<td class="p-2 border">${sanitizeHtml(productLabel)}</td>`;
      html += `<td class="p-2 border text-right">${formatCurrency(stbhMain)}</td>`;
      html += `<td class="p-2 border text-center">${yearsMain}</td>`;
      html += `<td class="p-2 border text-right">${formatCurrency(perBase)}</td>`;
      if (showAnnualCol) {
        html += `<td class="p-2 border text-right">${formatCurrency(annualFromPerBase)}</td>`;
      }
      if (showDiffCol) {
        // Highlight difference in bold red for clarity
        html += `<td class="p-2 border text-right font-semibold text-red-600">${formatCurrency(diffBase)}</td>`;
      }
      html += `</tr>`;
    }
    // Extra premium row (only for main person)
    if (isMain && perExtra > 0) {
      const annualFromPerExtra = perExtra * periods;
      const diffExtra = annualFromPerExtra - extraAnnual;
      html += `<tr>`;
      html += `<td class="p-2 border">Phí đóng thêm</td>`;
      html += `<td class="p-2 border text-right">—</td>`;
      html += `<td class="p-2 border text-center">${paymentTerm || '—'}</td>`;
      html += `<td class="p-2 border text-right">${formatCurrency(perExtra)}</td>`;
      if (showAnnualCol) {
        html += `<td class="p-2 border text-right">${formatCurrency(annualFromPerExtra)}</td>`;
      }
      if (showDiffCol) {
        // Highlight difference in bold red
        html += `<td class="p-2 border text-right font-semibold text-red-600">${formatCurrency(diffExtra)}</td>`;
      }
      html += `</tr>`;
    }
    ...
    if (mdp3Fee > 0 && mdp3Stbh > 0 && mdp3Years > 0 && (window.MDP3.getSelectedId && window.MDP3.getSelectedId()) === personId) {
      ...
      if (perMdp > 0) {
        const annualFromPerMdp = perMdp * periods;
        const diffMdp = annualFromPerMdp - mdp3Fee;
        html += `<tr>`;
        html += `<td class="p-2 border">Miễn đóng phí 3.0</td>`;
        html += `<td class="p-2 border text-right">${formatCurrency(mdp3Stbh)}</td>`;
        html += `<td class="p-2 border text-center">${mdp3Years}</td>`;
        html += `<td class="p-2 border text-right">${formatCurrency(perMdp)}</td>`;
        // Show annual and difference columns only when not paying yearly
        if (showAnnualCol) {
          html += `<td class="p-2 border text-right">${formatCurrency(annualFromPerMdp)}</td>`;
        }
        if (showDiffCol) {
          html += `<td class="p-2 border text-right font-semibold text-red-600">${formatCurrency(diffMdp)}</td>`;
        }
        html += `</tr>`;
      }
    }
  ...
  function buildMdp3OtherSummary(targetAge, freq) {
    try {
      ...
      if (fee > 0 && selId === 'other') {
        ...
        // Determine mdp years using computeSupplementYears (treat other as supplementary person)
        let mainInfo = getCustomerInfo(document.getElementById('main-person-container'), true);
        let mdpYears = 0;
        try {
          if (typeof computeSupplementYears === 'function') {
            mdpYears = computeSupplementYears(otherInfo.age || 0, mainInfo.age || 0, targetAge, 65);
          } else {
            mdpYears = Math.max(0, Math.min(targetAge, 65) - (otherInfo.age || 0) + 1);
          }
        } catch (e) {
          mdpYears = Math.max(0, Math.min(targetAge, 65) - (otherInfo.age || 0) + 1);
        }
        ...
        if (perMdp <= 0) return '';
        const periods = (freq === 'half') ? 2 : (freq === 'quarter' ? 4 : 1);
        const annualFromPer = perMdp * periods;
        const diff = annualFromPer - fee;
        // Determine whether to show annual and difference columns
        const showAnnualCol = freq !== 'year';
        const showDiffCol = freq !== 'year';
        let html = ``;
        // Summary row for the other person (name row)
        html += `<tr>`;
        html += `<td class="p-2 border font-semibold">${sanitizeHtml(name)}</td>`;
        html += `<td class="p-2 border text-right">—</td>`;
        html += `<td class="p-2 border text-center">—</td>`;
        html += `<td class="p-2 border text-right font-semibold">${formatCurrency(perMdp)}</td>`;
        if (showAnnualCol) {
          html += `<td class="p-2 border text-right font-semibold">${formatCurrency(annualFromPer)}</td>`;
        }
        if (showDiffCol) {
        ...
        html += `</tr>`;
        // Product row for MDP3
        html += `<tr>`;
        html += `<td class="p-2 border">Miễn đóng phí 3.0</td>`;
        html += `<td class="p-2 border text-right">${formatCurrency(stbhBase)}</td>`;
        html += `<td class="p-2 border text-center">${mdpYears}</td>`;
        html += `<td class="p-2 border text-right">${formatCurrency(perMdp)}</td>`;
        if (showAnnualCol) {
          html += `<td class="p-2 border text-right">${formatCurrency(annualFromPer)}</td>`;
        }
        if (showDiffCol) {
          html += `<td class="p-2 border text-right font-semibold text-red-600">${formatCurrency(diff)}</td>`;
        }
  ...
  function customGenerateSummary() {
    ...
    try {
      ...
      // Dynamic period label: năm, nửa năm hoặc quý
      const periodLabel = (freq === 'year') ? 'Phí đóng (năm)' : (freq === 'half' ? 'Phí đóng (nửa năm)' : 'Phí đóng (quý)');
      // Determine whether to show the annual and diff columns for Part 1
      const showAnnualColPart1 = freq !== 'year';
      const showDiffColPart1 = freq !== 'year';
      html += `<th class="p-2 border">${periodLabel}</th>`;
      if (showAnnualColPart1) {
      ...
      // Determine if there is an MDP3-other person
      let mdp3Fee = 0;
      let mdp3Sel = null;
      let mdp3OtherName = '';
      let mdp3Years = 0;
      ...
      if (mdp3Fee > 0 && mdp3Sel === 'other') {
        ...
        const otherInfo = getCustomerInfo(otherForm, false);
        // Compute mdp3Years using computeSupplementYears
        try {
          if (typeof computeSupplementYears === 'function') {
            mdp3Years = computeSupplementYears(otherInfo.age || 0, mainInfo.age || 0, targetAge, 65);
          } else {
            mdp3Years = Math.max(0, Math.min(targetAge, 65) - (otherInfo.age || 0) + 1);
          }
        } catch (e) {
          mdp3Years = Math.max(0, Math.min(targetAge, 65) - (otherInfo.age || 0) + 1);
        }
      }
      const hasMdpOther = (mdp3Fee > 0 && mdp3Sel === 'other');
      // Add headers for each supplementary person
      suppPersons.forEach(p => {
      ...
      let sumExtra = 0;
      let sumSuppMain = 0;
      const sumSuppEach = new Array(suppPersons.length).fill(0);
      let sumMdpOther = 0;
      let sumPerTotal = 0;
      let sumDiff = 0;
      for (let i = 0; (mainInfo.age + i) <= targetAge; i++) {
        ...
        try {
          if (mdp3Fee > 0) {
            if (mdp3Sel === 'main-person-container') {
              ...
              // Apply only within mdp3Years
              mdp3OtherVal = (contractYear <= mdp3Years) ? mdp3Fee : 0;
            } else {
        ...
        // Annual supplementary total including mdp3OtherVal
        const annualSupp = suppMain + suppEachArr.reduce((a,b) => a+b,0) + mdp3OtherVal;
        ...
        html += `<tr>`;
        html += `<td class="p-2 border text-center">${contractYear}</td>`;
        html += `<td class="p-2 border text-center">${ageThisYear}</td>`;
        html += `<td class="p-2 border text-right">${formatCurrency(mainThisYear)}</td>`;
        html += `<td class="p-2 border text-right">${formatCurrency(extraThisYear)}</td>`;
        html += `<td class="p-2 border text-right">${formatCurrency(suppMain)}</td>`;
        suppEachArr.forEach((val, idx) => {
          html += `<td class="p-2 border text-right">${formatCurrency(val)}</td>`;
        });
        if (hasMdpOther) {
          html += `<td class="p-2 border text-right">${formatCurrency(mdp3OtherVal)}</td>`;
        }
        html += `<td class="p-2 border text-right font-semibold">${formatCurrency(perTotal)}</td>`;
        if (freq !== 'year') {
          // Highlight difference column in bold red for visibility
          html += `<td class="p-2 border text-right font-semibold text-red-600">${formatCurrency(diff)}</td>`;
        }
        html += `</tr>`;
        // accumulate sums
        sumMain += mainThisYear;
        sumExtra += extraThisYear;
        sumSuppMain += suppMain;
        suppEachArr.forEach((val, idx) => { sumSuppEach[idx] = (sumSuppEach[idx] || 0) + val; });
        if (hasMdpOther) {
          sumMdpOther += mdp3OtherVal;
        }
        sumPerTotal += perTotal;
        if (freq !== 'year') sumDiff += diff;
      }
      // Add final totals row
      html += `<tr>`;
      // first two columns merged label
      html += `<td class="p-2 border font-semibold text-center" colspan="2">Tổng</td>`;
      html += `<td class="p-2 border text-right font-semibold">${formatCurrency(sumMain)}</td>`;
      html += `<td class="p-2 border text-right font-semibold">${formatCurrency(sumExtra)}</td>`;
      html += `<td class="p-2 border text-right font-semibold">${formatCurrency(sumSuppMain)}</td>`;
      suppPersons.forEach((p, idx) => {
        html += `<td class="p-2 border text-right font-semibold">${formatCurrency(sumSuppEach[idx] || 0)}</td>`;
      });
      if (hasMdpOther) {
        html += `<td class="p-2 border text-right font-semibold">${formatCurrency(sumMdpOther)}</td>`;
      }
      html += `<td class="p-2 border text-right font-semibold">${formatCurrency(sumPerTotal)}</td>`;
      if (freq !== 'year') {
        // Highlight total difference in bold red
        html += `<td class="p-2 border text-right font-semibold text-red-600">${formatCurrency(sumDiff)}</td>`;
      }
      html += `</tr>`;
      html += `</tbody></table></div>`;
